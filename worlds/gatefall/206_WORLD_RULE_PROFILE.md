# Gatefall — World Rule Profile 1.85
...
**Two content systems no longer read this ladder for their population.** The penalty zone (Section 8.3) and the class trial (Section 18.2) are sized by the **Combat Tier** (Section 6.7) from Profile 1.67 forward. A population sized to a Rank *label* is calibrated against an ordinary crew of that Rank, never against the one person in the world who grows. System Rank remains their floor.

**These two and no others, because these two are the ones the System drops on him.** A penalty zone he does not choose to enter and a class trial he cannot retreat from are content the System selects *for* him, so the System is what must size them honestly. Everywhere else the Bearer picks his own difficulty and the profile lets him: an instant dungeon runs at its key's own recorded Rank (Section 17), and a public Gate runs at the Rank BGM assessed it (Section 9) — a posted Gate is contracted against, staffed to a legal minimum and insured on that number, so it cannot bend around one hunter without the assessment becoming a fiction. That the Bearer's *chosen* content can therefore be easier than his *imposed* content is the intended shape, not a gap.
...
`standing_worn_reduction` is the reduction the equipped loadout carries at all times. A once-per-exchange or once-per-scene effect is a tactical resource the Bearer may spend, hold, or waste — not standing protection — and is **excluded**. Five hits is the attrition band where Endurance is actually tested: a Bearer who absorbs twenty is never pressured, and one who absorbs two dies to a single unlucky exchange without the content having measured him.
...
**Two consumers, and they are the imposed ones.** The class trial (Section 18.2) and the penalty zone (Section 8.3) read this tier, and nothing else does. Both are content the System drops on the Bearer — one he cannot retreat from, one he did not choose to enter — so the System is what must size them honestly. Where he picks his own difficulty the profile lets him pick badly: an instant dungeon runs at its key's own recorded Rank (Section 17), and a public Gate at the Rank BGM assessed (Section 9). **The System owes an honest fight only where it picked the fight.**
...
These are aggregate rates across the metro. **They are rolled, not narrated at need.**
...
1. **Gates.** Roll the metro's day count — Chicago **1d4+3**, Prague **1d2**. For each, roll its true Rank on Section 9.2.
...
- **Rank.** A key's Rank comes from its source, and there are exactly three: a **boss drop** carries the Rank of the Gate that dropped it (Section 11.2); a **shop purchase** carries the explicit Rank bought off the Section 12.5 catalogue, at that Rank's price; a **Daily Premium Key** carries the Rank its own d100 Rank roll produced at rotation (Section 12.5). The instance runs at the key's recorded Rank and **never re-Ranks later** — a key keeps the Rank it was created at, so growth after acquisition does not raise it and a stockpiled key is never retroactively hardened. A key's Rank is therefore never read from the Bearer's own standing at the moment of use, and an instant dungeon is not sized to him: he chooses his own difficulty here by choosing which key to spend, which is the whole of this section's risk economy.
...
- **Core mechanic — three abilities:**
  ...
  - **Standing anchor sense** — a set anchor is a place he remains **connected to**, not only a place he can return to. While an anchor is set, **disturbance there reaches him**: presence where there was none, violence, or a mana event. It returns **the fact and never the detail** — no identity, no count, no intent; knowing *what* is Keen Sense's work and Keen Sense requires being present. Bounded exactly as an anchored step is: the same instance, Gate, or contiguous stretch of the city, and never across that boundary. **It costs nothing, takes no action, and pays from every set anchor at once.**
- **Acquisition — rift anchors.** Standing an exchange in a place he has reason to hold, the Bearer may **set an anchor** there: a fixed point he can rift-step to later **without line of sight**, from anywhere in the same instance, Gate, or contiguous stretch of the city. Setting is free and takes the exchange; an anchor is permanent until released, and **releasing is free, instant, and taken at will from anywhere** (1.72) — the exchange is the price of *setting*, not of letting go, so the cap is a working set rather than a permanent commitment: release what has stopped earning its slot and set the replacement when he is next standing there. **Anchor cap = effective Agility ÷ 20** (floor, minimum 1): Agility 100 holds 5, Agility 200 holds 10. Anchors are the class's collection, and they are what the evolution is built from — a Riftwalker's power is a **map**, accumulated one held position at a time.
- **Resource costs.** As above; all actives ride the pool (Section 5). Rift-stepping to an anchor rather than a visible point costs the Rank-ladder Mana of the instance's Rank rather than a flat 6, and does not consume the once-per-exchange line-of-sight step. **Outside any instance — the contiguous-city case — the ladder reads at E-Rank: 5 Mana** (1.72). Ordinary city ground is the bottom of the ladder because crossing air nobody warded is not the act of crossing a sealed instance, and the flat 6 is deliberately *not* used here: this sentence names it as what an anchored step costs instead of, so borrowing it back for the city would overrule the rule rather than complete it. The constraint on this half of the class is the anchor cap, the per-scene rung and Section 19.1 exposure — not the Mana price.
- **Growth path — authored rungs, read against System Rank** (Section 6.6):

| System Rank | Rift-step reach | Deflection | Anchor use | Anchor sense |
|---|---|---:|---|---|
| **C** | line of sight | 50% | one anchored step per scene | he may **check** an anchor deliberately and learn whether it has been disturbed |
| **B** | line of sight, or **through** an obstruction he has perceived past | 60% | two per scene | disturbance reaches him **as it happens**, unprompted, wherever he is |
| **A** | any point within 100 m, sight or no sight | 70% | unlimited | it resolves into **which kind** — presence, violence, or mana |
| **S** | any point in the instance | 70%, and a deflection may **return** the attack to its source | unlimited, and an ally he touches may be sent instead | one anchor at a time may be sensed **as though he stood at it**, for one exchange |
...
```text
╔══ ◈  R I F T S ═════════════════════════════════════════════════════════════
     Rift-step       <reach>              Mana 6    <ready | used this exchange>
     Deflection      <n>% reduction       Mana 8    reaction
     Anchor sense    <rung>
     Anchors         <filled> / <cap>     (cap = Agility ÷ 20)
╟──  A N C H O R S  ─────────────────────────────────────────────────────────╢
     <name>          <place>       <steps left this scene>   <quiet | DISTURBED>
╚═════════════════════════════════════════════════════════════════════════════
```
...
╟──  A N C H O R S  ─────────────────────────────────────────────────────────╢
     The landing     Rogers Park stairwell     1 of 1    quiet
     Coldwater       Frozen Gallery, vein 3    1 of 1    DISTURBED
     The lot         behind Ironline Reg V     1 of 1    quiet
╚═════════════════════════════════════════════════════════════════════════════
```
