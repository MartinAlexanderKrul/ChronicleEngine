# Gatefall — World Rule Profile 1.71
...
- **Acquisition — rift anchors.** Standing an exchange in a place he has reason to hold, the Bearer may **set an anchor** there: a fixed point he can rift-step to later **without line of sight**, from anywhere in the same instance, Gate, or contiguous stretch of the city. Setting is free and takes the exchange; an anchor is permanent until released. **Anchor cap = effective Agility ÷ 20** (floor, minimum 1): Agility 100 holds 5, Agility 200 holds 10. Anchors are the class's collection, and they are what the evolution is built from — a Riftwalker's power is a **map**, accumulated one held position at a time.
- **Resource costs.** As above; all actives ride the pool (Section 5). Rift-stepping to an anchor rather than a visible point costs the Rank-ladder Mana of the instance's Rank rather than a flat 6, and does not consume the once-per-exchange line-of-sight step.
- **Growth path — authored rungs, read against System Rank** (Section 6.6):
