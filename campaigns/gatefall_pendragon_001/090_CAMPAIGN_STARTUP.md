...
world_rule_profile: "Gatefall World Rule Profile 1.123"
...
latest_restorable_checkpoint: campaigns/gatefall_pendragon_001/saves/900_CHECKPOINT_0164
...
6. **Fix a channel violation or continuity slip by re-narrating, not through in-character self-correction.** Never have an NPC awkwardly catch and retcon its own line ("oh, I made that up") — acknowledge the miss briefly out-of-character if needed, then just deliver the corrected version directly. Where reasonable, prefer inferring the fact was actually established off-screen over concluding a hard retcon is needed.
7. **Resolve a pending numeric payout (severity bonus, fee, etc.) the moment the scene reasonably allows it** — don't leave "amount undetermined, still filing" dangling across turns for the player to chase a second or third time. Author the concrete figure grounded in the campaign's established economic scale and deliver it in-fiction.
8. **Give each loaded NPC a genuinely distinct voice** — rhythm, humor, physicality, reasoning style — not a shared "clipped, states things plainly" register regardless of who's speaking. Two characters can both be terse for completely different underlying reasons; if two NPCs' lines would read the same with names swapped, that's the failure. Related to the still-open `F-057` (itemized-exposition dialogue) and `F-066` (NPC scenes shaped as question-loops, raised this session) — same root cause at three different scales (single turn / whole scene / cross-NPC register).
9. **Never contest the player's account of established canon from an incomplete search.** Before pushing back on something the player states about canon, search exhaustively — later/superseding events, promoted chronicle entries, not just the first grep hit — and weight recency (a later Event supersedes an earlier one by default). If still genuinely unclear after a real search, present it as an open question rather than an argued position, and lean toward the player's account.
...
12. **Send the ledger HTML files whenever a panel links them.** Confirmed 2026-09-26. When a `/system` panel serves a derived-ledger link (`assets/alexander_pendragon_skill_ledger.html`, `assets/alexander_pendragon_inventory_ledger.html`), **send the file** in the same turn, without asking. The player follows the session from a phone and cannot open a repository path; a link with no file is the same "partial render they cannot tell from complete" failure Profile 1.120 was written to remove. Regenerate first if the sheet has moved since the last save.

13. **Once a task is in a guild office's hands, nothing further is needed from Alexander personally.** Confirmed 2026-09-26, after two corrections in one session. A roster, a commander's name, a Region V filing or a staffing decision handed to an office is **that office's job end to end**. Do not list it among his open items, do not put its deadline on his clock, and do not narrate him doing it. Assume the office meets the deadline unless the player says otherwise or the office visibly fails on screen. Surface it again only when the office needs something only he can give.
