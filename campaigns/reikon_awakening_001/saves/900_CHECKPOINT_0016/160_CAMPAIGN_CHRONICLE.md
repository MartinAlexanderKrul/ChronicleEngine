...
The Rift-clear milestone XP gap is resolved: Section 8.2 specifies a flat x10 grade-baseline multiplier (World Rule Profile 0.7), and the Ashgate clear was retroactively awarded its 70 XP milestone, crossing the threshold into Level 3. Two further rulings now govern growth going forward: Stats have no upper bound (Section 4.2), and Ascensions grant 5 points instead of 2 (Section 4.1), applied retroactively. **8 allocation points are unspent**, awaiting Daedalus's own decision on Stats versus Abilities.
