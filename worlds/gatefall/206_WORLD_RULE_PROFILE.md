# Gatefall — World Rule Profile 1.96
...
- **Core mechanic — three abilities:**
  - **Rift-step** — a true teleport, not a dash. **Two uses, and they are different actions**:
    - **The in-scene step** — to any point in **line of sight**; Mana 6, **once per exchange**, granting +1 modifier step to the immediate follow-up action. This is the combat tool, and it is the one that overlaps Flash Step; it is kept because a Riftwalker without it would have to buy a rune to fight.
    - **The travelling step** — to **any place he has personally stood**, at the Mana ladder below. It **takes the exchange and removes him from the scene**: this is travel, not a manoeuvre, and it cannot be spent to leave a fight and return inside it. The one exception is the answering step below.
...
- **Mana by distance, and distance is measured in *containers*, never in metres.** Gatefall tracks no coordinates and the Runtime is not asked to invent any. Every rung below is a question about which **ledgered container** the origin and the destination sit in — objects the world already carries because other sections already needed them: places are entities (`210_PLACES.md` and the campaign's own ledger), **districts are tracked state under Section 9.1.3**, and a metro is Chicago or Prague. Read the two ends, take the smallest container holding both, and pay its rung:

```text
     both ends in the same scene, in line of sight     6     (the in-scene step)
     both ends inside the same place entity            8     a building, an instance, a Gate
     both ends inside the same district                12
     both ends inside the same metro                   20
     different metros                                  40
```

  **Where the ledger does not place a destination in a district, it prices at the metro rung.** Ambiguity resolves *against* the Bearer, always, so that a vague destination is never the cheap one and nobody has to argue about a boundary mid-scene. **An instance's interior prices as the district its mouth stands in** — a Gate interior is a slice of another world (Section 10) and sits in no district of ours, but its mouth does.

- **No rift-step crosses an instance boundary, at any Rank, in either direction.** Not out of a Gate, a Section 9.6 red gate, a Section 17 instant dungeon, a Section 8.3 penalty zone or a Section 18.2 trial, and not into one. **This is stated as a hard limit rather than left to the distance table**, because every one of those sections rests on it: a red gate *"seals on entry and cannot be exited until its boss dies"*, a penalty zone has *"no other exit and no retreat"*, the clear window is what makes Section 13.2's loot declaration mean anything, and Section 9.4's legal minimums assume a crew that is committed once it crosses. **The mouth is the only way home, for him as for everyone.** An anchor set inside an instance is reachable while he is inside it and goes quiet the moment he leaves — and the instance's collapse releases it, returning the slot.
...
- **What an anchor grants over an ordinary destination** — three things, by rung below: **arrival is never blind**, **the answering step** (rift to a disturbed anchor as a *reaction*, from anywhere, the one travelling step that does not remove him from a scene), and **warded ground** (his own rift-space, where deflection is free).
...
```text
╔══ ◈  R I F T S ═════════════════════════════════════════════════════════════
     In-scene step   line of sight        Mana 6    <ready | used this exchange>
     Travelling step anywhere he has stood  <6|8|12|20|40 by container>
...
```text
╔══ ◈  R I F T S ═════════════════════════════════════════════════════════════
     In-scene step   line of sight        Mana 6    ready
     Travelling step anywhere he has stood  6 | 8 | 12 | 20 | 40 by container
