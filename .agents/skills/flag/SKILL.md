...
- It does not evaluate, implement, or resolve the flag — that's a separate, explicitly-requested piece of work (via the spawned task or a direct ask), never bundled into `/flag` itself. **When that work is asked for, use the `engine-change` skill**, which carries what actioning a flag owes: Decision 069's classification test, the roadmap milestone that must be named, the world-side version-bump file set, and the line at which world authoring stops and campaign canon begins.
