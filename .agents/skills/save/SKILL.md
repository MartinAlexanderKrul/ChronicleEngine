...
This reverses the campaign's earlier practice of writing `100_CHARACTER_SHEET.md`/`180_CURRENT_STATE.md` continuously through a session (visible in this campaign's history as repeated "live, unpromoted-to-checkpoint" edits) — that pattern is retired. A shop roll or a combat exchange is now held as a pending delta and narrated from memory of the conversation, never confirmed by re-reading a file you just wrote. Only `/save` derives the target set from the session's events and commits it to disk.

**One exception, and only one: a closed-channel ruling.** When the player rules that an NPC cannot know something, that row goes into the Closed Channels table in `130_NPCS_AND_FACTIONS.md` immediately — mid-scene, before the next line. It mints no identifier, bumps no provenance, and changes no canonical state, so it is outside what the prohibition above protects. It is written at once because a ruling held in conversation evaporates with the scene, and the table's existing rows record the same fact leaking two and three times after correction (F-003, `npc-knowledge` skill). Nothing else earns this exception.
...
2. **Completion contract.** A checkpoint exists only when ALL of these are true on disk:
   ...
   - Live ledgers updated from the session's **derived** target set (from its events — not from recollection of what changed). Every Character an Event references also brings its relationship with the Bearer (re-read `qualities` and `type` against the `state` you write, and advance `qualities_as_of`) and its own `location` and `condition` (Decision 095). The validator fails a relationship whose record moved past its standing date.
