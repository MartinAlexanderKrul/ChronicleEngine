...
- **The Assay's local archive — every case file from the stretch of shelving Alexander could reach in the channel, taken whole into the dimensional inventory** (`EVT-000983`), on the same "stored whole, no partial retention" basis already established for Tolo's Ninth Quiet library at `EVT-000783`. Filed as **Special** (Section 12.9 kind 3 — provenance and legibility both unresolved for most of it). Includes Earth's own reopened case file, pulled with the rest. Most of the material is in scripts Alexander's stolen fluency does not cover; exact scope and value are unknown until someone can actually read it. No licensed-market or System-shop path exists for any of it — this is evidence and leverage, not loot with a sale price.
