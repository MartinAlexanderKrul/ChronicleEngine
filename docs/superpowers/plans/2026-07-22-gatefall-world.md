...
- Reikon's profile (`worlds/reikon/206_WORLD_RULE_PROFILE.md`) is the **shape reference** for profile structure (numbered sections, normative templates). Reuse its engine-proven patterns (Rank Health, Rank-baselined damage, compact notifications, canonical render template). Do not copy its flavor, magnitudes, or vell economy.

---

## Canonical Anchors (normative numbers — copy into files verbatim where a task says so)

Ranks everywhere: **E-Rank, D-Rank, C-Rank, B-Rank, A-Rank, S-Rank**.
...
- Kill XP by threat Rank: E-Rank 10 / D-Rank 25 / C-Rank 60 / B-Rank 150 / A-Rank 400 / S-Rank 1000. Elite ×2, boss ×4.
...
- Capability unlocks (authored in Profile Task 3): each stat has named unlocks at **30 and 50** (e.g., Perception 30 = read true Rank of Gates at or below own Rank; Perception 50 = read up to one Rank above; Agility 30 = act on ambush warning; Strength 30 = grapple one Rank up; Vitality 30 = shrug minor wounds; Intelligence 30 = +1 concurrent System quest slot).
...
**A4. Health.**
- Protagonist and NPC hunters use derived `rank_health` for their effective Rank; monsters likewise (per Rank).
...
- Rank frequency: E-Rank 50% / D-Rank 25% / C-Rank 15% / B-Rank 7% / A-Rank 2.5% / S-Rank 0.5%.
- Timer to dungeon break by Rank (from detection): E-Rank 7 / D-Rank 6 / C-Rank 5 / B-Rank 4 / A-Rank 3 / S-Rank 2 days.
- Legal minimum party by Rank: E-Rank–D-Rank none / C-Rank 4 licensed / B-Rank 8 / A-Rank 16 + one A-Rank / S-Rank national-asset operation.
- Assessed Rank confidence: **confirmed** (instrumented survey) or **unconfirmed** (remote reading). Unconfirmed true-Rank roll at first entry, d100: 01–15 one Rank lower; 16–70 as assessed; 71–92 one Rank higher; 93–00 anomaly — roll on the anomaly table.
- Anomaly table, d100: 01–50 population one Rank above assessment; 51–80 **red gate** (sealed until boss dies); 81–95 archetype twist doubled (A6); 96–00 dungeon-break-in-progress on arrival.
- Population formula per Gate of Rank G: **3d6 common beasts (Rank G) + 1d2 elites (Rank G, ×2 Rank Health) + 1 boss (Rank G, ×4 Rank Health, damage one Rank up)**. Emptiness only as an authored exception with in-world cause.
...
- Licensed market: E-Rank crystal **$150**, D-Rank $600, C-Rank $2,500, B-Rank $11,000, A-Rank $50,000, S-Rank auction-only. Beast core ≈ 2.5× same-Rank crystal. Black market: +40% price, felony risk.
- Freelance contract payout: E-Rank Gate $2,000–5,000, D-Rank $8,000–15,000 (split by party contract). Guild-salaried B-Rank ≈ $240k/yr. Prague rates ≈ 70% of Chicago, paid CZK/EUR.
- Cost of living: Chicago shared apartment $1,400/mo; Prague equivalent 18,000 CZK/mo.
- System shop (gold): shop buys an E-Rank crystal for **10 g** (D-Rank 40 g, C-Rank 170 g, B-Rank 750 g, A-Rank 3,400 g). Sells: lesser healing potion 25 g, standard 90 g, greater 400 g; lesser mana potion 20 g; antidote 30 g; E-Rank weapon 100 g, D-Rank 450 g, C-Rank 2,000 g; instant-dungeon key (own Rank) 500 g; elixir of a stat (+1 permanent, max 3 lifetime per stat) 5,000 g.
...
- Crystals: one per beast killed, Rank = beast Rank. Cores: elites and bosses always drop one.
- Boss drop, d100: 01–40 weapon/armor piece (Gate Rank); 41–65 rune (one random authored skill); 66–80 potion cache (3 potions keyed to Gate Rank); 81–90 instant-dungeon key (Gate Rank); 91–97 skill book (authored table); 98–00 elixir.
- Red gates and anomaly Gates roll loot one Rank above assessment. Named uniques only as authored items with provenance.
...
1. `# 1. The Gatefall` — the event ~10 years ago, first breaks, awakening; timeline of 5–8 dated world milestones (first Gate, first break disaster, founding of authorities, first S-Rank hunter, present day). Use absolute years (Gatefall event = 2016, present = 2026).
...
6. `# 6. The Czech Position` — CGA, the single Czech S-Rank hunter as national celebrity/political football, Karlov Guild, Prague's scene.
...
- Produces: section numbers Tasks 4–5 must continue from (Task 4 starts at Section 9); the terms **Bearer** (the System's holder), **stat point**, **modifier step**, and **Rank Health** used by all later tasks.
...
2. `# 2. Scope` — overrides bind the Bearer only; every other character resolves as an ordinary engine character whose capability is their fixed Rank. NPC hunters get no sheets.
...
6. `# 6. Vitality` — A4: Rank Health reuse, natural recovery tiers, treatment interaction, the death rule (death is final; campaign hard-ends).
7. `# 7. Skills` — acquisition (rune / skill book / earned-by-doing with a concrete earned-skill rule: an approach used successfully in ≥3 distinct dangerous scenes may be ratified by the System as a skill), skill entries carry name/Rank/mana cost/effect in modifier-step or damage-magnitude terms; author a **starting skill table of 8 skills** (e.g., Sprint, Dagger Mastery, Mend, Stone Skin, Flash Step, Keen Sense, Silent Step, Mana Bolt) with costs and effects — these are the skills runes can teach at E-Rank/D-Rank.
8. `# 8. The Daily Quest and Penalty Zones` — A1 daily rewards, the exact regimen (A10's objectives), the 24 h window, penalty zone: 4-hour survival instance at the Bearer's System Rank, populated per A5's formula at that Rank, no loot, exit on survival.
...
9. `# 9. Gates` — A5 verbatim as normative tables: spawn rates, Rank frequency, timers, party minimums, confidence and the true-Rank d100 table, the anomaly d100 table, the population formula, and: **"A Gate is populated by default. Emptiness is an authored exception carrying an in-world cause — and in Gatefall, an empty-looking dungeon is itself a warning sign."** Dungeon breaks: what pours out (the Gate's remaining population), response order (Chicago: BGM emergency contract → nearest guild strike team; Prague: CGA → Karlov or nearest international branch).
10. `# 10. Dungeon Archetypes` — the A6 d8 table, each twist one mechanical sentence. Rule: archetype is set at Gate instantiation and named in the assignment when the Rank is confirmed (surveyors see the interior), unknown when unconfirmed.
11. `# 11. Loot` — A8 verbatim: crystal/core rules, boss d100 table, red-gate/anomaly Rank bump, named-uniques rule. Item Ranks E-Rank–S-Rank with the rule that gear Rank sets its damage/protection exactly as monster Rank sets Rank Health.
...
17. `# 17. Instant Dungeons` — keys (Rank = Gate Rank rolled from), opening rules (2-hour instance, populated per A5 formula at key Rank, loot per A8, no timer, no BGM jurisdiction — legally unregulated space and the profile says so).
...
- [ ] **Step 2: Author each ledger** with: identity paragraph; **Interior factions** (≥2 named factions with a disagreement — spec character-depth law applied at institution level); **What they control**; **What they want**; **Current tensions** (from spec seeds: Ironline recruiting against Horizon; the three-way bid for Karlov; CGA protecting its S-Rank hunter); REC + ENT Object Blocks; REL blocks in the regulator ledgers.
...
- [ ] **Step 1: Resources ledger** — 1 REC + 4 ENT (Resource): mana crystals (Ranked family), beast cores, Ranked hunter gear, System-shop consumables. Each: description, market behavior (licensed vs black market), and the A7 price table row copied verbatim. Allocate + log IDs.

- [ ] **Step 2: Seed events ledger** — 1 REC + 5 EVT with full Object Blocks (provenance source `ruling`, absolute event_times in 2026): (1) Chicago — a mis-ranked Gate incident brewing (an unconfirmed D-Rank reading on the Red Line that is worse); (2) Chicago — the district auction guild war (Horizon vs Ironline); (3) Prague — the Old Town Gate that keeps being ranked upward; (4) Prague — the three-way foreign bid for Karlov, CGA favoring whoever keeps the S-Rank home; (5) Global — the rising break-rate statistic, publicly unexplained (consistent with the bible's owner-only section — check it before writing). Each event: what is established as having happened by world-start, referencing actor IDs.
