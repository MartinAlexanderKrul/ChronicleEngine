# Gatefall — World Rule Profile 1.38
...
**World:** Gatefall
**Profile Version:** 1.38
**Engine Compatibility:** 0.2.0; Data Model 0.1.5
**Status:** Active
**Compatibility Status:** frozen at version 1.38 (Rules Section 14.6, Decision 074), declared on repository date 2026-07-30. Version 1.38 is an **additive** advance over frozen 1.37: it authors the B-Rank rung of Section 7.3's scope-axis and capability-axis category ladders for all seven Section 7.3 starting skills that use them (Keen Sense, Silent Step, Exploit Pattern, Field Command, Resonance Extraction, Sprint, Flash Step) — closing the same authoring gap that stood in Flash Step's way (`EVT-000220`) before it recurs for the other six as each next reaches Master at its current Rank. It also explicitly flags A-Rank and S-Rank for these seven skills as an **open question** rather than silently unauthored: this world's canon gives every named S-Rank hunter one unique signature technique rather than a templated ladder continuation (`worlds/gatefall/220_NOTABLE_FIGURES.md`), and whether a Section 7.3 starting skill scales that far by the same mechanical pattern is left to an owner ruling under Section 20.3 rather than assumed. **No stored field, magnitude, cost, counter, probability, owned item, completed transaction, or resolved outcome changes.** Data Model 0.1.5 remains unchanged.
