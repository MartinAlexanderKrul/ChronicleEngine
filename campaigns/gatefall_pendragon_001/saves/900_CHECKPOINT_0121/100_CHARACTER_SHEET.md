...
provenance:
  source: EVT-000712
  game_date: "2026-08-25T10:40:00-05:00"
...
provenance:
  source: EVT-000698
  game_date: "2026-08-24T10:16:00-05:00"
...
canonical_state:
  ...
  condition: |
      Health **4,016/4,016**, Mana **2,016/2,016** — **LEVEL 48, SYSTEM RANK A, Combat Tier S, carded S-Rank, 10:16 on 2026-08-24 (Monday)**, at the Rogers Park apartment, unshowered, BGM still ahead of him before noon. **25 ability points unspent. Gold 1,530,081.**

      **Combat Tier re-derived at the 2026-08-24 readiness gate (§6.7, Profile 1.92 step 2):** `offense_tier` S (5,823 clears the S common's 4,000), `defense_tier` S (`4,016 / 237.65 = 16.9 ≥ 5`, the two automatic vambrace procs now counting), `max(A, min(S,S)) = S`. **The render was stale, not the tier** (`F-048`).

      **Four levels in one morning and not one earned against anything that could hurt him.** Two A-Rank instant dungeons off his own balcony (`EVT-000694`, `EVT-000696`), both Overgrown Temple; combined 20 commons, 5 elites, 2 bosses, 18,200 XP, **LEVEL 44 → 48** at the A-Rank rung's +63 per level. **Nothing rolled against him in either.** `gatefall.skill_credit` classifies **none** for both — use counters moved, no qualifying scene did, no mastery advanced. That is the honest result of choosing two keys because they were beneath him, and the structural shape of it is `F-046`.

      **Maximum Health 3,008 → 4,016** (4 × effective Vitality 1,004); **maximum Mana 1,512 → 2,016** (2 × effective Intelligence 1,008). Both closed full on the level-48 restoration; nothing spent since. **No Stat allocation moved** — all 25 points banked against **level 50**, which lands the fifth title slot, the next Title Assessment and **System Rank S** together.
  ...
  system_state:
    ...
    riftwalker:
      ...
      anchors:
        cap: 48
        cap_formula: "**effective Agility 965 / 20, floor, minimum 1 (Section 18.8) = cap 48**, twelve set. Re-derived at `EVT-000709` against a stored note still reading Agility 713 — Section 18.8 reads the cap live and the note had not followed the level 44-48 crossings. **This value has gone stale on five separate occasions** (`EVT-000560`, `EVT-000568`, `EVT-000633`, `EVT-000667`, and here), which is the `F-037`/`F-048` shape: a Stat-derived figure rendered as a standing line with no rule naming when it recomputes. Prior re-derivation history: `170_CHANGELOG.md`."
        set:
          - name: "The fence line"
    ...
    temporal_state:
      campaign_time: "2026-08-25T10:40:00-05:00"
      mana_recovery_mode: resting
      mana_recovery_remainder_units: 0
      health_recovery_mode: resting
      health_recovery_remainder_units: 0
      note: "**Settled through `EVT-000698`, anchor 2026-08-23 23:30 -> 2026-08-24 10:16.** Asleep 23:36 -> 06:00 at full pools; awake and active from 06:00. Mana was spent inside both instant dungeons and **restored to cap by the level-up award (Section 3.2 step 4) at each of the four crossings**, the last of them on the second boss kill; nothing has been spent since, so both pools close full at **4,016/4,016** and **2,016/2,016** with zero carry. No damage was taken at any point — nothing in either instance acted against him. Full history: `170_CHANGELOG.md`."
    quiet_days: 5
    ...
    unspent_points: 25                     # `ability_points_earned - sum(allocated)` = 371 - 346. Four level-ups (45-48) paid +5 each at `EVT-000694` and `EVT-000696`; none allocated
    pending_rewards:
      ability_points: 12
    ...
    stats:
      ...
      intelligence: 1001
    effective_stats:
      strength: "973 (base 959 + equipment 14)"
      agility: "965 (base 921 + equipment 44)"
      vitality: "1004 (base 1003 + equipment 1)"
      perception: "927 (base 922 + equipment 5)"
      intelligence: "1008 (base 1001 + equipment 7)"
      note: "**2026-08-24, LEVEL 44 → 48 across two self-opened A-Rank instant dungeons** (`EVT-000694`, `EVT-000696`). The Section 3.2 System Rank A rung pays **+63 to every base Stat** at each of the four crossings, **+252 to every base Stat** in total; equipment lines unchanged. **Maximum Health 3,008 -> 4,016** (4 x effective Vitality 1,004); **maximum Mana 1,512 -> 2,016** (2 x effective Intelligence 1,008). Both pools filled on the level-48 restoration and nothing has been spent since. No decade level crossed — **level 50 is the next Title Assessment and the System Rank S crossing**, two levels out. Derivation check against Decision 079: `base = creation_array + automatic(48) + allocated`, with `automatic(48) = 378 + 8 x 63 = 882`, so Strength `11 + 882 + 66 = 959`, Agility `12 + 882 + 27 = 921`, Vitality `8 + 882 + 113 = 1003`, Perception `11 + 882 + 29 = 922`, Intelligence `8 + 882 + 111 = 1001`. **The XP came from kills and clear milestones, not from danger** — nothing rolled against him in either instance.
 ...
 **Prior crossings compressed** — every level-up and Stat movement is held in full by `160_CAMPAIGN_CHRONICLE.md` and `170_CHANGELOG.md`. **Load-bearing values live in `stats`, `effective_stats`, `tracked_counters` and `pending_rewards`, not in this note.**"
    ...
    equipment:
      ...
      feet: "**Arcanist Feet [A-Rank]** — Good · Intelligence +7 · reduction 17% · catalogue, 24,000 g (`EVT-000609`) · EQUIPPED, displacing the [C-Rank] pair (Intelligence +3, 8%), banked unsold."
      ...
      accessory_2: "**Warded Vambrace [A-Rank] (second)** — Good · **no Stat line** · **once per exchange, a landed hit against Alexander takes −30% damage (multiplicative with worn reduction)** · bought back from Walt Adamik for $3,000 on 2026-08-20 and **EQUIPPED at `EVT-000565`**, displacing the Ascendant Hunter's Band [D-Rank], now banked. **Two vambraces run as two independent once-per-exchange procs** — duplicate accessory type, both apply per Section 11.5, and each composes multiplicatively over the other and over worn reduction. **Both fired as a pair for the first time at `EVT-000567`.** Bought because the defence axis is what pins Combat Tier; the cost is 5 Strength and about 9 damage a strike."
      total_physical_reduction: "**≈51.5% combined** (Coif 15%, Watcher Torso 12%, gauntlets 8%, Titan Legs 15%, Arcanist Feet [A] 17%, multiplicative) as of `EVT-000609`'s feet swap — up from ≈46.2%. The bought Adaptive Gale Torso [A] (20%) remains banked, unequipped. Vambrace −30% is a separate once-per-exchange effect, not part of this figure."
    skills_known:
      - "Stone Skin [C-Rank] ★☆☆☆☆ Novice · Mana 3 · 45% physical damage reduction while sustained; ends when Mana lapses or dropped. **Impact spread (C):** answers an environmental or hazard source, not only a creature's blow. **Successful uses 11 · qualifying scenes total 8 · mastery progress 0/3 toward Practiced · rank ascensions 1.** Rune-ascended [D]→[C] at `EVT-000475`."
      - "Rupture [S-Rank] ★★★★☆ Expert · Mana 11 · **reach 5 m** · a heavy strike at **×2.90 of its skill-rank baseline** (S-Rank baseline 1000, Section 7.2), resolving as `(1000 + effInt + focus power) × 2.90 × result` under §6.2 — **DMG 5,823 standard before reduction** at effective Intelligence 1,008 (`(1000+1008)×2.90=5,823`; re-derived at `EVT-000696`. Earlier: `170_CHANGELOG.md`.) **`magnitude_floor` 290 binds** — S-Rank Adept authors ×2.30, and Profile 1.83 clause 1 ratcheted the floor ×2.60 → **×2.90** on the two levels this skill climbed under a swallowing floor (`EVT-000601`). Categories: **Compound** (D); **Structural** (C); **Ward-breaking** (B); **Propagating** (A) — carries to one further target in contact with or behind the first; **`Unmaking`** (S) — **the strike ignores worn physical reduction entirely.** **Successful uses 131 · qualifying scenes total 42 · mastery progress 0/3 toward Master · mastery level EXPERT (4) · rank ascensions 5 · magnitude_floor 260.** +3 uses / +0 scenes at `EVT-000694`-`EVT-000696` (no danger; `F-046`). **At its §7.5 ceiling (System Rank A + 1); finished until System Rank S.**"
      - "Flash Step [A-Rank] ★★★★★ Master · Mana 7 · **reach: any point within 100 m, sight or no sight** — the metre band this line promised from the day it was written and withheld until A-Rank. Grants +1 modifier step to the follow-up action, and the window covers the **whole scene** — §7.2 saturates duration at one Rank above native and this skill has stood three Ranks above native since C-Rank, which this line had never rendered (corrected at `EVT-000601`, clause 2 — a rendering correction, not a grant). **`Arrival` 4/5** (§7.4) — Master's rung remains. Category **`Return` (A):** once, inside the follow-up window, step back to the point left from without paying again. **Breakthrough B-Rank Master -> A-Rank Adept at `EVT-000571`**, taken on a Master rung four minutes old, Mana 6 -> 7 the earned road's price. **At its System Rank B+1 ceiling; no further offer.** Retained **Through the barrier** (C): crosses a closed constructed barrier rather than only open distance, given a held read of the far side; never a Gate boundary, §9.6 seal or §17 wall. **Successful uses 77 · qualifying scenes total 30 · mastery progress complete Master · mastery level MASTER (5) · rank ascensions 3.** Adept → Expert at `EVT-000589`. **At its §7.5 ceiling.** "
      ...
      - "Broken Rhythm [C-Rank] ★★☆☆☆ Practiced · Passive · reading a live opponent's weight-shift and tell-cadence to react before the movement completes; +1 modifier step on evasion/reaction against a perceived attacker's telegraphed action. **Scope is mastery's axis — 1→5 beneficiaries, plus one per Rank above native; at C-Rank Novice, 3.** **`Tempo` 2/5** (§7.4) — reaction economy, which nothing else in this profile governs. `scope_floor` 3. Categories: **`Cold read`** (D) — **the step applies from the FIRST exchange against an attacker whose cadence he has not observed**, rather than requiring a read first; **`Shared cadence`** (C) — beneficiaries need not perceive him, the read carrying to anyone already on a Field Command relay. **Successful uses 98 · qualifying scenes total 12 · mastery progress 1/3 toward Adept · mastery level PRACTICED (2) · rank ascensions 1 · scope_floor 0.** Rune-ascended [E]→[C] at `EVT-000588`, two Ranks. **`F-014`'s complaint is retired** — it once went Novice to Master for nothing and now has categories and a dividend."
      - "Dagger Mastery [A-Rank] ★★★★☆ Expert · Passive · adds **+1.25** to a Quickknife chassis multiplier (×0.75 → **×2.00**), on both equipped Quickknives — `+0.25 per Rank above native, +0.05 per mastery level` (§7.2, §7.4). Categories: **Any blade** (D); **Thrown** (C) — covers a blade launched by Dimensional Projection; **`Improvised`** (B) — **any edged or pointed object he wields counts as a blade**, a rung owed since the skill reached B holding only D and C; **`Follow-through`** (A) — **a blade strike that KILLS carries its excess damage into one further target within reach**, answering a week of overkill waste. **Successful uses 337 · scenes 37 · mastery progress 2/3 toward Master · mastery level EXPERT (4) · ascensions 4.** +1 use / +0 scenes at `EVT-000694`. History in `160_CAMPAIGN_CHRONICLE.md`."
      - "Twin Fang [B-Rank] ★★★★☆ Expert · Mana 5 · while two Quickknives are equipped, follow one ordinary Quickknife strike with a separately rolled strike from the other in the same action. Each strike uses only its own weapon power; the enabled second strike has a **×2.50** Twin Fang mastery multiplier at B-Rank Expert — `1.00 + 0.35 × 3 + 0.15 × 3` — since the crossing at `EVT-000629`. **Stale, owed a §6.2 re-derivation:** 2,985 and package 4,191 were computed at the pre-crossing ×2.35; previews resting on them stay withheld. **The authored rung gained outright and no floor was needed.** Category **`Both fangs`** (B): **the enabled second strike may be directed at a DIFFERENT TARGET than the first**, which retires the credit problem — it no longer has to hit a body that survived the first. Occupies the action's offensive slot; no combining with Rupture. **A successful use is recorded only when the second strike lands and materially contributes**; a target dying to the first strike spends the Mana and credits nothing (`EVT-000512`). **Successful uses 136 · qualifying scenes total 29 · mastery progress 0/3 toward Master · mastery level EXPERT (4) · rank ascensions 3 · magnitude_floor ×1.60.** +1 use / +0 scenes at `EVT-000694`. Expert → Master at `EVT-000589`; breakthrough [C] Master → [B] Adept at `EVT-000590`."
      ...
      - "Silent Step [D-Rank] ★★★★★ Master · Mana 4 · +1 modifier step on stealth and unseen-approach actions for the scene. **Scope is mastery's axis — 1→5 beneficiaries, scene-long at every level** (§7.4), plus one per Rank above native; at Expert, **5**. **`Composure` 5/5** (§7.4) — maxed on adoption by five repaid levels. `scope_floor` 5. Category **`Concealment through action` (D):** concealment survives **one** attack or manipulation — strike, cut, lift, open or take once without the effect ending; it ends on the second. It needs terrain that can hide someone; a cast with nowhere to break to is refused before it costs anything. **Successful uses 48 · qualifying scenes total 25 · mastery progress complete Master · mastery level MASTER (5) · rank ascensions 1.** +2 uses / +1 scene this span. Use history: `160_CAMPAIGN_CHRONICLE.md`."
      - "Mana Bolt [B-Rank] ★★★★★ Master · Mana 6 · **no distance limit; a target DETECTED but not seen is a legal target** · a ranged bolt resolving as `(150 + effInt + focus power) × result_multiplier` under §6.2 — **DMG 2,200 standard before reduction** at effective Intelligence 1,008 (`(150+1008)×1.90=2,200`; re-derived at `EVT-000696`. Earlier: `170_CHANGELOG.md`.) **The stored ×1.75/×1.90 discrepancy on this line predates this checkpoint and is unrepaired here — flagged, not resolved** (see `170_CHANGELOG.md`). **`magnitude_floor` 175 binds.** Categories: **Arcing** (D) — the bolt bends around intervening cover; **Unsighted** (B) — a resolved Keen Sense or held read substitutes for eyesight. Never crosses a Gate boundary, a §9.6 seal, or a §17 instance wall. **Successful uses 88 · qualifying scenes total 21 · mastery progress complete Master · mastery level MASTER (5) · rank ascensions 2 · magnitude_floor 160.** +1/+0 `EVT-000696`. **Expert → MASTER at `EVT-000650`**, third qualifying scene: six A-Rank commons in a Flooded Mine ambush, all one-shot on critical or strong results, none landed in return."
      - "Field Command [C-Rank] ★☆☆☆☆ Novice · Passive · relay one active Exploit Pattern by a brief actionable call to allies who can perceive and understand Alexander. Each recipient treats the Pattern as personally understood, needs no separate tactical read, and gains its +1 modifier step on genuinely exploiting actions; Alexander may update the call before a recipient's roll, never afterward. Ordinary communication conveys information but not the Pattern's mechanical benefit. **Scope is mastery's axis — 1→5 simultaneous recipients** (Section 7.4), plus one per Rank above native; at C-Rank Novice, **3** — exactly what Adept already gave, so **the ascension bought the categories and nothing else**. **`Conduction` 2/5** (§7.4) — the profile's one sharing skill, **inert while he works alone**. `scope_floor` 3. Categories: **`Relay at range` (D)** — recipients need not perceive him; the call carries to anyone he has spoken with in the scene, by radio, phone or prearranged signal. **`Composite relay` (C)** — relay **two** Patterns at once. **Successful uses 26 · scenes 10 · progress 0/3 · mastery level NOVICE (1) · ascensions 1.** **Rune-ascended [E] Adept → [C] Novice at `EVT-000585`**, two Ranks. Ceiling A. "
      - "Resonance Extraction [C-Rank] ★★★☆☆ Adept · Passive · a successful read establishes a Resonance Pattern for one vein, +1 modifier step on extraction until exhausted/shifted. **Five further miners may work his calls** — C-Rank Adept computes 4 on §7.4's authored *0 → 4 additional miners* ladder (Adept 2, plus 2 Ranks above native) and **`scope_floor` 5 binds**, so the rung bought the category rather than the count. **`Yield` 4/5** (§7.4) — Master's rung remains. Category **`Buried seam`** (C): **a Pattern can be established on a deposit that is NOT EXPOSED** — behind rock, under water, or inside a structure — where it previously needed a visible face. **Successful uses 48 · scenes 22 · progress 2/3 toward Expert · mastery level ADEPT (3) · ascensions 2.**  Expert → Master then **breakthrough [D] → [C] Adept, both at `EVT-000588`**."
      - "Flux Sight [S-Rank] · Stat Passive · reads Rank/Health/Mana of monsters and hunters; confirms Gates one Rank above System Rank; new at B, appraises an unidentified item's Rank/stats on sight (Section 4.4). **S-Rank grant:** reads what an unidentified thing is FOR, not merely its Rank and stats; and with THE UNMEASURED equipped it returns a subject's CEILING alongside its current state (`EVT-000591`). **Uses 104 · Perception 927 · S-Rank** — +7 this span (a wristguard appraised free for Walt, `ENT-000226` read, and the five taken CONSULT pieces). Use history: `160_CAMPAIGN_CHRONICLE.md`. It stands at S-Rank, which *is* the Section 4.4 ceiling (System Rank A + 1); the ceiling rose with System Rank A at `EVT-000589` and both S conditions, Perception 80 and System Rank A, are met. Flux Sight is authored to S-Rank so the §4.4 authoring clamp never binds it; the **System Rank + 1 ceiling does**."
      - "Overpower [S-Rank] · Stat Passive · sustained restraint; break ranked structures; **redirection** — a foe he holds becomes a weapon, thrown or driven into another target with the strike transferring to it, and a creature can be shifted off its own footing mid-exchange, denying it a chosen line (Section 4.4). **S-Rank grant:** restraint and breaking lose their Rank cap entirely, and a ranked structure he breaks fails STRUCTURALLY rather than locally. **Uses 5 · Strength 973 · S-Rank.** Rose B → A at `EVT-000515`: restrain up to three Ranks above, and **break ranked inanimate structures one Rank above the skill**. No Stat moved to earn it."
      - "Pre-empt [S-Rank] · Stat Passive · shared warning — allies act normally in a surprise exchange alongside him; **first blade** — in any exchange he opens he resolves before any opponent up to B-Rank acts, whatever order the fiction implies (Section 4.4). **S-Rank grant:** *first blade* resolves him before ANY opponent at any Rank. **Uses 27 · Agility 965 · S-Rank.** Rose B → A at `EVT-000515`."
      - "Conduit [S-Rank] · Stat Passive · Mana recovery **+25 percentage points** at both Section 5.2 rates — **35%/hr active, 50%/hr resting** against the 10/25 baseline (A-Rank rung, `EVT-000515`). **S-Rank grant:** recovery is CONTINUOUS, accruing during a scene rather than settling from elapsed time — the A-Rank line said in terms that it cannot reach inside a fight. **Uses 0 · Intelligence 1,008 · S-Rank.** Rose B → A at `EVT-000515`; held rung rose A→S at `EVT-000487`. Composition order is fixed (Section 5.2): base, Conduit, environmental halving, then any title multiplier; a Frozen Gallery halves 30/45 rather than 10/25. It cannot reach inside a fight — recovery settles from elapsed time, an exchange is not an hour, and Section 5.3's exhaustion is untouched."
      - "Bulwark [A-Rank] ★☆☆☆☆ Novice · Mana 10 · **75% physical damage reduction for two exchanges**; reductions multiply, never add, so no stack reaches immunity. Categories: **Anticipated** (D) — raised in reaction to a blow already declared, not only before the exchange opens; **Covering the line** (A) — the reduction extends to **every** ally within reach, superseding C's single-ally `Covering`. B's cap conversion is the second exchange. **Rune-ascended [E] Adept → [A] Novice at `EVT-000701`** (500,000 g, four Ranks in one consumption); mastery reset (§7.5). **Successful uses 6 · qualifying scenes total 6 · mastery progress 0/3 toward Practiced · mastery level NOVICE (1) · rank ascensions 1.**"
      - "Shrug Off [S-Rank] · Stat Passive · ignores minor-wound penalty; **threshold absorption** — once/scene, a hit crossing the injury threshold inflicts damage only; **total suppression** — **every** wound's modifier-step penalty is suppressed until scene end rather than one, and environmental hazards up to B-Rank cause no onset at all rather than halved onset (Section 4.4). **S-Rank grant:** threshold immunity covers creatures more than one Rank below, i.e. up to A-Rank, and environmental hazards up to A-Rank cause no onset. **Uses 5 · Vitality 1,004 · S-Rank.** Rose B → A at `EVT-000515`."
      - "Mend [A-Rank] ★☆☆☆☆ Novice · Mana 6 · restores Health at ×1.00 baseline (375 before focus). **Rune-ascended [E] Expert → [A] Novice at `EVT-000610`**, four Ranks in one consumption; mastery reset (§7.5). **Successful uses 1 · scenes 0 · mastery progress 0/3 · mastery level NOVICE (1) · rank ascensions 1.**"
      - "Dimensional Projection [C-Rank] ★★★★☆ Expert · Mana 4 · **5 projectiles · range 21 m** · withdraw owned portable objects into unoccupied points in range and launch them at perceived targets as a **named multi-strike** (§6.2) — **one ranged physical strike per projectile**, **Agility to hit**, ordinary Bearer physical-damage formula on *that object's* power and chassis. Projectiles **split freely**; a failed placement does not spend the others. Never inside a creature, object, barrier or occupied space. Holds the action's offensive slot — no combining with Rupture or Twin Fang. Mana paid **once for the action**. Mastery ladder **1/2/2/3/3** projectiles, **5/7/10/15/20 m**, Mana **6/5/5/4/4**, **+1 and +3 m per Rank above native**. **Successful uses 27 · qualifying scenes total 16 · mastery progress 1/3 toward Master · mastery level EXPERT (4) · rank ascensions 1 · magnitude_floor 0.** **ADEPT → EXPERT at `EVT-000651`**; rune-ascended [E] Adept → [C] Novice at `EVT-000570`, two Ranks. Advance history: `170_CHANGELOG.md`. Categories: **`Recall and launch` (D)** — launch and recovery no longer contest the exchange's one inventory change, **deleting `EVT-000438`'s limit**; **`Held read` (C)** — a point perceived but not currently seen qualifies where a read is held, never through a sealed barrier, Gate boundary, §9.6 seal or §17 wall. "
      - "Sprint [B-Rank] ★★★☆☆ Adept · Mana 4 · +1 modifier step on movement, pursuit, and distance-closing actions, **for the whole scene** — §7.2 saturates duration at one Rank above native. **`Control` 1/5** (§7.4) — its duration axis saturated at C-Rank, so clause 2 sends every level here. Categories: **Broken ground** (D) — the step holds across footing that would otherwise demand its own check; **Vertical** (C) — vertical and overhanging surfaces count as ground while the effect holds; **Terrain denied** (B) — hostile or hazardous footing authored by an archetype twist or environment imposes **no movement penalty and forces no check at all**. **Successful uses 28 · qualifying scenes total 20 · mastery progress 0/3 toward Expert · mastery level ADEPT (3) · rank ascensions 3.** +1 use this span (the cab tail). Rune-ascended [C]→[B] at `EVT-000571`."
      - "Seamwork [C-Rank] ★☆☆☆☆ Novice · Mana 3 · a resolved working of one object that **already carries a mana line** — a ranked item, an infused piece, a core, a crystal already seated. **Native C-Rank** (§7.3), ratified at `EVT-000604`; evidence and the §18.3.1 entry-Rank reasoning are held by `progression_candidates.seamwork` and the chronicle. Also grants **+1 modifier step on any action assisting an artificer’s work** (§12.10’s assist step reads this skill). **THE BOUNDARY: it never creates an item.** Matter carrying no mana line is inert to it and the Mana is refused before it is spent; it never raises a piece past its **authored ceiling**, always spends material, and is gated by its own Rank. He is **not** a licensed artificer — no standing, no commissions — and a §12.10 fusion still resolves on the artificer performing it. Rungs: **Read and open** (C, native) — presence, Rank, the maker’s method and habits including mid-process corrections, and a finished working opens and closes intact; **Seat** (B) — material worked in, adding to what the piece carries; **Lift** (A) — a working comes out whole, the donor half of a fusion; **Raise** (S) — a piece carried to its authored ceiling, and two workings read against each other before anyone commits. Mastery axis **`Hand`, the cost of being wrong**: at Novice a failed working spends its material and closes the piece to him for the scene; at Master a failure spends nothing and returns why. **Successful uses 1 · qualifying scenes total 1 · mastery progress 1/3 toward Practiced · mastery level NOVICE (1) · rank ascensions 0.** Counters initialise at zero per §7.1; pre-ratification evidence establishes acquisition only (`F-040`). Use history: `160_CAMPAIGN_CHRONICLE.md`."
      - "Rift-step [Riftwalker] · Class ability · Mana 6 · **once per exchange** · a true teleport to any point in **line of sight**, and at System Rank B also **through an obstruction he has perceived past**. **Grants +1 modifier step to the immediate follow-up action.** Reach is sight, not distance — no metre figure until A-Rank. **A step to a set anchor does not consume this once-per-exchange step** and is priced separately (`system_state.riftwalker`). **Owner ruling (`EVT-000551`): a passenger may be carried below S-Rank while physical contact is maintained through the step, at 10× ordinary Mana cost** until System Rank S normalizes it. **Successful uses 18** — +5 this span (`EVT-000700`, `EVT-000708` x2, `EVT-000709`, `EVT-000710`, `EVT-000711`), all transport rather than tactics. **Per-use history is held by `160_CAMPAIGN_CHRONICLE.md`**, including the passenger step carrying Hollis Rennard clear at ten times ordinary cost."
    ...
    tracked_counters:
      ...
      - { path: skills.seamwork.rank_ascensions, baseline_value: 0, baseline_as_of: EVT-000604, current_value: 0 }
      - { path: progression.ability_points_earned, baseline_value: 323, baseline_as_of: EVT-000597, current_value: 371 }
      - { path: stats.strength_allocated, baseline_value: 52, baseline_as_of: EVT-000597, current_value: 66 }
      ...
      - { path: stats.intelligence_allocated, baseline_value: 102, baseline_as_of: EVT-000597, current_value: 111 }
      ...
      - { path: skills.rupture.successful_uses, baseline_value: 16, baseline_as_of: EVT-000130, current_value: 131 }
      - { path: skills.rupture.qualifying_scenes_total, baseline_value: 8, baseline_as_of: EVT-000130, current_value: 42 }
      - { path: skills.rupture.mastery_progress, baseline_value: 2, baseline_as_of: EVT-000130, current_value: 0 }
      - { path: skills.flash_step.successful_uses, baseline_value: 6, baseline_as_of: EVT-000130, current_value: 77 }
      ...
      - { path: skills.broken_rhythm.mastery_level, baseline_value: 1, baseline_as_of: EVT-000308, current_value: 2 }
      ...
      - { path: skills.dagger_mastery.successful_uses, baseline_value: 2, baseline_as_of: EVT-000130, current_value: 337 }
      - { path: skills.dagger_mastery.qualifying_scenes_total, baseline_value: 1, baseline_as_of: EVT-000130, current_value: 37 }
      - { path: skills.dagger_mastery.mastery_progress, baseline_value: 1, baseline_as_of: EVT-000130, current_value: 2 }
      - { path: skills.twin_fang.successful_uses, baseline_value: 0, baseline_as_of: EVT-000130, current_value: 136 }
      - { path: skills.twin_fang.qualifying_scenes_total, baseline_value: 0, baseline_as_of: EVT-000130, current_value: 29 }
      ...
      - { path: skills.silent_step.mastery_level, baseline_value: 1, baseline_as_of: EVT-000235, current_value: 5 }
      ...
      - { path: skills.bulwark.rank_ascensions, baseline_value: 0, baseline_as_of: EVT-000158, current_value: 1 }
      - { path: skills.mend.rank_ascensions, baseline_value: 0, baseline_as_of: EVT-000158, current_value: 1 }
      ...
      - { path: skills.rift_step.successful_uses, baseline_value: 0, baseline_as_of: EVT-000519, current_value: 18 }
    ...
    gold: 1030081  # 1,530,081 - 500,000 g Bulwark [A-Rank] rune, Ivory Daily Premium cycle, EVT-000701
    inventory:
      ...
      consumables:
        ...
        - "**Greater Healing Potion x13** — was 10; **+3 from the Daily Random Box at `EVT-000688`** (d100 9 -> potion cache, d4 1 -> three healing; the A-Rank cache tiers to *greater*). Restores Health to full."
      ...
      materials:
        - "**Mana Crystal [A-Rank] x 100** — **unchanged, and wholly turned over this span.** +264 in (162 mined +11 dropped at `EVT-000694`; 82 mined +9 dropped at `EVT-000696`, both deposits +2 Prospector), -264 out (173 sold `EVT-000695`, 91 sold `EVT-000697`, both at 3,400 g). Prior: the Flooded Mine take. Licensed value $50,000 each; that desk stays declined."
        - "**Beast Core [A-Rank] x 8** — was 1; **+7 this span** under Section 11.1: 3 at `EVT-000694`, 4 at `EVT-000696`. **The shop refuses cores** (Section 12.5). Section 12.10 prices a fusion at one core at the result's Rank — **the unfused Warded Vambrace [A-Rank] pair is now affordable several times over.**"
    ...
    daily_premium:
      cycle: |
          ...
          **LIVE: Ivory, rotated 2026-08-24 06:00** (`EVT-000689`) at System Rank A. Series d8 2. Rank rolls d100 91/37/66/50/49 -> weapon +2 clamped **S**, armor **A**, accessory +1 **S**, rune **A**, key **A**. Bags: d4 2 -> Horizon Reacharm, d8 1 -> head/Gale; accessory and key bags each held one entry and gave **Mindspun Loop** and **Runic Key** unrolled, emptying both. Consumable bag refilled, d5 4 -> **Ironblood Phial** (Clarity Phial excluded then returned). Rune skill d10 10 -> **Bulwark**. **1/6 bought** - the Runic Key, 250,000 g (`EVT-000690`). Expires 2026-08-25 06:00.

          **The Bulwark rune was an ascension, not an acquisition, and it was BOUGHT** (`EVT-000701`, 500,000 g): [E] -> [A] under Section 7.5 with a mastery reset. Bulwark now stands at **[A-Rank] Novice** on `skills_known`.

          **RETIRED: Prismatic, rotated 2026-08-23 23:36** (`EVT-000688`), seventeen hours late under the rule above, **expired unbought in full at the next 06:00**. Rank roll d100 99 gave an **Aegis Guard Shield [S-Rank]** - Vitality +24, 28% reduction, 225,000 g, the first S-Rank equipment offer this campaign generated. Slept through. **0/6.**

          **Prior:** Cobalt (2026-08-22) closed 1/6, the Adaptive Gale Torso [A-Rank] (`EVT-000609`). Full history: `160_CAMPAIGN_CHRONICLE.md`.
      model_bags:
        settled_through: "2026-08-24 rotation (Ivory)"
        weapon:
          previous_draw: "Horizon Reacharm"
          remaining: ["Farline Longshot", "Spellthread Focus"]
        armor:
          previous_draw: "head/Gale"
          remaining: ["head/Watcher", "torso/Arcanist", "hands/Gale", "hands/Watcher", "legs/Gale", "feet/Gale", "feet/Arcanist"]
        accessory:
          previous_draw: "Mindspun Loop"
          remaining: []
          refill_note: "**Bag empty.** The next rotation refills the full five-model accessory set and excludes Mindspun Loop on that first draw, returning it to the bag afterward (Section 12.5's draw-without-replacement rule)."
        consumable:
          previous_draw: "Ironblood Phial"
          remaining: ["Restoration Draught", "Sovereign Panacea", "Mender's Seal", "Quicksilver Phial", "Clarity Phial"]
          refill_note: "Refilled at the 2026-08-24 rotation after the Clarity Phial emptied it; Clarity Phial was excluded from that first draw and has been returned to the bag."
        key:
          previous_draw: "Runic Key"
          remaining: []
          refill_note: "**Bag empty.** The next rotation refills the full six-model key set and excludes Runic Key on that first draw."
      purchased: "**2/6 on the live Ivory cycle** - the Runic Key [A-Rank], 250,000 g, bought `EVT-000690` and **spent the same morning** opening the first instant dungeon (`EVT-000694`); and the Bulwark [A-Rank] rune, 500,000 g, bought and consumed on the spot `EVT-000699` — ascension E→A, four Ranks in one consumption. The other four rows are unbought and expire 2026-08-25 06:00."
      offers:
        weapon: "Horizon Reacharm [S-Rank] - Premium model - both hands - 60/40 Perception/Strength split of the S-Rank 22-point weapon budget = Perception 14, Strength 8, **+1 further Perception and +1 further Strength (premium) = Perception +15, Strength +9** - weapon power 22 - armed strike x0.85 - its hold-distance step also applies to protecting one adjacent ally - 225,000 g - unpurchased - a two-handed chassis he does not fight with."
        armor: "Adaptive Gale Head [A-Rank] - Premium model - head - Agility +9 (7 budget + 2 adaptive) - physical reduction 17%+3pp = **20%** - 30,000 g - unpurchased - competes with the equipped Adaptive Titan Coif [B-Rank] (Strength +7, 15%)."
        accessory: "Ascendant Mindspun Loop [S-Rank] - Premium model - accessory - Intelligence +15 (12 budget + 3 ascendant) - 180,000 g - unpurchased - **runs into the standing Warded Vambrace question**: both accessory slots hold stat-less A-Rank vambraces, and this is the same trade the Ascendant Warlord's Loop has been waiting behind since `EVT-000570`."
        consumable: "Ironblood Phial - 30% physical damage reduction for one scene - 750 g - unpurchased"
        rune: "Premium Rune - teaches **Bulwark** - [A-Rank] - UNBOUND - NPC-ELIGIBLE - 500,000 g - **BOUGHT AND CONSUMED `EVT-000701`**: the [E] -> [A] ascension under Section 7.5, mastery reset. Bulwark now reads [A-Rank] Novice."
        key: "Runic Key [A-Rank] - instant-dungeon key, the boss drops one guaranteed rune result in addition to its normal boss-drop roll - 250,000 g - **BOUGHT `EVT-000690`, SPENT `EVT-000694`** - its guarantee returned a Keen Sense rune [A-Rank], `unbound-awakened`, banked intact."
      previous_cycle: "**2026-08-23, Prismatic series - 0/6 purchased**; rotated seventeen hours late at 23:36 under the standing rule and expired unbought in full at the 2026-08-24 06:00 boundary, **the Aegis Guard Shield [S-Rank] with it**. Prior: **2026-08-22, Cobalt series - 1/6 purchased** (Adaptive Gale Torso [A-Rank], `EVT-000609`). Prior: 2026-08-21, Verdant series - 0/6, expired in full."
    non_daily_quests:
      capacity_total: 6
      active:
