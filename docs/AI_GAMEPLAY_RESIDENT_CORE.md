...
7. Run only the proactive triggers whose profile-declared boundary is this exchange. If this exchange closes a dangerous progression scene, perform its one batched audit and attach the required `progression_audits` result—including `none`. If it closes a potentially qualifying work/practice scene, seal only the bounded evidence notes and stable scene key for the next promotion barrier. Otherwise perform no progression-candidate work.
...
Checkpointing normally promotes already-settled in-flight state; it does not perform immediate costs, harm, recovery, rewards, or known-skill counter settlement for the first time. A profile may explicitly defer **non-combat candidate classification** to the promotion barrier, where sealed work/practice notes are classified in one batch; this narrow exception does not defer any existing skill's use or mastery counters. If the player has to ask whether mana recovered, XP was earned, or **a known skill's counters advanced**, the prior exchange failed settlement and must be corrected before play continues.
