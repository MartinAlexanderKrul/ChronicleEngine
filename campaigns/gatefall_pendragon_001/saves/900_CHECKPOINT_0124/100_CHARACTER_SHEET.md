 ...
 **Prior crossings compressed** — every level-up and Stat movement through checkpoint 0123 is held in full by `160_CAMPAIGN_CHRONICLE.md` and `170_CHANGELOG.md`. **Load-bearing values live in `stats`, `effective_stats`, `tracked_counters` and `pending_rewards`, not in this note.**"
    ...
    skills_known:
      ...
      - "Dimensional Projection [S-Rank] ★☆☆☆☆ Novice · Mana 6 · withdraw owned portable objects and launch at perceived targets as a named multi-strike (§6.2), one strike per projectile, Agility to hit. **5 projectiles** (1 mastery + 4 Rank), **range 17 m** (5 + 3×4). **`magnitude_floor` was never written on the `EVT-000729` ascension** — a real ratchet gap, flagged not guessed. `Recall and launch` (D)/`Held read` (C) held; **`Volley`** (S). Re-derived `EVT-000736`. **Successful uses 29 · qualifying scenes total 18 · rank ascensions 2 · mastery progress 2/3.** +2 uses / +2 scenes: two volleys (`EVT-000741`, `EVT-000742`)."
