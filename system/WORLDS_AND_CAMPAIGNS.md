...
| `campaigns/reikon_awakening_001/` | `worlds/reikon/` | Daedalus | In progress | `900_CHECKPOINT_0016` | 2026-07-22 |
...
- **`campaigns/reikon_awakening_001/`** — Resumable from `900_CHECKPOINT_0016`, a clean stopping point just outside the closed tannery E-Rift. Warehouse 7's MAIN objective is fully and permanently resolved (the bonded core was extracted safely via resonance work, no milestone XP to Daedalus since the clearing act was the specialist's); a second solo E-Rift closed unguarded, earning Daedalus the full Rift-clear milestone XP under World Rule Profile 0.7; two flagged gaps are on record rather than silently resolved; 2 Ascension allocation points remain unspent pending the player's decision. This campaign keeps a **save index** at `saves/README.md` recording each checkpoint's status and restorability; read it as part of restoration and honor it (Destructive-Command Guards). `900_CHECKPOINT_001` is quarantined and must not be restored. The campaign has **no baseline checkpoint**, so `/restart` is unavailable and must not be attempted.
