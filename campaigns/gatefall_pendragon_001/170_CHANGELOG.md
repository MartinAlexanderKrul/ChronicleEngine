...
**2026-07-29 — Gatefall World Rule Profile 1.32 adopted (`EVT-000187`; no fictional time consumed).**

- **What was missing.** Fixed-Rank NPC hunters and monsters had no numeric Mana pool to spend or exhaust. Their supernatural capacity was subsumed in Rank without a number behind it, so attrition could not constrain their strongest actions and no future capability could report their remaining capacity.
- **What now exists.** Every fixed-Rank entity carries Rank Mana from Section 6.1: **20 / 50 / 125 / 300 / 750 / 2,000** at E through S. Skill-like actions cost a fraction of that entity's own maximum by role — **minor 10%, signature 25%, boss-tier 50%**, floored to an integer, ambiguity taking the lower tier — while ordinary attacks and movement remain free. Sections 5.2 and 5.3 apply unchanged, including −1 modifier step at 0 Mana until recovery above 25%.
- **Individual variance.** Section 6.1.2 permits up to ±10% on both pools: authored for recurring named NPCs, rolled once for elites and bosses, and absent for common monsters. Section 6.2 damage always reads the flat table value, so pools vary while combat arithmetic remains reproducible.
...
**2026-07-29 — Gatefall World Rule Profile 1.35 adopted (`EVT-000190`; no fictional time consumed).**

- **What was wrong.** Section 7.2 spends every Rank above native on three things for a modifier-step skill: whole-scene duration at the first step, one further affected subject, and one authored category. On a skill that is instantaneous and carries no scope count, the first two are inert and the third existed for no skill outside Section 7.3's scope table — so **Sprint and Flash Step could ascend for nothing**, surrendering mastery for a Rank label with no capability behind it. Section 7.5 asserted the opposite three times over: its net-gain sentence, its `Rank N` ordering, and its rule making every Master-level skill below the ceiling ascension-ready. **This was live**: Flash Step stands at D-Rank Expert, one mastery level below the Master that triggers the offer, and Rules Section 13.2 forbids reopening a settlement that has already spent mastery — so the repair had to arrive before that settlement, not after.
- **What changed in the profile.** A skill is **ascension-eligible** at a target Rank only where that Rank's grant is authored and non-empty for it; where it is not, Section 7.5 withholds the offer and states the withholding rather than making it. An offer surfaced for an ineligible skill is a misstated offer that restores the mastery it consumed. Section 7.3 gains a capability-axis ladder beside its scope ladder: **Sprint** broken ground at D and vertical at C; **Flash Step** through the barrier at C, bounded to constructed barriers, requiring a read of the far side, and barred from Gate boundaries, Section 9.6 seals and Section 17 instance walls. Flash Step's B rung is left unauthored under Section 20.3, the ceiling putting it out of reach at System Rank D.
- **What changed in live state.** `profile_version` 1.34→1.35 and ledger provenance advanced to `EVT-000190`. **Nothing else.** No skill Rank, mastery level, counter, Stat, pool, item, or resolved outcome moved. Sprint stands at native E-Rank and Flash Step at native D-Rank, so no capability rung is live and none is retroactive. No ascension offer was issued, withdrawn, or re-adjudicated, and none was outstanding.
- **Repository allocation (2026-07-29).** Allocated `EVT-000190` in `system/ID_REGISTRY.md`. Advanced `system/WORLDS_AND_CAMPAIGNS.md`, `090_CAMPAIGN_STARTUP.md`, `worlds/gatefall/README.md` and the three `test_gatefall_*` contracts to Profile 1.35. The checkpoint migration-chain text additionally gained the **1.33→1.34** link, which the 1.34 adoption had left out of `180_CURRENT_STATE.md` line 34.
- **Validators.** `tools/validate_repository.ps1` now rejects a skill standing at a Rank its Section 7.3 ladder does not author; `tools/test_progression_audit_contract.ps1` proves that rejection and the restored pass. All six Gatefall validators run clean against the resulting state.

**2026-07-29 — Gatefall World Rule Profile 1.36 adopted (`EVT-000191`; no fictional time consumed).**

- **What was wrong.** Profile 1.35 stopped an *ascension* spending mastery for a Rank whose grant was never authored, by withholding the offer. A Stat Passive makes no offer — its Rank derives from a base Stat and rises the instant a threshold is crossed — so Section 7.2's eligibility rule had nothing to withhold. A governing Stat reaching **54** with that skill's B-Rank rung unauthored would have handed it a Rank whose effect does not exist.
- **What changed in the profile.** Section 4.4 now clamps a Stat Passive's derived Rank by two limits: the Section 7.5 ceiling of System Rank + 1, and the deepest rung authored for that skill. Stat value beyond either is held intact and applies when the binding limit lifts. Flux Sight is authored to S and never held; Overpower, Pre-empt, Shrug Off and Multitask are authored through C. A held rung is stated at the crossing checkpoint and rendered `<Next Rank> held: rung unauthored` in `/system`.
- **What changed in live state.** `profile_version` 1.35 → 1.36. **Nothing else.** No Stat Passive is held today: System Rank D already caps all five at C-Rank and C is authored for every one. Flux Sight and Multitask stay D-Rank; Overpower, Pre-empt and Shrug Off stay E-Rank. No `/system` row changes.
- **Repository allocation (2026-07-29).** Allocated `EVT-000191` in `system/ID_REGISTRY.md`. Advanced the profile version in `system/WORLDS_AND_CAMPAIGNS.md`, `worlds/gatefall/README.md`, `090_CAMPAIGN_STARTUP.md`, `100_CHARACTER_SHEET.md`, `180_CURRENT_STATE.md`, and the three `tools/test_gatefall_*_contract.ps1` pins.
- **Enforcement.** `tools/validate_repository.ps1` applies the clamp inside the Stat Passive derivation it already ran against the ceiling, so the rule fails validation rather than living only in prose.
- **Validators.** All fourteen repository validators run clean against the resulting state.
