...
**Condition:** Full 110/110 Health; 2/14 Mana (Fireball spent, not yet recovered); Level 3, 25/170 XP; 8 unspent allocation points. Authoritative values in `100_CHARACTER_SHEET.md`.
...
- **Ascension is settled, twice over.** Level 3, 25/170 XP, Mana Affinity 14, Fireball Strength 1, maximum Mana 15, maximum Health 110. **8 allocation points are unspent** (raised from 2, retroactively, by `EVT-000045`) — deliberately left for Daedalus's own allocation decision, not spent by either ruling that granted them.
...
7. **8 allocation points are unspent.** Points come only from levelling (Section 4.1) and allocation is irreversible (Section 4.5); the choice belongs to Daedalus's player and has not been made.
8. **Power, Speed, and Perception have no authored uncapped growth formula.** Stats have no upper bound (Section 4.2, `EVT-000045`), but only Endurance (`max_health`) and Mana Affinity (a fixed +1 die ceiling, never more) have a defined payoff. Points banked into the other three past their old die-relevant range currently do nothing mechanical — a real gap, flagged rather than filled with an invented formula, pending a future ruling.
...
Resume just outside the collapsed Ashgate E-Rift. Health is full (110/110); Mana is low (2/14, unspent since the Fireball); Emberbrand is spent. All nine Ashgate Broodlings are confirmed dead, harvested for nine E-cores (core purse 55); the extracted Ashgate core is carried separately, unappraised. XP stands at 25/170 at Level 3 — the Rift-clear milestone gap is resolved (×10 grade baseline, World Rule Profile 0.7) and was applied retroactively to Ashgate, firing this Ascension. Stats now have no upper bound and Ascensions grant 5 points instead of 2 (World Rule Profile 0.8, `EVT-000045`), applied retroactively to both of Daedalus's completed Ascensions. **8 allocation points are unspent and await Daedalus's player's decision** on Stats versus Abilities before the next uncertain action that would benefit from them — bearing in mind Mana Affinity still only ever buys up to +1 on the die, and Power/Speed/Perception have no uncapped payoff authored yet.
