# Gatefall — World Rule Profile 1.73
...
4. **Breaks.** Section 9.10's board runs its timers on the tick. A posting under floor past its window breaks, and the break is **sited by the same d100 above** rather than assumed to be elsewhere.
5. **Postings regenerate.** The day's Gates produce board postings; the board is not permitted to decay to zero and stay there.
6. **Agendas advance.** Every named entity holding an agenda (`011_ENGINE_DATA_MODEL.md` Section 7.9) advances on its own clock, and the tick rolls whether any of them **initiates** — a call, an arrival, an ask, a piece of news. A world in which contacts only ever answer is a world with one moving part.
