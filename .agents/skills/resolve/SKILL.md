---
name: resolve
description: Use when resolving any uncertain action, attack, or dice roll in Chronicle Engine play; when entering a Gate (true Rank, archetype, population); when applying damage or tracking Health; or when the player questions a combat number. Also when the player types /resolve or /roll.
---
...
2. **Gate entry is rolls, not narration.** On first entry of an unconfirmed Gate, *roll* the true Rank (§9.5 d100), *roll* the archetype (§10 d8), then derive the population from the §9.7 formula by *rolling* its dice (3d6, 1d2). The sensor's read is flavor laid over the rolled result — never a substitute for rolling it. A Rank or population you announced without a roll is invented.
3. **Damage is the §6.2 formula, shown, not a vibe.** `standard-hit baseline (¼ attacker Rank Health) × skill multiplier × degree-of-success multiplier × (1 − reductions)`. Show the arithmetic in the OOC block. An E-Rank Bearer's improvised dagger on a standard success is `10 × 0.5 × 1 = 5`, not "8." NPC and monster damage ride **their own Rank** (§6.2) — a D-Rank beast hits at 25, no "one Rank up."
4. **Health is a tracked ledger, every exchange — tracked in conversation, not written to a file.** After each hit, state the running total in the OOC block: `Alexander: 30 → 17/40`. A hit that landed is subtracted the same turn. Never restate an old total after damage — the graze you narrated is damage you must apply. This running total is held as pending state; it reaches `100_CHARACTER_SHEET.md`/`180_CURRENT_STATE.md` only when `/save` runs (see the `save` skill).
...
| "The sensor already told them the population" | The sensor's line is flavor. Until you rolled Rank → archetype → population, there is no population; you narrated one into being. |
...
- A Rank, archetype, or monster count you can state but did not roll
