...
| `campaigns/gatefall_pendragon_001/` | `worlds/gatefall/` | Alexander Pendragon | In progress | `900_CHECKPOINT_0002` | 2026-07-24 |
...
- **`campaigns/gatefall_pendragon_001/`** — Gatefall's first campaign. Session 1 is in progress; **latest restorable checkpoint `900_CHECKPOINT_0002`** (captured 2026-07-24 after the Red Line Corridor Gate clear, in the post-boss loot window). Protagonist Alexander Pendragon cleared his first Gate — true grade E, one below the unconfirmed D assessment — with no crew casualties, and the System attached to him mid-fight per the onset ruling (World Rule Profile Section 14.7): he is now a Bearer, Level 2, XP 40/200, unknown to his crew. Loot has not yet been collected and the crew has not yet exited the Gate. The checkpoint was captured under **Gatefall World Rule Profile 1.0, a workshop draft (unfrozen)** — restoring it surfaces the *unfrozen — not save-trustworthy* warning (Rules Section 14.6, Decision 074). The campaign has **no baseline checkpoint**, so `/restart` is unavailable; `/continue` restores `900_CHECKPOINT_0002`.
