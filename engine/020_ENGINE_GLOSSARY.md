...
Distinct from a [Runtime Command](#runtime-command), which is out-of-character control of the session and is engine-general. When a slash token could be either, the runtime command table is matched first; runtime command names are reserved and a world must not redefine them. A world that declares a diegetic command should also declare its **canonical render template** — a fixed layout bound to canonical state — which every Runtime renders verbatim so the display is identical across sessions and substrates (Reikon's `/system` template is in `worlds/reikon/206_WORLD_RULE_PROFILE.md`). Defined in the Runtime Command Interface (`docs/AI_GAMEPLAY_RUNTIME_PROFILE.md`) and Decision 056.
