# Gatefall — World Rule Profile 1.117
...
A skill enters the Bearer's ledger by one of six routes, and only these:
...
**What this route carries that no other does: the technique can be taught onward.** A skill acquired by any of routes 1 through 4 is an **inscription** — the System wrote a capability into him and he was never shown how. A skill acquired by route 5 is a **method he watched someone perform**, and he can perform it for someone else. Live state records this on the skill:

```yaml
acquisition_route: rune | skill-book | earned | stat-passive | taught | succession
transmissible: true            # route 5 only; every other route records false
...
**Together those make the taught route the only road to a technique that does not yet exist.** Route 6 hands him twenty finished things at somebody else's mastery and forecloses the twenty-first; route 5 hands him fewer, slower, at Novice, and is the only one of the six that can produce something nobody has ever held. That is the trade, stated, and it is now a trade rather than a strictly dominated option.
...
**A method he was never shown is an inscription however it arrived.** Route 5 grants `transmissible` because a teacher performed the method in front of him; a body performed nothing. He cannot teach onward what he took off a corpse, and Section 7.1's rule that `transmissible` is never granted retroactively holds here without exception.
