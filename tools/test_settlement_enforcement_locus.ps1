...
try {
    ...
    foreach ($name in $controlRed) {
        # Two causes produce this and the message has to name both, because the
        # hint used to name only the first and sent a reader looking for a
        # missing fixture file when the real cause was a suite red on live canon.
        $problems.Add("Control round: '$name' is red in the UNMUTATED fixture, so nothing can be attributed to a mutation. Either the fixture is missing something that suite reads -- add it to the copy above -- or that suite is simply red right now, in which case run it directly and fix that first. This suite reports rather than guesses which.") | Out-Null
    }
