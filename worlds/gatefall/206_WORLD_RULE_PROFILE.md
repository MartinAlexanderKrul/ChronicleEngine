# Gatefall — World Rule Profile 1.59
...
### Authored Formation Result — First Principles

Repeated, materially successful reconstruction of unobserved physical structure from observed geometry and physical law may form the stable candidate `applied_analytical_reconstruction`, signature `apply-formal-physical-or-geometric-training.reconstruct-unobserved-structure-or-coverage.act-on-the-derived-finding`. Its complete ratification result is authored here, at 1.58, **before the candidate reaches its threshold** — the same ruling 1.55 made for Dimensional Projection, and for the same reason: authoring the result is what makes the skill safe to form.

| Skill | Rank | Mana cost | Effect |
|---|---|---:|---|
| **First Principles** | E-Rank | Passive | +1 modifier step on any action that reconstructs unobserved structure, coverage, or reach from observed geometry and physical law — sightlines and sensor cones, load paths and structural extent, trajectory, span, and reachability. The finding is a derivation from measurable evidence, and it is only ever as good as the measurements it stands on. |

**It derives; it does not perceive, and that boundary is what keeps it distinct.** First Principles never reveals what is *present* — that is Keen Sense's work and costs Mana — and it establishes **no Pattern**, so it grants no standing exploit and does not occupy an Exploit Pattern slot. It answers *what must be true of this space, given what I can measure*, and its output is a conclusion about structure rather than a read of a subject. Its modifier does not stack with another modifier derived from the same measurements; unrelated advantages still combine to the ordinary net cap. A derivation resting on wrong or insufficient measurements is wrong, and the die resolves that as it resolves anything else — the skill improves the reasoning, never the evidence.

**Its mastery track is scope, and that is authored rather than derived.** First Principles is a permanent passive already at the Section 4.3 step cap: no multiplier, no reduction fraction, no duration to lengthen, and Mana 0, so Section 7.4's −10%-per-level rule is inert too. Every closed-form mastery axis is inapplicable at once — **exactly the state Broken Rhythm reached and paid twelve qualifying scenes to discover**. The remedy is applied here at authoring time instead of after the fact. Mastery grants **1 / 2 / 3 / 4 / 5** beneficiaries from Novice through Master: at Novice it benefits the Bearer alone, and each level extends the step to one further person he has walked through the derivation with, who then acts on the finding as though he had reached it himself. It accordingly carries a `scope_floor` under Section 7.2's ratchet like every other scope-axis skill.

A successful application is each resolved action the step materially improved, his own or a benefiting ally's. Several may occur in one scene; one qualifying scene contributes at most one mastery point.
...
| **First Principles** | Structure, coverage, or reach reconstructed from his own direct observation | **Inference from absence** — the derivation runs on what is *missing*: an unexplained gap in coverage, a load path that must exist somewhere, a route the evidence forbids, where the native effect needs positive measurements to work from | **Mechanism** — extends from where a built system's parts sit to how it *behaves*: how a mechanism, circuit, lock, or routine will act under a given input | **From secondhand measure** — the reconstruction holds on measurements he did not personally take: another's description, a photograph, a drawing, a relayed reading, where every rung below requires his own direct observation | **Predictive** *(A)* — he derives the structure's future state: where it fails, when it gives, and what it looks like after — a building's collapse line, a system's next configuration — before that state exists |
...
| **First Principles** | Scope — beneficiaries | 1 → 5 (**authored at 1.58, before the skill exists**; a permanent passive at the step cap has no other axis — the Broken Rhythm state, caught at authoring time rather than after twelve scenes. Unformed candidate; applies at ratification) |
