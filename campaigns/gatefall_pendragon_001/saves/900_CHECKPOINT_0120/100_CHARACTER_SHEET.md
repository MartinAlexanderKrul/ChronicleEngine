...
canonical_state:
  ...
  condition: |
      Health **4,016/4,016**, Mana **2,016/2,016** — **LEVEL 48, SYSTEM RANK A, Combat Tier A, carded S-Rank, 10:16 on 2026-08-24 (Monday)**, at the Rogers Park apartment, unshowered, BGM still ahead of him before noon. **25 ability points unspent. Gold 1,530,081.**
  ...
  system_state:
    ...
    class_quest:
      ...
      **Superseded history compressed at `EVT-000653`** — the missed Section 18.1 trigger and its retroactive fire (`EVT-000475`/`EVT-000478`), and the 2026-08-07 player-elected non-promotion of a played trial on `F-022`/`F-023`, are held in full by `160_CAMPAIGN_CHRONICLE.md` and `170_CHANGELOG.md`. Nothing from that span is canon; the class is Riftwalker, taken at `EVT-000519`."
    riftwalker:
      ...
      anchors:
        cap: 32
        cap_formula: "effective Agility 713 / 20, floor, minimum 1 (Section 18.8) — **cap 35**, nine set. Re-derived at `EVT-000667` (650 -> 713). Prior: Re-derived at `EVT-000653`'s level-43 crossing (effective Agility 587 -> 650); the thirty-third anchor lands at Agility 660. Prior: effective Agility 587 / 20 gave cap 29. **Re-derived at `EVT-000633`** — the stored cap (23, and a `cap:` field reading 11 beneath it, itself internally disagreeing) had not moved since `EVT-000568`'s effective Agility 323, through both System Rank A rungs (+63 each at `EVT-000616` and `EVT-000626`) that carried it to 587; Section 18.8 reads the cap live and neither figure had followed. Prior: **Re-derived at `EVT-000568`** — the level 30–34 crossings carried effective Agility 231 → 323 and Section 18.8 reads it live, so the working set went 11 → 16. Prior: effective Agility 231 / 20; the twelfth anchor lands at Agility 240. **Re-derived at `EVT-000560`** — the 1.80 backfill carried effective Agility 112 → 231, and Section 18.8's cap reads it live, so the working set went from full at five to four of eleven. The migration's step 3 names Stat Passives and damage previews explicitly; this is the same class of Stat-derived value and re-derives on the same terms."
        set:
          - name: "The fence line"
    ...
    mana: "2016/2016"
    world_clock:
      last_ticked: "2026-08-24T06:00:00-05:00"
      next_tick: "2026-08-25T06:00:00-05:00"
    ...
    temporal_state:
      campaign_time: "2026-08-24T10:16:00-05:00"
    ...
    pending_rewards:
      ability_points: 8
      ability_points_claim_note: "**+8 pending, unclaimed** -- +4 from the twenty-eighth daily (`EVT-000680`) and +4 from the twenty-ninth (`EVT-000693`), both at the Ascetic rate. Per `skill_rulings.ascetic_claim_timing_evt_000357` the rate is read at **claim**, not at issue, so both entries pay +4 while Ascetic stays equipped."
      ...
      status_recovery: 22                  # +1 pending from the twenty-ninth daily (EVT-000693); none claimed
      daily_random_box: 0                  # both banked boxes opened this span (EVT-000688, EVT-000693); none pending
      unresolved_gate_loot: "**Fully resolved.** Nothing outstanding. Full history: `160_CAMPAIGN_CHRONICLE.md`."
      latest_settlement: "**2026-08-24 (`EVT-000693`): twenty-ninth daily completed**, streak 16 -> 17. Box opened at once (d100 55 -> System gold, **150,000 g**); AP +4 and Status Recovery not claimed. **`EVT-000688`: the twenty-eighth's banked box opened** — d100 9 -> potion cache, d4 1 -> **3 Greater Healing Potions**. Prior settlements: `160_CAMPAIGN_CHRONICLE.md`."
 ...
 **Prior crossings compressed** — every level-up and Stat movement is held in full by `160_CAMPAIGN_CHRONICLE.md` and `170_CHANGELOG.md`. **Load-bearing values live in `stats`, `effective_stats`, `tracked_counters` and `pending_rewards`, not in this note.**"
    ...
    tracked_counters:
      ...
      - { path: skills.keen_sense.successful_uses, baseline_value: 1, baseline_as_of: EVT-000130, current_value: 106 }
      - { path: skills.keen_sense.qualifying_scenes_total, baseline_value: 1, baseline_as_of: EVT-000130, current_value: 69 }
      ...
      - { path: skills.exploit_pattern.successful_uses, baseline_value: 0, baseline_as_of: EVT-000132, current_value: 87 }
      - { path: skills.exploit_pattern.qualifying_scenes_total, baseline_value: 0, baseline_as_of: EVT-000132, current_value: 48 }
      ...
      - { path: skills.flux_sight.successful_uses, baseline_value: 0, baseline_as_of: EVT-000130, current_value: 97 }
      ...
      - { path: skills.sprint.successful_uses, baseline_value: 0, baseline_as_of: EVT-000149, current_value: 27 }
      ...
      - { path: skills.silent_step.successful_uses, baseline_value: 0, baseline_as_of: EVT-000235, current_value: 46 }
      - { path: skills.silent_step.qualifying_scenes_total, baseline_value: 0, baseline_as_of: EVT-000235, current_value: 24 }
      ...
      - { path: skills.rift_step.successful_uses, baseline_value: 0, baseline_as_of: EVT-000519, current_value: 13 }
    ...
    inventory:
      ...
      consumables:
        ...
        - "**Greater Mana Potion x8** — was 5; **+3 from the Daily Random Box at `EVT-000665`** (d100 23 -> potion cache, greater tier at System Rank A, d4 4 -> three mana). Restores Mana to full · banked."
      ...
      gear:
        ...
        - "**Mana-null disc — LEFT AT THE CICERO SCAR** (`EVT-000666`), pressed to the warp-heart slab 04:22, not retrieved. Blinds passive mana-signature scanning through that surface only. Walt Adamik, $1,800."
    ...
    daily_premium:
      cycle: |
          ...
          **The Bulwark rune is an ascension, not an acquisition.** He holds Bulwark [E-Rank] Adept, so at 500,000 g it is [E] -> [A] under Section 7.5 with a mastery reset, or `unbound-awakened` instruction for an eligible awakened who lacks it. A Runtime error first described it as a new skill and was corrected in-session.
      ...
      purchased: "**1/6 on the live Ivory cycle** - the Runic Key [A-Rank], 250,000 g, bought `EVT-000690` and **spent the same morning** opening the first instant dungeon (`EVT-000694`). The other five rows are unbought and expire 2026-08-25 06:00."
      offers:
        ...
        rune: "Premium Rune - teaches **Bulwark** - [A-Rank] - UNBOUND - NPC-ELIGIBLE - 500,000 g - unpurchased - **not a new skill**: he holds Bulwark at [E-Rank] Adept, so this is an [E] -> [A] ascension under Section 7.5 with a mastery reset, or `unbound-awakened` instruction for an eligible awakened who lacks it."
    ...
    non_daily_quests:
      ...
      active:
        - quest_key: "warehouse-meridian-ownership"
          ...
          objective_counters: "**Clue condition met** (`EVT-000188`) and **shell-company ownership layer established** (`EVT-000189`); Cormant Holdings connected with fair confidence to the lanyard logo (`EVT-000287`). Handed to Rosalind Fenn (`EVT-000374`); her inquiry traced back on a natural-2 critical failure (`EVT-000490`) and she now works untraceable sources only. **`EVT-000541`: a live Gate signature read inside the fence and an undisclosed anomaly-sensing layer found in the site security. `EVT-000612`: a second windowless structure behind the main shell, and a maintenance schedule rather than constant presence.** **`EVT-000646`-`EVT-000652`: THE PURPOSE HALF IS ESTABLISHED.** Entered in person; Flux Sight converted the site's own unconfirmed C-Rank reading to **confirmed A-Rank by capability** (§9.5) and returned the apparatus's purpose — **containment, not extraction**, the boss held by an old load-bearing binding that is slowly degrading. Corroborated first-hand by the site's containment monitor **Hollis Rennard** (`ENT-000212`), whose unlogged eleven-week plant duty-cycle climb is the operation's own evidence the containment is failing. **OWNER/OPERATOR STILL NOT IDENTIFIED FIRST-HAND** — she names only Meridian Cold Storage LLC, has never met a person from the company, and files to an inbox. `completion_condition` needs both; still attached, still unrevealed. Full progress history in `160_CAMPAIGN_CHRONICLE.md`."
        ...
        - quest_key: "marnie-disappearance"
          ...
          objective_counters: "**Proximity and clue conditions both met** (`EVT-000325`) — the coffee stall off Cermak, a missed standing Tuesday check-in, and the loading dock Marnie herself wanted watched. **Corroborated physically** (`EVT-000326`, `EVT-000328`): her own staging space, work abandoned mid-job, and a finished dagger auto-identifying at **S-Rank** under the same maker's stamp as the wallet lining. **Location lead** (`EVT-000333`): Loomis Street, no exact address; the CONSULT inquiry panicked Walt's contact and closed that channel (`EVT-000352`); the street located and read twice (`EVT-000353`, `EVT-000372`), a manned camera blind spot found by direct math/physics analysis. **Handed to Rosalind Fenn** (`EVT-000374`), dock withheld. **`EVT-000490` (natural 91 → 71, success): a storage unit under a name not hers, cash, prepaid through October, three weeks before the silence — the first fact about *her* rather than her absence.** **Reveal condition NOT met**: what actually happened to her, and whether she is safe, remain unestablished. Reward unchanged at D-Rank/150 XP; the S-Rank dagger is loot from `EVT-000326`, not a reward of this quest. Full progress history in `160_CAMPAIGN_CHRONICLE.md`."
      ...
      note: "**Capacity derives from System Rank alone** from `EVT-000365` (Profile 1.49, Section 8.4): System Rank A gives **6**. Initialized at `EVT-000115`. **Three Hidden pointers attached** — 2026-08-06 (`EVT-000188`), 2026-08-13 (`EVT-000325`, the Marnie thread), and 2026-08-20 (`EVT-000555`, the Cicero Scar sublevel), see `active` above; all three render `[HIDDEN] ???` until their reveal_conditions are met. **One Urgent quest accepted and completed inside two minutes this span** (`uncontained`, `EVT-000648`-`EVT-000649`) — it held slot 4/6 while active and released it on completion. Slots **2/6 used** - `the-sublevel-under-the-cicero-scar` revealed and completed at `EVT-000667` and released its slot (Section 8.4.4), leaving the Hidden pointers on the warehouse and on Marnie. The cap has never bound, which is why it stopped being a skill."
    daily_quest:
      streak: 17
      last_completed: "2026-08-24"
      last_failed: "2026-08-07"
      active: false
      note: "**Twenty-ninth daily completed mid-morning** (`EVT-000693`), streak 16 -> 17 — the 10 km run taken as the lakefront loop with Owen Callahan in a little under fifty minutes, calisthenics finished on the living-room floor afterwards. No roll (routine). **17 is not a multiple of seven**, so the box it created rolled once rather than twice; the next streak upgrade lands at 21. Three rewards created pending: Ability Points +4 (Ascetic equipped), one Status Recovery, one Daily Random Box — **the box was opened immediately and the other two were not**. Full streak history back to the fourteenth's deliberate failure: `160_CAMPAIGN_CHRONICLE.md` and prior checkpoint captures."
      current_quest: "No quest active; the twenty-ninth completed (see `note`). Prior daily detail: `160_CAMPAIGN_CHRONICLE.md`."
      next_issue: "2026-08-25T06:00:00"
