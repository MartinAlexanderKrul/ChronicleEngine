# Gatefall — World Rule Profile 1.37
...
**World:** Gatefall
**Profile Version:** 1.37
**Engine Compatibility:** 0.2.0; Data Model 0.1.5
**Status:** Active
**Compatibility Status:** frozen at version 1.37 (Rules Section 14.6, Decision 074), declared on repository date 2026-07-30. Version 1.37 is an **additive** advance over frozen 1.36: it declares Section 14.3's operational trigger-dispatch manifest (`trigger_manifest_version`, the `gatefall.quest.urgent`/`gatefall.quest.hidden`/`gatefall.skill_formation` domains, and each domain's `ratification` block) and corrects the Tier-2 timing contract from *permitted, Runtime-timed* to **mandatory at the first qualifying yield** — the design question Decision 081 ruling 2 already settled, now actually versioned rather than silently live under the frozen 1.36 text. Both are declare-only: they name candidate deltas, eligibility headings, identity fields, and settlement policy the Runtime already read from prose, and hand a validator a structured place to check ratification thresholds and pre-authored result keys instead of a hard-coded one. **No stored field, magnitude, cost, counter, probability, owned item, completed transaction, or resolved outcome changes**, and no Urgent offer or Hidden pointer is issued or reinterpreted retroactively. Data Model 0.1.5 remains unchanged.
