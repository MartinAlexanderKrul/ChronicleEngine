...
**Coverage is engine-general and prospective** (Decision 092). Every campaign is covered; there is nothing to opt into. What a campaign declares is only *where its obligation begins* — `disposition_baseline`, an entity identifier recorded once in `090_CAMPAIGN_STARTUP.md` at adoption — so that a cast written before the model existed is backlog rather than a wall of failures. Characters recorded after the baseline carry the requirement.

This is deliberately unlike participation coverage (Section 12.4.4), which a world opts into. That shape is right for an obligation whose cost falls on a world's own authoring conventions. It is wrong here, and Decision 092 records why: a world that declines the character model does not thereby get simpler NPCs, it gets NPCs played from the Runtime's context, which is the one behaviour the engine has seven design flags saying it must not produce. An opt-out from personhood is an opt-in to the defect.

**A missing baseline means fully covered, never uncovered.** A campaign that declares none is read as having a baseline of zero, so every Character it records is inside coverage. Silence must fail toward the obligation rather than away from it — the opposite convention is how a mechanism becomes unreachable while every gate stays green, which is `F-002`'s cause and the reason Section 12.4.5's malformed-baseline case exists.

**The obligation attaches to a configured campaign** — one declaring a startup configuration. A directory holding ledgers and no `090_CAMPAIGN_STARTUP.md` is a fragment or a fixture rather than something a Runtime can boot, and it has no NPCs to play badly. This is a boundary and not a loophole: deleting a startup file to escape the obligation costs the campaign its resumability entirely, so the escape is dearer than the obligation. That is not true one level in, which is exactly why a *configured* campaign declaring no baseline is read as covered rather than exempt.
...
**What writes one** (Decision 092). An agenda is authored in the same act as the disposition it belongs to: a Character that receives `Want`, `Fear`, `Secret` and `Voice` receives an agenda alongside them, derived from its `Want` and bounded by its own capability, standing and reach. The two are not separable — a want with no pursuit is a preference, and an actor holding commitments and no agenda exists exclusively in relation to the protagonist (Section 7.7).

This clause exists because Section 7.9 and Runtime Section 2.4 specified settlement **in full** while nothing created the thing to settle. That is the shape `docs/DEVELOPMENT_WORKFLOW.md` requires a decision introducing settled state to answer for, and it had produced zero live instances of Decision 082's commitments, Decision 083's supply, and Decision 088's needs before it produced zero agendas. The three questions that rule asks are answered here: **what creates it** is the authoring of a disposition; **what enforces that creation** is the same enforcement point that reads the disposition, since the two are written together; and **the absence is detectable** — a covered Character carrying a disposition and no agenda is a structural state a gate can see, which is the first construct in this family for which that is true.
