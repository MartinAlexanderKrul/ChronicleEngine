...
- **Rupture and Mana Bolt finally have authored ranges (`EVT-000397`, Profile 1.57)** — **Rupture reaches 5 m, flat at every Rank**; **Mana Bolt has no distance limit** and requires line of sight at native Rank. Neither had ever carried a written range: the player noticed the two read the *identical* `§6.2` formula after 1.56, differing only by multiplier bands exactly 1.00 apart and by Mana cost, and ninety in-fiction days had settled the difference by convention alone. **No damage number, counter, or mastery level moved** — Rupture still deals 299 for 9 Mana, Mana Bolt 98 for 5. Mana Bolt's ladder relaxes the **sense** rather than the reach, which is why *Arcing* (D, held now) and *Homing* (A) keep their value.
