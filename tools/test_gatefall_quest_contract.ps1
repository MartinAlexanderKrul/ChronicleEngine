...
Assert-True ($profile -match '(?m)^# Gatefall .+Profile 1\.20\r?$') "Gatefall Profile 1.20 is not active."
...
Assert-True ($character -match 'profile_version: "1\.20"') "Live Gatefall character was not migrated to Profile 1.20."
...
Assert-True ($startup -match 'world_rule_profile: "Gatefall World Rule Profile 1\.20"') "Campaign startup does not bind Profile 1.20."
...
Assert-True ($index -match 'World Rule Profile 1\.20, frozen') "World index does not advertise frozen Profile 1.20."
