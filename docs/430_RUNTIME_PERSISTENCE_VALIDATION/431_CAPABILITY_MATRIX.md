...
|----|------------|-------------------------|----------------|--------------------------|
| V03-01 | Canonical checkpoint form | Four-digit checkpoint directory contains all eight ledgers and exactly one conforming manifest | Passed — gate and observed | `test_checkpoint_contract.ps1`; Gatefall checkpoints 0001–0014 |
| V03-02 | Promotion barrier completeness | Every affected live target is written and read back before the immutable checkpoint is sealed | Passed — observed | Gatefall checkpoints through 0014; manifests and `180_CURRENT_STATE.md` promotion target lists |
...
| V03-09 | Structured profile metadata | New checkpoint records world, version, and freeze status, or explicit no-profile | Passed — gate and observed | Gatefall workshop-draft Checkpoint 0004 and frozen Profile 1.6 Checkpoint 0014; `test_checkpoint_contract.ps1` Contract 6 |
...
| V03-19 | Prototype findings are dispositioned | The live prototype may reveal and drive changes to Rules, Data Model, Runtime, or other architecture; every finding must be explicitly classified, governed, versioned, migrated, and revalidated, with no known architectural gap left unaddressed at postmortem | In progress — Decision 076 finding implemented and revalidated | Decision 076 records the foundational Architecture-Freeze exception; Data Model 0.1.3 and its explicit immutable-checkpoint migration close the discovered Relationship Texture gap. Final determination remains the 0.3 postmortem. |
