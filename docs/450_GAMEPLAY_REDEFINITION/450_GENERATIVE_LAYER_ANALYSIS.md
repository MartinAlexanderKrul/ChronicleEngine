...
**Status:** Analysis — no engine change proposed for adoption in this pass; two variants costed for an owner ruling
...
**What Variant A risks, stated plainly.** `F-011`, `F-012` and `F-015` are all counters narrated and never written. Moving counter settlement off the exchange moves it toward the failure those flags record. The mitigating fact — and it is worth checking rather than accepting from this document — is that **the gates enforcing settlement already fire at the promotion barrier, not at the exchange.** `test_progression_audit_contract.ps1` and `test_participation_audit_contract.ps1` assert against promoted checkpoints. If that holds, the per-turn obligation is *already* prose with no enforcement point, and it is prose costing about 1,100 tokens of the scarcest surface in the engine. Variant A would then be moving an unenforced obligation to where its enforcement actually is, rather than weakening anything.
...
**The arguable leg, stated rather than resolved.** Variant A's claim rests on *"the gates enforcing settlement already fire at the barrier, not the exchange."* This document asserts that from reading those suites' purpose and fixtures; it has not been proven by mutation. **If the owner rules the other way — or if the check shows per-exchange settlement genuinely is enforced somewhere this analysis did not find — then Variant A is trading a real guarantee for room, and Variant B with a deliberate warning move becomes correct.** The way to settle it is to run the two audit suites against a fixture that settles correctly at the barrier and incorrectly at the exchange, and see whether either goes red. That test does not exist and should be written before the ruling, not after.
...
- The three `-> ~` condensation figures in Variant A are **estimates**. Nothing in this document should be adopted on them; draft the condensed sections and re-measure with `tools/measure_runtime_context.ps1` before the ruling.
