# Gatefall — World Rule Profile 1.36
...
**World:** Gatefall
**Profile Version:** 1.36
**Engine Compatibility:** 0.2.0; Data Model 0.1.5
**Status:** Active
**Compatibility Status:** frozen at version 1.36 (Rules Section 14.6, Decision 074), declared on repository date 2026-07-29. Version 1.36 is an **additive correction advance** over frozen 1.35, closing the same hole on the one axis 1.35 could not reach. Version 1.35 stopped an *ascension* from spending mastery for an unauthored Rank by withholding the offer — but a **Stat Passive** makes no offer. Its Rank derives from a base Stat and rises on its own the moment a threshold is crossed, so Section 7.2's eligibility rule had nothing to withhold, and a Stat reaching 54 while the skill's B-Rank rung stood unauthored would have handed it a Rank whose grant does not exist: a number this profile states and nothing defines, the Section 20.2 defect. Section 4.4's derivation now clamps by **two** limits rather than one — the Section 7.5 ceiling of System Rank + 1, and the **deepest rung authored for that skill** — with Stat value beyond either **held intact** and applying the instant the binding limit lifts. Flux Sight is authored to S-Rank and is never held; Overpower, Pre-empt, Shrug Off and Multitask are authored through C-Rank until Section 20.3 reaches higher. A held rung is **recorded, never silent**: the checkpoint that first crosses such a threshold states the skill, the Rank held, and the Stat value that reached it, and the `/system` row renders `<Next Rank> held: rung unauthored` in place of a threshold. `tools/validate_repository.ps1` enforces the clamp alongside the ceiling it already checked. **No stored field, magnitude, cost, counter, probability, owned item, completed transaction, or resolved outcome changes.** Data Model 0.1.5 remains unchanged.
