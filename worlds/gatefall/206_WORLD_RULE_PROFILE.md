# Gatefall — World Rule Profile 1.104
...
**Say plainly what this is.** `200_WORLD_BIBLE.md` Section 9 authors a terminal event for this world that is **a single break that does not close**. A Gate Walker's far door is that, in miniature, by his own hand, and the profile does not pretend otherwise. **The power is unbounded and the consequence is the setting's own ending arriving early if he is careless with it.** Opening one costs **Mana equal to S-Rank Rank Health — 4,000** (Section 6.1), the only fixed price in this section, because a doorway onto a world is not sized by anything smaller.
...
**That clock is Section 5.2.1, and it runs on his Mana.** The saturation figure is what his body is *offsetting*, not a quantity anyone tracks: the pool is the organ that sheds it, so the cost of standing there is read off the pool. **Section 5.2 recovery does not accrue on the far side and the pool falls by 10% of maximum Mana per hour** — a full pool is ten hours, and the 4,000 this door costs is spent before he arrives. At zero the drain crosses into Health (Section 5.3), and Section 12.5's Greater potions degrade to their Standard form there, so **nothing in the shop buys him more time.** No new counter exists anywhere for this and none is to be invented: the expedition's whole budget is the two pools already on his sheet.
