...
- **Two pre-existing parse defects, fixed after Checkpoint 0174.** `110_WORLD_LEDGER.md`'s `ending_routes` listed its routes straight under the mapping with no key, and `120_INVENTORY_AND_OWNERSHIP.md`'s `current_funds.note` had lost its closing quote (broken since Checkpoint 0162 or earlier). The routes now sit under `routes:`, at the same indentation so the validator's regex and `test_ending_routes.ps1`'s anchors are unchanged, and the quote is restored. Both blocks parse; no value changed.
