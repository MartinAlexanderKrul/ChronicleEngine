...
| `campaigns/reikon_awakening_001/` | `worlds/reikon/` | Daedalus | In progress | `900_CHECKPOINT_0016` | 2026-07-21 |
...
- **`campaigns/reikon_awakening_001/`** — Resumable from `900_CHECKPOINT_0016`, a clean stopping point just outside the closed Ashgate E-Rift: all nine Broodlings confirmed dead, the core extracted, the Rift-clear milestone XP gap settled (World Rule Profile 0.7, ×10 grade-baseline multiplier, applied retroactively) firing an Ascension to Level 3, and Stats now uncapped with the Ascension point rate raised from 2 to 5 (World Rule Profile 0.8), applied retroactively — 8 allocation points left unspent pending the player's decision. Mana Affinity's die contribution still ceilings at +1 regardless (Law VII); Power, Speed, and Perception have no uncapped formula yet, a recorded gap rather than an invented one. The Warehouse 7 core remains unextracted, with Daedalus holding observation-only clearance there. This campaign keeps a **save index** at `saves/README.md` recording each checkpoint's status and restorability; read it as part of restoration and honor it (Destructive-Command Guards). `900_CHECKPOINT_001` is quarantined and must not be restored. The campaign has **no baseline checkpoint**, so `/restart` is unavailable and must not be attempted.
