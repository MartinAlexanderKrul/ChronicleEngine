# Gatefall — World Rule Profile 1.39
...
**World:** Gatefall
**Profile Version:** 1.39
**Engine Compatibility:** 0.2.0; Data Model 0.1.5
**Status:** Active
**Compatibility Status:** frozen at version 1.39 (Rules Section 14.6, Decision 074), declared on repository date 2026-07-30. Version 1.39 **removes the Bearer duplicate-consumption reroll** from Section 7.1's Consumption Settlement. When the Bearer consumes instruction teaching a technique he already knows at or above its `teaching_rank`, the System no longer re-draws the originating skill table. He chooses instead between leaving the item **intact** — identity, binding, and transfer value untouched, which is what makes it real instruction for an eligible awakened recipient — and **spending it for mastery** as the matching Mastery Rune or Mastery Book. The reroll predates working NPC consumption (`EVT-000178` is the first time an unbound rune actually taught a non-Bearer), which now gives a duplicate a better outlet than a re-draw; and it carried an **unbounded procedure** — "reroll until the result is unknown to him" has no terminating condition once the Bearer knows every technique on a table. Section 7.1's Bearer branch labels are also disambiguated to name which side of the comparison governs, since "known at a higher Rank" read equally well as the item's Rank or the Bearer's. **No stored field, magnitude, cost, counter, probability, owned item, completed transaction, or resolved outcome changes**; no consumption-time reroll has ever been executed in a live campaign. Data Model 0.1.5 remains unchanged.
