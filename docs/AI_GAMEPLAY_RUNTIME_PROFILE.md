...
- **Resident per-turn layer — `docs/AI_GAMEPLAY_RESIDENT_CORE.md`.** The Player Agency Contract; the compression intent envelope; Interaction Cadence; intent/fact grounding; the Action Resolution Bright Line and Modifier Step Contract; the Information Boundary; the NPC channel check; Turn-State Settlement; the Profile-Declared Proactive Trigger Audit; promotion-obligation awareness; canon-determinism; and the rule that an applicable World Rule Profile is loaded before affected resolution. These checks fire on every turn at the point a fact or outcome would be narrated.
