...
Class 3 is the finding. `439_NPC_GROUNDING_ANALYSIS.md` records why it resists mechanization rather than asserting it: a line generated from recollection and one generated from a correct read are identical in the transcript whenever the recollection happens to be right.

**The obvious escape was measured and is closed.** Decision 085 rejected the derived check at 38% false positives, but that measurement was taken for a broader check than the one that would be built here, and citing it would have been an inherited number rather than a tested one. `tools/measure_npc_grounding_proxy.ps1` reports the rate under four partitions: **37.4%** across all participations, **38.4%** restricted to fiction-bearing Events, **37.5%** restricted to NPCs holding a relationship with the protagonist, and **37.3%** with both filters. The hypothesis — that recurring, characterization-bearing NPCs would show a lower miss rate and support a targeted gate — is refuted. The rate is flat to within a percentage point, meaning presence-without-change is uniformly legitimate across the whole population, **there is no subset to retreat to**, and a gate would flag roughly three legitimate cases for every real one on precisely the characters the campaign is most about.
...
**Required refinement:** 0.4 should decide whether an NPC encounter must leave evidence that a record was fetched, and at what writer cost. What it may **not** do is reach for a derived check over existing canon: that route is now measured across four partitions and closed. Any proposal must therefore add a written trace at encounter time — a real cost on the writer — or accept that class 3 stays resident-only and say so.
