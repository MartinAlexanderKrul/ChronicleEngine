...
- **Consumables:** 1 Lesser Mana Potion consumed mid-fight; banked count 3 → 2. 3 Lesser Healing Potions unchanged.
...
- **A pre-existing ledger discrepancy, flagged not resolved:** `shop_holdings` renders **8** Lesser Healing Potions where this record says **3**. It predates this session and no Event since has touched potions, so it sat outside Checkpoint 0065's derived target set.
