...
|----|------------|-------------------------|----------------|--------------------------|
| V03-01 | Canonical checkpoint form | Four-digit checkpoint directory contains all eight ledgers and exactly one conforming manifest | Passed — gate and observed | `test_checkpoint_contract.ps1`; Gatefall checkpoints 0001–0015. Checkpoint 0015 is the first native Data Model 0.1.3 capture. |
| V03-02 | Promotion barrier completeness | Every affected live target is written and read back before the immutable checkpoint is sealed | Passed — observed, including automatic context preservation | Gatefall checkpoints through 0015; Checkpoint 0015 was initiated by the resident Context-Preservation Watch after twenty-one resolved exchanges, promoted all eight ledgers, advanced the registry through `EVT-000084`, updated the index, and emitted `exports/play_export_0003.md`. |
...
| V03-09 | Structured profile metadata | New checkpoint records world, version, and freeze status, or explicit no-profile | Passed — gate and observed | Gatefall workshop-draft Checkpoint 0004; frozen Profile 1.6 / Data Model 0.1.2 Checkpoint 0014; and the immutable frozen Profile 1.6 / Data Model 0.1.3 Checkpoint 0015, whose manifest correctly recorded no migration at capture. Current live restoration additionally applies the terminology-only Profile 1.6→1.7 Rank migration and additive Profile 1.7→1.8 `/system` layout adoption. `test_checkpoint_contract.ps1` Contract 6. |
...
| V03-19 | Prototype findings are dispositioned | The live prototype may reveal and drive changes to Rules, Data Model, Runtime, or other architecture; every finding must be explicitly classified, governed, versioned, migrated, and revalidated, with no known architectural gap left unaddressed at postmortem | In progress — Decision 076 implemented, revalidated, and exercised by a native 0.1.3 checkpoint | Decision 076 records the foundational Architecture-Freeze exception; Data Model 0.1.3 and its explicit immutable-checkpoint migration close the discovered Relationship Texture gap. Automatic Checkpoint 0015 proves the live campaign can promote and checkpoint natively under 0.1.3 while exporting the transcript. Final determination remains the 0.3 postmortem. |
