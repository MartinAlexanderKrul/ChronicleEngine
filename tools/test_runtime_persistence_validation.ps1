...
function New-BranchFromCheckpoint {
    ...
    foreach ($ledger in $requiredLedgers) {
        ...
        $text = $text.Replace('schema_version: "0.1.2"', 'schema_version: "0.1.8"')
        $text = $text.Replace('schema_version: "0.1.3"', 'schema_version: "0.1.8"')
        ...
        if ($ledger -eq "180_CURRENT_STATE.md") {
            ...
            $text = $text.Replace("Data Model 0.1.2", "Data Model 0.1.8")
            $text = $text.Replace("Data Model 0.1.3", "Data Model 0.1.8")
        }
...
try {
    ...
    Replace-ExactlyOnce `
        ...
        -Pattern '(?ms)\A(.*?schema_version:) "0\.1\.8"' `
        -Replacement '$1 "0.1.4"' `
        -Case "S-01"
    ...
    Assert-Contains $schemaResult.Output "live canon must conform to current Data Model 0.1.8" `
