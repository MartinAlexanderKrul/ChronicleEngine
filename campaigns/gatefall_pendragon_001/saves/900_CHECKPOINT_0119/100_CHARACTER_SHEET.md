...
provenance:
  source: EVT-000687
  game_date: "2026-08-23T23:30:00-05:00"
...
canonical_state:
  age: 25
  location: "**The Rogers Park apartment (`ENT-000087`), Chicago** — 2026-08-23, night, asleep beside Owen Callahan. Rift-stepped home from the warehouse district (`EVT-000686`) after declining to visit Walt Adamik (nothing due). Prior: The Nook, 1418 W Glenwood, evening — disclosed the BGM cooperation offer to Wade Bishop (`EVT-000685`). Prior: BGM Region V Field Office (`ENT-000093`), afternoon — voluntary re-assessment, carded S-Rank, reawakened classification filed; standing cooperation arrangement agreed with Desmond Farraday (`EVT-000683`-`EVT-000684`). Prior: Owen's boat, midday (`EVT-000680`)."
  ...
  system_state:
    ...
    mana: "1512/1512"
    world_clock:
      last_ticked: "2026-08-23T06:00:00-05:00"
      next_tick: "2026-08-24T06:00:00-05:00"
    ...
    temporal_state:
      campaign_time: "2026-08-23T23:30:00-05:00"
      mana_recovery_mode: resting
      mana_recovery_remainder_units: 0
      health_recovery_mode: resting
      health_recovery_remainder_units: 0
      note: "**Settled through `EVT-000687`, anchor 09:52 -> 23:30.** Active most of the span; 21 Mana spent (Keen Sense x7, three rift-steps), closed to full across the day. No damage. Both pools close **3,008/3,008** and **1,512/1,512**. Full history: `170_CHANGELOG.md`."
    unspent_points: 5                      # `ability_points_earned - sum(allocated)` = 351 - 346. `EVT-000665` claimed +4 and put all 14 into Strength; `EVT-000667` paid +5 at LEVEL 44
    pending_rewards:
      ability_points: 4
      ability_points_claim_note: "**+4 pending from the twenty-eighth daily** (`EVT-000680`, Ascetic equipped), unclaimed. Prior: `EVT-000665` claimed at +4. Status Recovery was already inside the 20."
      ability_points_note: "**2026-08-21: TWENTY-FIVE points allocated, and the record said fifteen** (`EVT-000591`, reconciled at `EVT-000594`). Vitality took **fifteen across two passes** (+10 then +5) — base 484 → **499**, effective 485 → **500**, **max Health 1,940 → 2,000**. **This is a change of strategy and the reasoning is recorded because of it.** With System Rank A re-pinning every Stat Passive at [S] the instant it released them, points buy derived numbers only — and **Vitality is the axis that gates Combat Tier S**, which needs 2,700 max Health at 46% reduction. Fifteen points closed **60 of a 760-Health gap**, which is itself the argument that **allocation has stopped being the lever**: at the new **+63 per level** rung the remaining 175 effective Vitality is under three levels. **Prior allocation history compressed at `EVT-000653`** — the `EVT-000585`, `EVT-000570`, `EVT-000564` and `EVT-000553` passes are held in full by `170_CHANGELOG.md`. Live figures are `unspent_points` and `stats` above."
      status_recovery: 21                  # +1 pending from the twenty-eighth daily (EVT-000680)
      daily_random_box: 1                  # +1 pending from the twenty-eighth daily (EVT-000680); opened at EVT-000665 (1 -> 0) prior
      unresolved_gate_loot: "**Fully resolved.** Nothing outstanding. Full history: `160_CAMPAIGN_CHRONICLE.md`."
      latest_settlement: "**2026-08-23 (`EVT-000680`): the twenty-eighth daily completed**, streak 15→16, before the boat. Three rewards pending, none claimed: AP +4, Status Recovery, Daily Random Box. Prior: **2026-08-22 (`EVT-000605`): the thirtieth daily's regimen completed** (streak 14 → 15) — run and calisthenics closed en route to Ashfield staging. Three rewards created pending and **none claimed this span**: Ability Points +4 (Ascetic equipped), one further Status Recovery, one further Daily Random Box. Prior: **2026-08-20 (`EVT-000553`): the twenty-seventh daily's three rewards settled.** AP **+4** all to Vitality — base 126 → 130, effective 131, `maximum_health` re-derived 508 → **524**, filled from cap. Status Recovery **18 → 19**, unclaimed. Box opened standard (streak 13, not a multiple of seven): 🎲 d100 **81** → ranked gear at System Rank B; 🎲 d3 **1** → weapon, 🎲 d7 **1** → **Quickknife [B-Rank]**, Agility +11, power 11, Excellent, unbound — banked. Prior settlements: `160_CAMPAIGN_CHRONICLE.md`."
    ...
    skills_known:
      ...
      - "Rupture [S-Rank] ★★★★☆ Expert · Mana 11 · **reach 5 m** · a heavy strike at **×2.90 of its skill-rank baseline** (S-Rank baseline 1000, Section 7.2), resolving as `(1000 + effInt + focus power) × 2.90 × result` under §6.2 — **DMG 5,092 standard before reduction** at effective Intelligence 756 (`(1000+756)×2.90=5,092`; re-derived at `EVT-000667`. Earlier derivations: `170_CHANGELOG.md`. **`magnitude_floor` 290 binds** — S-Rank Adept authors ×2.30, and Profile 1.83 clause 1 ratcheted the floor ×2.60 → **×2.90** on the two levels this skill climbed under a swallowing floor (`EVT-000601`). Categories: **Compound** (D); **Structural** (C); **Ward-breaking** (B); **Propagating** (A) — carries to one further target in contact with or behind the first; **`Unmaking`** (S) — **the strike ignores worn physical reduction entirely.** **Successful uses 128 · qualifying scenes total 42 · mastery progress 0/3 toward Master · mastery level EXPERT (4) · rank ascensions 5 · magnitude_floor 260.** **At its §7.5 ceiling (System Rank A + 1); finished until System Rank S.**"
      ...
      - "Mana Bolt [B-Rank] ★★★★★ Master · Mana 6 · **no distance limit; a target DETECTED but not seen is a legal target** · a ranged bolt resolving as `(150 + effInt + focus power) × result_multiplier` under §6.2 — **DMG 1,721 standard before reduction** at effective Intelligence 756 (`(150+756)×1.90=1,721`; re-derived at `EVT-000667`. Earlier derivations: `170_CHANGELOG.md`. **The stored ×1.75/×1.90 discrepancy on this line predates this checkpoint and is unrepaired here — flagged, not resolved** (see `170_CHANGELOG.md`). **`magnitude_floor` 175 binds.** Categories: **Arcing** (D) — the bolt bends around intervening cover; **Unsighted** (B) — a resolved Keen Sense or held read substitutes for eyesight. Never crosses a Gate boundary, a §9.6 seal, or a §17 instance wall. **Successful uses 87 · qualifying scenes total 21 · mastery progress complete Master · mastery level MASTER (5) · rank ascensions 2 · magnitude_floor 160.** **Expert → MASTER at `EVT-000650`**, third qualifying scene: six A-Rank commons in a Flooded Mine ambush, all one-shot on critical or strong results, none landed in return."
    ...
    tracked_counters:
      ...
      - { path: progression.ability_points_earned, baseline_value: 323, baseline_as_of: EVT-000597, current_value: 351 }
      ...
      - { path: skills.rupture.successful_uses, baseline_value: 16, baseline_as_of: EVT-000130, current_value: 128 }
      ...
      - { path: skills.dagger_mastery.successful_uses, baseline_value: 2, baseline_as_of: EVT-000130, current_value: 336 }
      ...
      - { path: skills.twin_fang.successful_uses, baseline_value: 0, baseline_as_of: EVT-000130, current_value: 135 }
      ...
      - { path: skills.mana_bolt.successful_uses, baseline_value: 0, baseline_as_of: EVT-000303, current_value: 87 }
    ...
    inventory:
      ...
      materials:
        - "**Mana Crystal [A-Rank] x 100** -- the Flooded Mine take. 203 mined of 208 + 8 Prospector across four worked deposits (`EVT-000652`) = 211, **+7 beast drops repaired at `EVT-000664`** (six commons and the elite under Section 11.1, never awarded at Checkpoint 0114) = **218**; **111 then 7 sold** to the System shop at 3,400 g for **401,200 g** (`EVT-000661`). **Structured line added at `EVT-000664`** -- the holding was carried only in `120_INVENTORY_AND_OWNERSHIP.md` prose since Checkpoint 0114, the same gap as the containment plant. Licensed value $50,000 each (Section 12.1); that desk was declined deliberately."
        - "**Beast Core [A-Rank] x 1** -- the Flooded Mine elite's core (`EVT-000650`), owed under Section 11.1 (*elites and bosses always drop one core each*) and **never awarded at Checkpoint 0114**; repaired at `EVT-000664`, player-caught. **The first A-Rank core this campaign has held.** Shop refuses cores (Section 12.5). Section 12.10 prices a fusion at one core at the result's Rank or four at the Rank below -- which is what the unfused Warded Vambrace [A-Rank] pair has been waiting on."
    ...
    daily_premium:
      ...
      model_bags:
        settled_through: "2026-08-22 rotation (this read)"
        weapon:
          previous_draw: "Colossus Greatarm"
          remaining: ["Execution Longsword", "Horizon Reacharm", "Farline Longshot", "Spellthread Focus", "Aegis Guard Shield"]
        armor:
          previous_draw: "torso/Gale"
          remaining: ["head/Gale", "head/Watcher", "torso/Arcanist", "hands/Gale", "hands/Watcher", "hands/Arcanist", "legs/Gale", "feet/Gale", "feet/Arcanist"]
        accessory:
          previous_draw: "Seer's Lens"
          remaining: ["Hunter's Band", "Mindspun Loop"]
          refill_note: "Drawn from the post-Heartward-Sigil refill set (Hunter's Band, Quickstep Charm, Seer's Lens, Mindspun Loop). Quickstep Charm was already drawn last cycle; Heartward Sigil rejoins the bag on the next natural refill."
        consumable:
          previous_draw: "Quicksilver Phial"
          remaining: ["Clarity Phial"]
        key:
          previous_draw: "Armory Key"
          remaining: ["Runic Key", "Alchemist Key"]
      purchased: "1/6 on the live Cobalt cycle — the Adaptive Gale Torso [A-Rank], 30,000 g, bought `EVT-000609`. **Banked, unequipped** — the player asked whether to swap it in over the worn Watcher Torso and the question was never answered; the Watcher Torso remains worn. **ROTATED live at the 2026-08-22 06:00 boundary**, checked at this `/system shop` read (part of `/system all`). Twelve real draws: fabrication series d8 3 → **Cobalt**; Rank d100s weapon 90 (+2 → S), armor 7 (A, flat), accessory 86 (+2 → S), rune 54 (A, flat), key 78 (+1 → S). Model bags: weapon → **Colossus Greatarm** (bag refilled from the Umbral/Verdant remainder), armor → **torso/Gale**, accessory → **Seer's Lens**, consumable → **Quicksilver Phial**, key → **Armory Key**. Rune skill d10 8 → **Mana Bolt** — offered at [A-Rank] against his known [B-Rank] Adept, genuinely above current Rank and not inert. Offers stand until 2026-08-23 06:00."
      offers:
        weapon: "Colossus Greatarm [S-Rank] · Premium model · two-handed · 75/25 Strength/Vitality split, +2 further Strength (premium) · armed strike ×1.0 base +0.15 premium = ×1.15 · weapon power 22 · 225,000 g · unpurchased — surrenders both Quickknives, Dagger Mastery and Twin Fang if equipped; stock only at this build"
        armor: "Adaptive Gale Torso [A-Rank] · Premium model · torso · Agility +9 (7 budget + 2 adaptive) · physical reduction 17%+3pp = **20%** · 30,000 g · **BOUGHT `EVT-000609`, banked unequipped** (read `unpurchased` against its own `purchased` field for a checkpoint; `F-037`, repaired `EVT-000618`) — a real upgrade over the worn Watcher Torso [B-Rank]'s 12%, at a Rank his gear has never carried on the torso slot"
        accessory: "Ascendant Seer's Lens [S-Rank] · Premium model · accessory · Perception +15 (12 budget + 3 ascendant) · 180,000 g · unpurchased — the shop's own version of this morning's Ashfield boss drop, Ranked higher and without the Boss Imprint"
        consumable: "Quicksilver Phial · +1 modifier step on Agility-governed actions for one scene, within the net ±3 cap · 750 g · unpurchased"
        rune: "Premium Rune — teaches **Mana Bolt** · [A-Rank] · UNBOUND · NPC-ELIGIBLE · 500,000 g · unpurchased — **not inert**: he holds Mana Bolt at [B-Rank] Master, and this offers ascension to [A-Rank], genuinely above current Rank"
        key: "Armory Key [S-Rank] · instant-dungeon key, boss drops one guaranteed equipment result in addition to its normal roll · 1,125,000 g · unpurchased"
      previous_cycle: "**2026-08-21, Verdant series — 0/6 purchased**; generated at System Rank A, the campaign's first, and expired unbought in full at the 2026-08-22 rotation. Prior: **2026-08-20, Umbral series — 1/6 purchased** (Ghost Quickknife [A-Rank], 50,000 g, `EVT-000557`), generated at System Rank B."
    non_daily_quests:
      capacity_total: 6
      active:
    ...
    daily_quest:
      ...
      active: false
      note: "**Twenty-eighth daily completed mid-morning** (`EVT-000680`), streak 15→16 — full regimen before the boat, no roll (routine). Three rewards created pending: Ability Points +4 (Ascetic equipped), one Status Recovery, one Daily Random Box, none claimed this span. Full streak history back to the fourteenth's deliberate failure: `160_CAMPAIGN_CHRONICLE.md` and prior checkpoint captures."
      current_quest: "No quest active; the twenty-eighth completed (see `note`). Prior daily detail: `160_CAMPAIGN_CHRONICLE.md`."
      next_issue: "2026-08-24T06:00:00"
