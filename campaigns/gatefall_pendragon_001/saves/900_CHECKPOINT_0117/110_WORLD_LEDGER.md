...
# Since Checkpoint 0102: EVT-000589 (a Bearer reaches System Rank A — a world-facing
...
**`GB-01`, `GB-02` and `GB-04` are retired from live tracking** (Section 9.10: a break spends the Gate; the release is one-time, and nothing here is re-entered or re-tracked). `GB-03` is likewise retired, cleared. **`GB-05`, `GB-06` and `GB-08` are live — `tracked_postings: 3` as of `EVT-000605`**, the board's entries since it emptied at `EVT-000413`. **`GB-05` and `GB-06` are now two days from surfacing to Alexander at the Ashfield staging** — Kesha Morrison's desk carries both, and today puts him in the room.
...
supply_sources:
  - owner: ENT-000134
    ...
    advanced: "2026-08-23T04:42:00-05:00"
    available: []
  - owner: ENT-000135
...
**All five are `advanced` to the current campaign anchor.** Four hold `available: []`, a real settlement rather than a convenience. `ENT-000130` (Kesha's desk) now carries three live entries — `GB-05`, `GB-06`, and today's `GB-08` — surfaced to Alexander this span. What changes from here is that the next read advances from a recorded anchor instead of from nothing, and a source whose `advanced` falls behind the campaign clock is a finding rather than an absence.

**A Bearer reached System Rank A, and nothing outside his own head registered it, 2026-08-21 16:53 (`EVT-000589`).** Recorded here rather than only on the character sheet because the gap it opens is a world fact. **His measured maximum Mana is 982**, which §19.2's re-pegged bands classify as **A-Rank** (475–1,224) on the instrument BGM and CGA both use. **His licence reads E.** He now carries 2,000 maximum Health against a fixed-Rank A hunter's 1,500 — **133% of one** — and three skills at [S-Rank], which nobody in this world has been recorded holding. **He has not requested re-assessment**, and §19.2 is voluntary by default: nobody is dragged in for out-performing a card. **The exposure trade in §19.4 is why**, and it is now the largest unreconciled fact in the campaign — an E-carded freelancer legally barred from strike contracts at C and above, who cleared four Gates yesterday and three today alone and unobserved.
...
aliases:
  - name: "what the fenced warehouse protects"
    quality: current
canonical_state:
  claim: "The fenced warehouse (`ENT-000166`) houses **a confirmed A-Rank Gate that its own operator believes is a C-Rank**, and the entire site exists to keep it suppressed rather than to farm it. Its registered operator is a shell entity, **Meridian Cold Storage LLC**, filed through a corporate formation mill with no traceable owner behind it. **Established in person 2026-08-22 (`EVT-000646`-`EVT-000652`):** the second structure is a purpose-poured containment room; its floor is a cooling manifold, not a slab; the 'climate plant' is suppression apparatus disguised as building services; and Flux Sight converted the site's own unconfirmed remote reading to **confirmed A-Rank by capability** (Profile §9.5), two Ranks above what every instrument on site reports. The Gate's boss carries an **old, load-bearing binding** holding it far under its own ceiling, degrading slowly. **The purpose half of this record is closed: containment, not extraction.** Who actually owns and operates it remains unestablished — its own containment monitor has never met a person from the company in eleven weeks. **Its suppression plant was removed entirely on 2026-08-22 21:08** (`EVT-000648`) and the slab now has no outflow."
  anchor: ENT-000166
  concealment: "The operation gives away nothing overt — no signage, no visible traffic — and its only trace is the physical investment in keeping conditions stable, which reads as anomalous only to someone who stops to compare it against its neighbors. The shell-company structure adds a second, deliberate layer: even a name traced to the property terminates at a formation service, not a person. **A third layer confirmed 2026-08-19**: a real anomaly-detection instrument nobody had disclosed, defeated only because it was tested for deliberately rather than assumed absent. **A fourth layer, established 2026-08-22 (`EVT-000647`): the staff themselves are compartmented.** The site's own containment monitor has worked it eleven weeks, has never met a person from the company, files twice daily to an inbox, was moved onto the post at three days' notice, and has twice been refused an answer about her predecessor. The concealment is not only against outsiders — **the operation is built so that the person standing next to the thing does not know what it is.**"
