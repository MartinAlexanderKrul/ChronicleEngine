# Gatefall — World Rule Profile 1.125
...
- **Replacement:** Experience (XP) awarded for genuine resolved challenge is causal canonical state. Crossing a threshold causes an immediate **level-up**: every base Stat rises by 1, five allocatable stat points enter the unspent pool, and Health and Mana restore in full. A spent stat point directly and permanently changes canonical capability. Skills acquired by rune, milestone award, or ratified practice are canonical capability.
...
1. Increase **all five base Stats** by the **rung gain** for the System Rank the Bearer holds as the level-up settles (Section 6.6), read from this table:
   ...
   |---|---:|---:|---:|---:|---:|---:|
   | **Rung gain, every Stat** | **+2** | **+4** | **+9** | **+23** | **+63** | **+30** |
...
The rung gains are derived from that ladder rather than chosen. Each is the per-level growth that carries a Stat from one rung's parity value to the next across the ten levels between them — parity being `Vitality = Rank Health ÷ 4` and `Intelligence = Rank Mana ÷ 2`, which is the same arithmetic Section 6.1 uses in the other direction. **The S-Rank row is the one rung with no parity target above it, and 1.105 authors it rather than inheriting A's (`F-054`).** Every rung below carries a Stat from one Rank's parity value to the next across ten levels; at level 50 the Bearer stands *at* S parity — automatic 1,008 plus the weakest legal creation Stat of 8 is 1,016 against a target of 1,000 — and the world has no Rank above S to aim at. Through 1.104 the S row simply repeated A's **+63**, which authored fifty-four further crossings of Rank-closing growth against a horizon that was already reached. At level 104 that had put the Bearer **4.4× S parity**, with a standard uncritical Rupture exceeding an S-Rank boss's whole Health pool — the exact failure Section 6.7 exists to name.

**+30 is one more Rank of growth, earned five times slower.** The Rank scale climbs **×2.5** (Section 6.1: Rank Health 40 → 4,000), so one further notch is 10,000 Health — a Vitality parity of 2,500 — and `(2,500 − 1,000) ÷ 50 = 30`. A Bearer crossing into level 100 arrives at that extended parity exactly as every earlier decade arrived at its own, and levels 51–100 remain worth something substantial without closing a gap that is already closed.

> **Past level 100 the rung is +1, for every System Rank.** This overrides the table above for any crossing entered at level 100 or higher.

**+1 is the ladder stopping, and it is not arbitrary.** Section 18.13.2 already declines to author rungs *"against a horizon the world does not have."* Five Stats at +1 is **+5 automatic**, which is exactly the level-up's own **+5 ability-point award** (Section 4.2) — so past 100 the automatic and allocated axes carry equal weight, and what the Bearer *chooses* finally decides his shape. Below that the automatic axis outweighs allocation 63:1 and a spent point means nothing.
...
| **B** | **Items.** Full effect of an unidentified rune, core, or item of equipment before use. |
...
**A Gate is read from outside (1.121).** The E-rung read fires when the Gate enters the Bearer's perception, at its mouth or across a cordon, and does not require him to enter it. It returns the Gate's true Rank as `confirmed by capability` on sight, ahead of any Section 9.5 entry roll. Player ruling C, recorded at `EVT-001052`; before 1.121 the rung's own wording implied this and never said it.
...
## 5.2.1 Draining Environments — where the pool is the organ

An ordinary environment can only slow the return of Mana. A **draining environment** consumes it, and **one is authored: the far side** (Section 18.13.1, `worlds/gatefall/270_THE_FAR_SIDE.md`). No Gate, Scar, anomaly, penalty zone, or trial drains, and a Runtime never rules that one does.

**Why a mana-dense place costs him Mana rather than giving it.** `200_WORLD_BIBLE.md` Section 5 says an unawakened body contracts the gray sleep because it has *"no rank, no pool, nothing to burn it off."* **The pool is the shedding organ.** The far side does not take his Mana; his body spends it, continuously and involuntarily, shedding what the place presses in. He is paying to stay unsaturated, and the bill does not stop while he stands there.

**On the far side, Section 5.2 recovery does not accrue at all.** Neither rate applies, **Conduit adds nothing**, the **Wellspring** title does not reach it, and there is no resting rate to fall back on — nothing in the Stacks permits rest, and a Bearer who sits down recovers exactly as much as one who walks. Section 18.2.2 already suspends elapsed-time recovery inside a trial; this is the same construction for a different reason.

**In its place the pool falls by 10% of maximum Mana per hour**, settled from elapsed fictional time exactly as Section 5.2 settles a gain — one figure for a narrated span, never a tick-by-tick stream. In the Section 5.2 half-percent units the far side is `rate_units = -20`, and `current_mana` is floored at zero rather than going negative.

**What that costs, stated plainly.** A full pool is **ten hours** in the Stacks and no more. A far door costs a flat 2,500 to open (Section 18.13.1), so a Bearer who walks through his own door arrives already down that much and has proportionally less: **the door is the largest single item on the expedition's budget, and it is spent before he arrives.** No rate, title, Stat, or consumable extends the ceiling — only leaving does.

**This is the authored reason the place cannot be lived in**, and it is arithmetic rather than assertion. There is no rest that outlasts it and no supply that offsets it.
...
**In a draining environment, zero is not the bottom.** Section 5.2.1's drain does not stop when the pool empties — the body goes on shedding with nothing left to spend it from, and **the drain continues against Health at 10% of maximum Health per hour.** The two clauses above bind throughout: he is at **−1 modifier step**, and every skill whose cost exceeds his zero pool is **unavailable**. That, not the Health loss, is what kills a Bearer in the Stacks — an empty pool in a place holding Combat Tier B-through-S things is the danger, and the Health drain is the fiction telling him he has already stayed too long.

**The drain inflicts no injury by the Section 6.3 hit test** — it is not a hit and no single quantity is removed. **But crossing below 25% of maximum Health while draining inflicts one injury of type `exhaustion`** (Rules Section 6.8), severity read by the Runtime from how far past the line he went. That injury is **the gray sleep arriving in an awakened body** (`200_WORLD_BIBLE.md` Section 5): the mana he could not shed has settled into tissue, which is precisely the civilian mechanism, reached by a man whose pool ran out rather than by one who never had a pool. It recovers on Section 6.3's clock and answers to the world's restorative alchemy (Section 12.5's off-shop note), never to a System consumable.

**Past that injury the condition is unauthored.** No profile text sets what a second, third, or chronic exposure compounds into, and **a Runtime that reaches it stops and asks the owner** rather than extending the ladder — Section 18.13.2's rule against inventing content against no evidence governs here exactly as it governs the seven unauthored terminal lines.
...
**A named per-exchange multiplier is its own factor (1.121).** Where an authored technique states a damage multiplier that grows with the fight, such as The Count in `270_THE_FAR_SIDE.md`, it enters the products above as one further factor. It is never a modifier step, so it sits outside the net ±3 cap that the engine applies to modifier steps, and it resets when the fight ends. The rule is general; The Count is the only technique that uses it.
...
A skill enters the Bearer's ledger by one of five routes, and only these:

1. **Rune.** A rune dropped as loot (Section 11) teaches one authored skill on use, consumed in the teaching, at the Rank of the Gate (or the Bearer's own System Rank, for a Daily Random Box result) it dropped from — the full rune table is authored with loot in Section 11.3.
2. **Earned by doing.** A method that passes the Skill-Formation Audit below and contributes qualifying evidence in **three distinct qualifying scenes** becomes ready for System ratification. A qualifying scene may involve danger, consequential work, or structured practice under the closed tests below. Evidence is recorded from the first qualifying scene; recognition never depends on the Runtime remembering prose across sessions. At the third scene the candidate becomes `pending-ratification` and the Runtime surfaces it at the scene boundary. Because an earned technique's name, Rank, Mana cost, and effect are not known merely from practice, final ratification requires an owner ruling unless this profile already authors the complete result.
3. **Stat Passive.** Crossing base Stat 30 grants the governing Stat Passive in Section 4.4; later thresholds raise its derived Rank. Equipment cannot trigger or advance it, and the Section 7.5 ceiling clamps the result.
4. **Taught.** A technique shown directly by one System-bearer to another, with no instructional item between them. It is the only route requiring a second person, the only one that transmits a **method** rather than an inscription, and the only one whose result need not exist on any table in this profile. See the Taught Instruction subsection below.
5. **Succession.** The Bearer personally kills a System-touched human and the System takes what they could do off the body. It is the only **instant** route, the only one whose price is paid entirely outside this profile, and the only one that widens by victim class rather than by table: an awakened human yields their techniques, a Section 13.1.1 bearer yields mastery, Stat Passives and class abilities as well. See Section 7.6, which authors it in full.

**Three of the first four are things that happen to him.** A rune is found, a Stat threshold is crossed, and a formation candidate is noticed by an audit he does not control. **Nobody teaches anybody anything in this world**, and until Profile 1.109 that was not a rule anywhere — it was the silent consequence of Section 2 leaving exactly one System in existence and Section 13.1 giving every other hunter no skill sheet to teach from. The fourth route does not relax either of those. It states what follows from them: **instruction between bearers is possible, there has never been a second bearer to try it with, and `270_THE_FAR_SIDE.md` Section 4 is where the others are.**
...
**Where a settled candidate lives (1.115).** A candidate's record is permanent and is never deleted, but a **terminal** one — `ratified` or `rejected` — is no longer live state. It has already written its result into `skills_known`, its counters into `tracked_counters`, and its reasoning into the canon-bearing Event that closed it; what remains on the candidate is an audit trail, not something a Runtime acts on before a scene.

Gatefall therefore names **two paths** for the Data Model 0.1.5 extension rather than one, which that document expressly permits — *"a world opts into and names their paths."*

- **`canonical_state.system_state.progression_candidates`** carries every candidate whose status is **open**: `tracking`, `pending-classification`, `pending-ratification`. This is readiness state, and the mandatory ratification gate above reads it.
- **`canonical_state.system_state.progression_candidates_settled`** carries every candidate whose status is **terminal**: `ratified` or `rejected`. Same shape, same required fields, same validation.

**A candidate moves at the moment it becomes terminal**, as the final step of the atomic implementation above — ratification and rejection each end by moving the record to the settled path. Nothing else moves it, and nothing moves back except an owner ruling reopening a `rejected` candidate, which returns it to the open path with its evidence intact.

**The invariant is unchanged and now spans both paths:** `domain` plus `key` is unique per subject **across the two together**. A candidate exists in exactly one of them.

**Why the split exists, stated plainly so it is not mistaken for tidiness.** The open path is loaded at every readiness gate; the settled path is fetched at settlement, with `tracked_counters` and `progression_audit_baselines`, which is the only moment it is read. At 1.114 this campaign carried **36 candidates in one field, 33 of them terminal**, and that single field was **55.8% of the protagonist's entire readiness load and 14% of the whole readiness surface** — a surface then sitting 67 tokens under its hard ceiling (`F-071`). A readiness selector names a field, and a field that accumulates settled history grows without bound while its name stays the same size. **This is the distinction that was missing, not a saving.**
...
A generated rune is a physical instructional item with a fixed identity. Complete live state records:
...
Binding follows provenance. Ordinary catalogue and Daily Random Box runes are `bearer-only`. Daily Premium runes, public-Gate boss runes, instant-dungeon and Runic-Key instruction, and authored found/world loot are `unbound-awakened` unless explicitly authored otherwise. Class-restricted instruction uses `class-bound:<class>`. **Spending a rune for mastery (Consumption Settlement below) is a Bearer-only choice regardless of the item's own binding** — an eligible NPC recipient is never offered it; only the Bearer's own duplicate-consumption branch has a mastery-spend option at all.
...
- **Bearer — known *at or above* `teaching_rank`.** The item has nothing to teach him, and **there is no reroll**. The System never re-draws a skill table to manufacture a result he happens to lack. He chooses instead, once, at consumption:
  ...
  - **Spend it for mastery.** Consumption instead counts as one qualifying dangerous-scene contribution toward one chosen known skill's current mastery level. This destroys the item and its transferable identity with it. It is the outlet for a `bearer-only` duplicate, which no other holder could ever consume.
...
### Taught Instruction — the route that needs a person (1.109)

**Who may teach, and it is a scope statement rather than a balance decision.** Only a holder of a System may teach by this route, and only to another holder of a System. Section 2 puts exactly one on Earth, and Section 13.1 gives an ordinary awakened no five-stat sheet, no mastery track, and therefore no method to hand over — **a hunter who is superb at something is superb at their Rank, and there is nothing underneath it to transmit.** The teachers this route contemplates are the failed champions authored in `270_THE_FAR_SIDE.md` Section 4, whose instrumentation has run without instruction for tens of thousands of years, and nothing else authored.

**It is not a download, and that is the whole difference.** A rune inscribes a capability in an instant; a teacher demonstrates a method and watches him fail at it. A taught technique therefore enters as a **candidate on the Skill-Formation Audit above**, not as a completed skill, with exactly two differences from an earned one:

- **The teacher supplies the signature.** Tests 1 through 3 — deliberate, repeatable signature, distinct capability — are satisfied at the first scene rather than argued, because the method came from someone who already holds it. A teacher pointing at a **combination** of skills he already knows satisfies test 3's combination clause the same way; that clause has always permitted composition and has never had anyone to suggest one.
- **The teacher's own authored entry supplies the result.** Ratification does not require the owner to invent a name, Rank, cost, effect or mastery axis, because the technique is authored on the teacher. What a champion can teach is stated on that champion under `270_THE_FAR_SIDE.md` Section 2's authoring discipline, and a Runtime that reaches an unauthored one stops and asks.

**Tests 4 and 5 stand entirely unchanged: three distinct qualifying scenes, materially contributing.** Being shown a thing is not holding it. The mandatory ratification gate, the consolidated adjudication queue, and the atomic implementation procedure above all govern this route exactly as they govern an earned one.

**Rank and mastery are untouched, and this route raises neither.** A taught technique enters at the **Section 7.5 ceiling** (`System Rank + 1`, clamped) or at the teacher's own authored Rank where that is lower, and it enters at **Novice** like everything else. **A teacher shortens the road to a technique and does not hand over the hands** — Section 12.5's *gold buys width, never hands* is preserved word for word, and so is the sentence next to it: mastery is never bought, and it is not taught either. It is still three qualifying scenes a level, twelve to Master, in his own hands.

**What this route carries that no other does: the technique can be taught onward.** A skill acquired by any of routes 1 through 3 is an **inscription** — the System wrote a capability into him and he was never shown how. A skill acquired by route 4 is a **method he watched someone perform**, and he can perform it for someone else. Live state records this on the skill:

```yaml
acquisition_route: rune | earned | stat-passive | taught | succession
transmissible: true            # route 4 only; every other route records false
taught_by: <teacher reference>
```

`transmissible` is fixed at acquisition, never rerolled, and **never granted retroactively to a skill already held** — a technique he has carried for a hundred levels off a rune is not one he was ever shown.

**Teaching an already-awakened recipient, with no item.** He may teach a `transmissible` technique to an awakened person directly. This is the first route in this profile by which anyone in the world gains a technique the Bearer did not loot for them, and it settles on the Consumption Settlement terms above with the item removed:

- The recipient learns it at `min(the Bearer's own Rank in it, the recipient's fixed Rank)`, exactly as an instructional item resolves for an NPC.
- **It costs no item and destroys nothing** — there is no item consumed at any point in the transfer. He may teach the same technique to as many eligible recipients as he has scenes for.
- **It takes scenes, not a minute.** Teaching is structured practice under Section 7.4 for the *recipient* and is settled at the promotion barrier like any other non-combat work. A hunter does not acquire a technique between sentences because a stronger hunter described one.
- **An unawakened person cannot receive one**, on the same footing as `unbound-awakened` instruction (Section 7.1): teaching transmits a method to a System-less body that can hold it, and awakening is not something this route can cause.
- **A Section 13.1 recipient still has no mastery track and no Stat sheet**, and gains neither. They hold the technique at their Rank, permanently at that value, exactly as they hold one consumed from a rune.

**What `transmissible` is worth, raised in 1.112 because route 6 made it worth measuring.** Through 1.111 the flag was a single grant — he could teach it onward — and Section 7.6 then authored a route that takes a technique instantly, at the victim's own mastery, and records `transmissible: false` on every entry it writes. **A route that is slower, costs three scenes, and arrives at Novice needs to give something the fast one cannot**, and `F-065` is the record of it not doing so. It gives two things, and both are structural rather than numerical:

- **Teaching credits the teacher.** When a recipient he has taught reaches the technique — ratified for a bearer, learned for a Section 13.1 awakened — **he receives one qualifying scene toward his own mastery of that same technique**, settled at the ordinary Section 7.4 promotion barrier. It is **once per recipient, never twice with the same person**, and it never applies to a technique he did not acquire by this route. This is not mastery bought or taught: it is Section 7.1's own *structured practice with real feedback* test, satisfied by the hardest version of that test there is. **Route 4 is the only route in this profile that compounds**, and a technique he can put in other hands gets better every time he does it.
- **Only a `transmissible` technique may be a component of a combination candidate.** Section 7.1 test 3 has always permitted a combination to qualify where *"the combined sequence creates a repeatable capability not already granted by its parts"*, and it has never said what may be a part. It says so now: **a skill he was shown may be decomposed and recombined; an inscription may not.** A rune wrote a capability into him whole and he has never seen its working — he can use it perfectly and cannot take it apart. Everything he holds off a drop table, and everything Section 7.6 takes off a body, is an inscription in exactly this sense.

**Together those make the taught route the only road to a technique that does not yet exist.** Route 5 hands him twenty finished things at somebody else's mastery and forecloses the twenty-first; route 4 hands him fewer, slower, at Novice, and is the only one of the five that can produce something nobody has ever held. That is the trade, stated, and it is now a trade rather than a strictly dominated option.

**Why this matters more than a technique, and it is the point of authoring the route at all.** Section 18.12.5 states the cost of a world that acquires one person who can answer anything: *"the response that used to exist has been reorganised around someone who was somewhere else,"* benches thinning, the mid-tier hollowing out. Section 12.5.1 already lets him spend gold on another awakened's **pools** and says the real limit is gold. **This route is the other half — capability rather than capacity — and together they are the only mechanism in this profile by which the Bearer can put back what Section 18.12 says his existence is taking out.** Nothing about that is free: every technique so transmitted is a Section 19 exposure thread with his name at the end of it, and an awakened hunter performing a method no instructor on Earth teaches is a louder anomaly than a potion in an evidence locker.
...
### Ratified Earned Technique — Wrought Hand

**Wrought Hand [S-Rank]** is an earned-by-doing technique under Section 7.1, ratified at `EVT-001121` on an owner ruling. It merges the two things the Bearer had been doing to other awakened people without a rule behind them: **coaching** a person's control of a power they already hold (`signature_control_coaching`, `EVT-001111`, `EVT-001114`) and **reshaping** a power they hold into something it was not (Kane's Standfast, `EVT-000952`). Both were played as provisional grants; this entry is the rule they were owed (Section 20.2).

| Skill | Rank | Mana cost | Effect |
|---|---|---:|---|
| **Wrought Hand** | S-Rank (native) | **400** for a reshaping working; **0** for coaching | Two workings on one other awakened person's own technique or signature, each aimed at a technique the Bearer can perceive. **Coaching:** graded exercises with real feedback, structured practice for the subject (Section 7.4). **Reshaping:** the Bearer spends Mana, rolls, and on success gives the technique a **branch**. |

**Native S-Rank, stated once.** An earned technique's native Rank is the Rank it is ratified at (Section 7.2), and Section 7.5's ceiling is System Rank + 1 capped at S, so at System Rank S the ceiling is S. Wrought Hand therefore stands at its native Rank and **at the ceiling**: it receives no Rank bonus, has no Rank above it to ascend to, and carries **no Section 7.3 Rank row and no Rank category**. Section 7.5 offers it nothing and it never becomes ascension-ready; its only growth is mastery.

**Coaching.** The Bearer runs a structured session on a person who is awakened and already holds the technique. The session is the subject's structured practice; for a Section 13.1 subject it yields **fictional control gains and no number** (no mastery track, no Stat sheet, Section 13.1). Coaching costs no Mana. It is the Bearer's own qualifying scene when its objective is bounded, its result is in doubt, and something is lost on a bad result (Section 7.4).

**Reshaping.** **The subject's consent is not required.** The working is aimed at a technique the Bearer can perceive in use, or one he has read; it is never aimed at a technique he only knows exists. It resolves as **one ordinary uncertain action** under Section 4: the subject's resistance and their fixed Rank are read as ordinary signed modifier steps from the established circumstances, and **no new step, cap or number is authored here**. On a failure the technique **breaks for the scene and no longer**, and the Mana is spent. On a success the technique carries a **branch**, which stands under Section 13.6 exactly as Standfast's three do: **method or reach only, permanent once earned, never a magnitude beyond the subject's Rank and the three Section 13.5 effects**, and independent of every other branch.

**What it never does.** It never adds a magnitude, so it cannot make a subject's technique stronger than their Rank allows. It never breaks a technique outright as an effect; that is Overpower's structural break (Section 4.4), and this skill's failure is the only way it breaks anything. It does not transmit a technique the Bearer was not shown (Section 7.1, route 4 still governs that). Reshaping a person who does not want it is a **Section 19 exposure thread** with the Bearer's name at the end of it, and what the subject then does with the branch is their fiction, not a resolved outcome.

**What this leaves untouched.** Kane's three Standfast branches are ratified standing law (`EVT-001079`, Section 13.6) and stand as they are; **no counter is backfilled** from `EVT-000952`, `EVT-001014` or `EVT-001070` or from Elena's lessons (Section 7.1: pre-ratification evidence establishes acquisition only). Wrought Hand governs reshaping and coaching from `EVT-001121` forward.

**Mastery.** The mastery axis is Mana, on the standing −10% rule: a reshaping working costs **400 / 360 / 320 / 280 / 240** from Novice to Master, and coaching stays at 0. It has no count of other people, because Section 7.4 clause 3 puts every such count on Rank and Rank adds none at S-native; **one subject per working or session at every level.** Its `Temper` dividend is authored in Section 7.4.
...
- **An item's authored ceiling binds this skill absolutely** (Section 4.4's ceiling read, and the campaign register it produces). Seamwork carries a piece **toward** its ceiling and never past it — a [C-Rank] piece at its ceiling is finished business *to his own hands*, and a [B-Rank] piece that can reach [A] can reach [A] and stop. **Ceilings are why he still shops**: the way to a higher ceiling is a higher-Rank core, and those are bought, commissioned, or killed for. **Through 1.109 this clause read *"cannot be raised by him or by anyone"*, and the second half of that was never true of anything but this skill (`F-058`).** Section 12.11 authors **re-coring**: an artificer reseats a piece around a higher-Rank core and the ceiling moves, after which Seamwork's own ladder has somewhere to go again. **The bound on this skill is unchanged** — he still cannot lift a ceiling with his own hands, because that is the signature's half of the trade (Section 12.8) and this skill has never held one.
...
|---|---|---|---|---|---|---|
| **Sprint** *(native E-Rank)* | +1 modifier step on movement, pursuit, and distance-closing actions | **Broken ground** *(D)* — the step holds across footing that would otherwise demand its own check: rubble, ice, slick or shifting surfaces, moving debris | **Vertical** *(C)* — vertical and overhanging surfaces count as ground while the effect holds; he crosses a wall or a girder as though it were floor | **Over the gap** *(B)* — a short unsupported span (open air, a stretch of open water) counts as ground for the duration, closing what would otherwise be a fall or a swim | **Unbroken** *(A)* — the effect no longer ends at an obstacle, a fall, or a change of medium: rubble, open air, water and collapsing ground resolve as one continuous line of movement until the duration lapses | **`No ground needed`** *(S)* — the effect no longer requires a surface at all: open air resolves as ground for its duration, so nothing in the terrain interrupts a line of movement. **Sustained cross-country travel at this Rank runs at 850 km/h, terrain allowing** (1.121; ratified at `EVT-001043` from the ~500 km in ~35 minutes the Paris to Amstelveen run implied) |
...
| **Wrought Hand** | Mana — the cost of a reshaping working (**1.124**); one subject at every level, so no count of others | 400 → 240 (400 / 360 / 320 / 280 / 240), coaching stays 0 | `Temper` — native S-Rank at the ceiling, so no Rank category exists to file it under |
...
| **Wrought Hand** | `Temper` — how well a reshaping working takes | The opening read names the technique's failure mode · A failed working leaves the technique unchanged instead of breaking it for the scene · A partial success keeps the branch in a narrowed form · The read names a workable branch, not only the failure · The branch holds on the subject's first unassisted use with no confirming roll |
...
## 7.6 Succession — what a killed System-touched human leaves behind

**The System transcribes.** When the Bearer personally kills a System-touched human, it takes what that person could do off the body and writes it into his ledger. This is the **sixth** acquisition route of Section 7.1 and the only instant one: every other route costs scenes, gold, or a Stat threshold, and this one costs a life.

**The trigger, exactly.**

- The victim is **System-touched** — either an **awakened human**, anyone carrying a Rank and a pool under Section 6.1, or a **System-bearer** under Section 13.1.1, whose whole authored population is the failed champions of `270_THE_FAR_SIDE.md` Section 4 and nobody else. Never a monster: Section 11.1 already authors what a monster leaves, and it is a core. Never an unawakened person, who holds nothing to take.
...
**What transfers is everything the body could do — and the two victim classes are not the same size.** Through Profile 1.110 this section took **one technique**, the signature, and nothing else. It now takes the whole of what the victim held, which is a far larger statement against a bearer than against a hunter, because Section 13.1 gives an ordinary awakened almost nothing underneath their Rank to take. The two rungs below are that difference written down rather than left to be discovered at the body.

### 7.6.1 Rung one — an awakened human

Section 13.6 makes an awakened human **one innate signature, fixed for life**, plus whatever runes have added on top, *"rare enough that a hunter with two tricks is notable and a hunter with three is a story."* Section 13.1 gives them no Stat sheet, no level, no mastery track and no class. Succession therefore takes:

- **Every technique they hold** — the Section 13.6 signature **and** every `learned_techniques` entry, not one of them. This is the whole of what 1.111 widens on this rung.
- Each enters at the holder's **effective technique Rank** under Section 13.6, clamped to Section 7.5's ceiling.
- **Mastery enters at Novice**, with successful uses, qualifying scenes and mastery progress at **zero**, because there is none on the body to take: Section 13.1 states that a learned technique carries *"no mastery track, no use counter, and no progression."* A rung that cannot reach mastery cannot violate the rule that mastery is never bought.
- **No Stat Passive and no class ability transcribes here**, because an awakened human holds neither. Section 4.4's passives are grown out of a base Stat and only a bearer has one; Section 18's classes are reached through a trial only a bearer takes.
- A signature grants **what it does** — method and reach — and never a magnitude beyond the arriving Rank, exactly as Section 13.6 binds it on the body. Section 13.5's three authored party effects do **not** come with it: an absorbed healing-touch signature grants its heal and not the innate-mender package of three field touches and one post-fight injury-tier reduction, and an absorbed barrier signature does not make him a party warden. This is Section 13.6's rule for a learned Mend, applied to a taken one.

### 7.6.2 Rung two — a System-bearer

Section 13.1.1 gives a bearer five Stats, `4 × effective Vitality` and `2 × effective Intelligence` pools, **skills with mastery levels**, **Stat Passives at their thresholds**, and **a class where one was taken**. Every one of those four is in scope, and each is answered here.

**A bearer has no Rank, so no victim Rank can be read.** Section 13.1.1 says it in as many words — *"No Rank, no Rank Health, no Rank Mana"* — and this section's arriving-Rank clause has always read the victim's. On this rung each skill enters at **the Rank authored for it on that bearer**, clamped to Section 7.5's ceiling. Where the champion's own entry authors no Rank for a technique, the Runtime **stops and asks the owner** under `270_THE_FAR_SIDE.md` Section 2's discipline and invents nothing — the same stop that governs meeting an unauthored entity there.

**Every skill transcribes at the victim's own mastery level.** A bearer's technique carries twelve qualifying scenes of somebody's hands, and on this rung killing them takes the hands with it. `mastery_progress` enters at 0 within the arriving level; `successful_uses` and `qualifying_scenes_total` enter at **zero**, because those are his lifetime counts and he has not used it yet.

**This is the profile's one exception to a rule it states twice, and it is authored as an exception rather than left to be found.** Section 7.1 and Section 12.5 both hold that mastery is never bought, and 1.109's taught route restated it word for word — *gold buys width, never hands* — and preserved it deliberately. Succession against a bearer is the single place where hands arrive without scenes. **What that does to the taught route is a real cost and is not settled here**: instruction from a champion is three qualifying scenes at Novice, and killing that same champion is instant and at Master, so route 4 is strictly the worse trade wherever both are available. Recorded as `F-065` rather than resolved by silence.

**A Stat Passive transcribes as a floor, never as a value.** Section 4.4 makes a Stat Passive's Rank *"canonical derived state, never a separately stored value"*, read from the Bearer's own base Stat on that section's threshold table; Succession cannot hand him a stored Rank without contradicting it. What it writes instead is a **succession floor** on that passive. The passive then renders at the **higher of his own derived Rank and the floor**, the floor itself clamped by Section 7.5's ceiling, and his derived Rank takes over silently the moment it passes the floor. Where the victim's derived Rank stands at or below his own, **nothing is written at all**. The victim's derived Rank is read from **their** base Stat on Section 4.4's table, which is the only Rank a bearer carries that is derivable rather than authored.

**A class ability transcribes as an ability and never as the class.** Section 18 binds a class to its trial, and nothing here reaches a trial. The named ability arrives at the ceiling and at the victim's mastery, and it grants nothing else the class carries: **no trial credit, no class evolution, no access to the rest of that class's kit, and no change to his own class line.** This is Section 13.6's *"grants its heal but not the full innate-mender package"* applied one layer up — the effect without the standing it came from.

**Stats and pools transfer nothing, on either rung, and that is deliberate.** Section 4.2 makes base Stats derive from allocation — `base_stat − creation_array[stat] − allocated[stat]` must be the same integer across all five Stats and equal `automatic(level)` — and a Stat arriving off a body is a point that reached a Stat with no Event declaring it, which is the exact defect that gate was built to catch. Succession takes **capability**, never capacity. Maximum Health and Mana follow from Vitality and Intelligence and therefore do not move either.

### 7.6.3 What binds both rungs

**The ceiling binds this route exactly as it binds every other, and the excess is lost.** A technique above Section 7.5's ceiling of **System Rank + 1** transcribes **at the ceiling**; the excess is not raised, waived, or banked against a later Rank, and the Mana cost reads from the arriving Rank's ladder. Categories at or below the arriving Rank are held; nothing above it is. **What 1.111 widened is how much comes across, never how high** — the ceiling is the one thing full absorption does not touch, and it is what keeps a killed champion from handing the Bearer a Rank the rest of this profile cannot price.

**Where he already holds the technique, nothing is taken** — no duplicate entry, no free Rank ascension, no mastery credit, no counter movement. The System records the death and moves on. Where he holds it *below* what the body carried, the ordinary Section 7.5 rune-ascension comparison governs and the arriving mastery is the rune route's, not the victim's: a technique he already owns is ascended, never replaced.

**Live state records the route, and a taken technique is never transmissible.** Section 7.1's acquisition block gains `succession`, and `transmissible` is **false** on every entry it writes:

```yaml
acquisition_route: rune | earned | stat-passive | taught | succession
transmissible: false           # succession is a thing taken, never a method shown
taken_from: <victim reference>
succession_event: <EVT-id>
```

**A method he was never shown is an inscription however it arrived.** Route 4 grants `transmissible` because a teacher performed the method in front of him; a body performed nothing. He cannot teach onward what he took off a corpse, and Section 7.1's rule that `transmissible` is never granted retroactively holds here without exception.

**It renders as a window** under Section 14.5.3's grammar, one line per entry taken:

```text
╔══ ◈  S U C C E S S I O N ═══════════════════════════════════════════════════
     <victim>             [<Rank or "bearer">]
     Transcribed          <technique>   [<Rank>]   ★☆☆☆☆ Novice   Mana <n>
                          <technique>   [<Rank>]   ★★★★★ Master   Mana <n>
     Held                 <technique>   — already known, nothing taken
╚═════════════════════════════════════════════════════════════════════════════
```

**The price is not in the profile, and 1.111 raised it.** Nothing here costs Mana, gold, an action, or a qualifying scene. What it costs is everything Sections 19 and 20.4 already author: killing a licensed hunter is an exposure event and a legal one, the body is evidence, the guild that carded them keeps a record, and a Bearer who has taken a technique is carrying the proof in his own ledger. **A Bearer who has taken a crew is carrying the crew**, and Section 13.6's *"in a world where every hunter is born as exactly one power, the man who can accumulate them is a walking impossibility"* is the whole exposure argument stated before this route existed. **The profile prices this at nothing and the world prices it at everything**, and that asymmetry is deliberate — Succession is a capability, not a strategy, and the Runtime is never obliged to make the world unaware of it.

### 7.6.4 Election — a taken technique may be declined

**Every clause above assumed a taken technique is a gain. It is not always one.** A technique whose effect removes an option from its own holder — rather than binding only the world around him — is not a grant merely because Section 7.6 acquired it as one, and Succession's automatic, total transcription had no way to refuse it. `F-068` (`engine/004_DESIGN_FLAGS.md`) recorded the case that broke this: `Plain Answer`, a Master-mastery compulsion that a taken technique made permanent and undeclinable — *cannot lie about a fact already asked directly and plainly* — landed on a Bearer whose whole exposure economy runs on not confirming what he is.

**The test, stated as a rule rather than left to the wording.** A technique's clause either binds the world (protects the holder — immune to compulsion, cannot be surprised, no fear penalty) or binds the holder's own choice (cannot lie, cannot act early, cannot refuse). **Only the second class is elective.** The first transcribes automatically, exactly as Section 7.6.1-7.6.3 already state it; nothing about ordinary yield changes.
...
**Election happens at the moment of transcription, not on a later window.** Section 14.5.3's Succession render (7.6.3 above) shows the complete set taken off the body — every technique, at its arriving Rank and mastery — in the same render that has always existed; that render is now also the only look the Bearer gets before choosing. For each entry whose clause binds his own choice under the test above, he elects it or does not in the same beat. A declined technique is **not acquired**: not banked, not stored, not recoverable later by any route this section authors. An elected technique whose clause is written two-directionally (binding both the world and the holder, as `Procedural Calm`'s *cannot be rushed or made to act early* bound his own timing alongside protecting him from outside pressure) may be **retained with its clause restated one-directionally** — protecting him from the world, never binding his own choice — rather than declined outright, where the fiction supports drawing that line without inventing a new effect.

**Section 7.6's economics are unchanged.** A declined technique still counts against the body for exposure and legal purposes exactly as a taken one does (7.6, "The price is not in the profile"), and route 4's comparison against Succession (`F-065`) is untouched — electing fewer techniques buys back nothing on the instruction side. This section governs only whether a yield can be poison, never what the kill costs or forecloses.

**Nothing else in this profile has been audited against the test above.** Section 16's assessed titles are granted, not chosen, by a different route entirely, and whether any of them carries a holder-binding clause is an open question this edge does not answer. See `F-068`, open design question 4.

---

## 7.7 Consolidation (1.116)

**Succession and the taught route both grow the ledger; nothing before this section ever shrank it.** A Bearer who accumulates techniques the way Section 7.6 describes accumulates duplicates with it — two, three, four grants that do the same functional work under different names, taken off different bodies at different times. This section is the one place the ledger folds rather than grows.

**Eligibility.** Two or more held techniques may consolidate into one only where they share a functional role: the same category (Section 15.3.1's Offense/Defense/Utility), and either identical mechanical shape (two flat-reduction Passives) or a close family the merge can hold as selectable tiers or modes without erasing a distinction (a free ranged read, a costed close read, and a costed multi-target read; a root, a bind, and a non-lethal takedown). **A Stat Passive is never eligible** — Section 4.4 derives its Rank from a base Stat rather than granting a stored value, and folding one into a flat grant would contradict that derivation. Two actives with genuinely different ranges, costs, or tactical roles are not eligible merely for sharing a category.

**No roll.** A Bearer consolidating techniques he already holds and already understands meets Rules Section 4.2's automatic-success standard on its own terms — failure is not a live possibility over recombining what is already fully his. This is a **deliberate departure from Section 7.6's own "nothing costs Mana, gold, an action, or a qualifying scene, but the world still prices it" framing**: consolidation is compression of existing capability, not new capability, so it carries neither the acquisition cost of a fresh technique nor Section 7.6's world-facing exposure price. It still takes a real downtime scene when played rather than ruled administratively — Section 7.1's discipline against buying width for nothing still governs the *scene requirement*, even though the *resolution* inside that scene is certain.

**The completeness test.** A consolidation is valid only if every constituent's distinct behavior is still reachable afterward, as its own tier, mode, or explicit component of the merged effect. A merge that would collapse two behaviors into one and silently lose either is not a valid consolidation under this section — it is either two techniques that were never actually eligible, or a merge that needs to be split back into modes rather than flattened. **Nothing here permits inventing a new mechanical value in the process**: a merged Passive's magnitude, where none of its constituents ever had one, is the *lowest* new authoring this section allows, and it is authored once, plainly, not derived from a formula the constituents never carried.

**What transfers, and what doesn't.** The merged technique keeps every constituent's `successful_uses` and `qualifying_scenes_total` summed (a use of any constituent was a use of the capability that now has one name), and keeps `transmissible: false` if any constituent was Succession-sourced and untransmissible — compression never launders a taken technique into a teachable one. Mastery level is the *highest* mastery level among the constituents; a constituent's `taken_from` history is preserved in the merged entry's provenance note rather than discarded, since Section 7.6.3's "a taken technique is never transmissible" reads that history, not just the flag. The constituent entries are struck from the ledger in the same Event that writes the merged one; a consolidation is never recorded as an addition without the corresponding removal in the same breath.
...
| 61–86 | A **rune** teaching one authored skill (Section 11.3 table) at the Bearer's System Rank. |
| 87–99 | A piece of **ranked gear** at the Bearer's System Rank (Section 11.5). |
| 100 | An **elixir** (+1 permanent to one stat, Section 12.5's lifetime cap applies). |

Any rune produced by a Daily Random Box has `instruction_binding: bearer-only` (Section 7.1). The box is the Bearer's alone and cannot be traded before opening because it is pending System state, not an object. Once opened, the chosen contents are ordinary dimensional-inventory holdings. One completion creates exactly one box. The streak changes only that box's roll mode at positive multiples of seven; it never adds a second box or changes the table.
...
```
   ...
   **Then the named institutions take their own share, and this step is where they do it (1.122).** The rates above are the trade as an **anonymous aggregate**, which is the right resolution while every guild is one bench in one metro. It stops being the right resolution once a guild holds offices across a whole region: at that scale the institution answers more Gates in a day than this step narrates in a week, and none of them could reach the fiction. **So every guild holding an `operational` branch rolls Section 13.7.5's daily dispatch here**, in this same pass, for **counts** rather than for Gates - it mints no posting, no per-Gate record and no break timer, and it is written to the tick's `guild_ops` line. **A background dispatch never resolves an A-Rank or S-Rank Gate**, at any bench depth: those surface through Section 9.11 above, or through the branch officer's own call, and they become scenes. The take rates in this step are unchanged and still govern the board; what 13.7.5 adds is who, by name, was doing the taking.
...
11. **The wider map.** Roll **d100** once on Section 9.1.6 for whether anywhere else in the country produced something the regions could not answer. **This step rolls three event kinds and nothing else** — an S-Rank Gate with nothing in range, a red gate at A-Rank or above, a break already loose — because everywhere that is not Chicago or Prague runs a light tick rather than this one. Steps 1–9 never leave the two simulated cities. **Section 9.1.8's reach pass runs here too, beside the light tick rather than inside it (1.123).** The light tick still rolls exactly what it always rolled - one d100 for what escalates to BGM National - and 9.1.8 separately reads the day's A-Rank and S-Rank counts across every zone the campaign can actually reach. **A campaign declaring no branch, anchor or agreement writes `none` and this step is unchanged for it.** **On a week's first tick, also roll Section 9.1.7** for the world outside the United States, which runs on the weekly clock beside Section 9.1.4 rather than this daily one.
...
**Each tick appends one entry to `world_ticks` in the campaign's world ledger**, carrying the boundary it ran for and what each of the tick's steps produced:
...
guild_ops    per guild holding operational branches (Section 13.7.5): the
             day's dispatched count by Rank, the duty-rate band, any distress
             call and how it resolved, and any A/S surfaced rather than
             cleared -- or none where no guild holds an operational branch
...
wider_map    the Section 9.1.6 d100, the event kind and its sited metro, the
             Section 9.11 reach roll and its band, or none
reach        the Section 9.1.8 A-Rank and S-Rank counts in reach, each
             S-Rank named with its sited zone and its Section 9.11 result,
             the one A-Rank that became a scene, and what the A-Rank posture
             absorbed -- or none where the campaign declares no reach zone
world        the Section 9.1.7 roll on a week's first tick -- its band, the
             sited region, and the door an ask arrived through -- or not-due
...
| 92–96 | **Escalation elsewhere.** Another metro has the crisis — Prague, a foreign branch, a district on the far side of Chicago. It pulls resources out of his city and thins the response here, raising pressure everywhere. **This is the weekly, shape-changing form and it acts on Chicago**; Section 9.1.6's step 11 is the daily, incident form and acts on him. They are different scales and do not double-count. |
...
### 9.1.6 The Wider Map — the light tick

Section 9.1 rolls Chicago. Prague runs the same ten steps beside it. **Everywhere else on Earth has no tick at all** — no Gate flow, no break clock, no board, nothing a Runtime can read when play reaches there — and for as long as a campaign stayed inside the two seeded cities that cost nothing. It stops costing nothing the moment a Bearer is carded S, signs a national incident-response agreement, and begins setting Section 18.8 anchors in cities the world has never simulated. Raised as `F-056`. The gap is not a city whose tick went stale; it is cities that were never given one, and Section 9.1's own reason for existing holds at the larger scale: **an un-run world is a readiness finding, not something nobody notices.**

**This is deliberately not a second Section 9.1.** Ten steps across thirty metros a day is a bookkeeping load no table survives, and nearly all of it would be Gates other people clear off-screen — which is already what the trade does in Chicago, in numbers, under step 4. **Superseded in part at 1.123: this filter is still the right one for escalation to BGM National, and is no longer the whole of what reaches him** - Section 9.1.8 adds a reach test for A-Rank and S-Rank Gates in jurisdictions where a campaign holds a branch, an anchor or an agreement. Everything below stands for every other place on the map. **What the wider map owes is not simulation; it is the three things that reach a national asset**: an S-Rank Gate nothing is in range of, a red gate no regional bench will enter, and a break already loose that outran the team sent to it. Those are rolled and written. **Nothing else is rolled at all.**

#### What a city's flow is, without rolling it

**Gate flow tracks licensed-hunter population, and this profile already demonstrates it twice.** Chicago carries ~1,200 licensed hunters against 5.5 Gates a day; Prague carries ~350 against 1.5 (`200_WORLD_BIBLE.md` Section 3, Section 9.1). That is **0.0046** and **0.0043** Gates per licensed hunter per day — within 8% of each other, on two cities authored a month apart and never written to agree. **The bands below are that ratio, not a new claim.**

| Band | Licensed hunters | Gates/day at tier III | Share of Band I | Rank ceiling | Worked example |
|---|---|---|---:|---|---|
| **I — Primary** | ≥ 1,000 | `1d4+3` (≈ 5.5) | 100% | full Section 9.2.1 | **Chicago** |
| **II — Major** | 400–999 | `1d4` (≈ 2.5) | 45% | full Section 9.2.1 | — |
| **III — Secondary** | 150–399 | `1d2` (≈ 1.5) | 27% | full Section 9.2.1 | **Prague** |
| **IV — Minor** | < 150 | `1d2−1` (≈ 0.5) | 9% | **no S-Rank Gate; A halved into B** | — |

**The bands ramp with the tier and are never restated.** Section 9.2.1 gives Band I's count at every tier; a lower band is that count times its share. The two authored rows do not move — **Prague's `1d2` *is* 27% of Chicago's `1d4+3`**, which is where the share column came from rather than a preference.

**The Rank ceiling is Section 9.1.7's mana-depth rule applied at metro scale, and it is not a difficulty setting.** Mana density sets **how high a puncture can reach**, not only how many open, and a metro's licensed-hunter population is the visible measure of it — the trade goes where the mana is, and 1,200 hunters in a city of 2.7 million is what a top-five Gate city looks like from outside. **A place thin enough to hold under 150 licensed hunters does not open S-Rank Gates**, because the pressure that would have to be behind one is not there to begin with. Band IV therefore rolls the tier distribution with **S at 0%** — the 0.5% carried up into A, that A halved, and the freed share into B: `E 50 / D 25 / C 15 / B 8.5 / A 1.5 / S 0`. **The C-and-above mass is unchanged at 25%**, which is why the break arithmetic below does not move.

**Bands I to III are untouched**, so **no authored city changes**: Chicago and Prague roll Section 9.2.1 exactly as they always have, and the ceiling only ever binds on places this profile has not authored.

A band answers every ordinary question about a metro without a roll: how much work there is, how deep its board runs (Section 9.10's three-to-six is Band I; a band down runs proportionally fewer), and how thick its bench is. **A Runtime asked whether there is Gate work in Boston reads the band and answers. It does not roll, and it does not author.**

#### The roster

**Band I holds exactly five metros**, because `210_PLACES.md` states Chicago is *"a top-five US Gate city in a country that measures cities that way"* and a top-five city implies four others.

| Band | Metros |
|---|---|
| **I** | New York · Los Angeles · **Chicago** · Dallas–Fort Worth · Houston |
| **II** | Atlanta · Boston · Detroit · Miami · Philadelphia · Phoenix · San Francisco Bay · Seattle · Washington DC |
| **III** | Charlotte · Denver · Honolulu · Kansas City · Las Vegas · Minneapolis–Saint Paul · New Orleans · Pittsburgh · Portland · Salt Lake City · San Diego · St. Louis |
| **IV** | Every other metro, and every county outside one |

**No Place or Settlement/Kingdom entity is allocated by this roster**, on the same footing as `210_PLACES.md`'s own Global section. These are names with a band, not authored places, and a metro becomes a place the day the campaign is standing in it (below).

**The roster checks out against the country's own totals.** Five Band I at 5.5, nine Band II at 2.5, twelve Band III at 1.5 is 68 Gates a day across the named roster, on roughly 15,300 of the United States' ~25,000 licensed hunters; the remaining ~9,700 sit in Band IV at ~0.5 a metro, adding ≈48. **That is ≈116 Gates a day in the United States** — and 25,000 licensed hunters at the ratio above independently gives **115**. Two roads, 1% apart.

**Federal geography.** The BGM runs **ten numbered regions**, each with a field office; **Region V is Chicago's**, as `200_WORLD_BIBLE.md` and `210_PLACES.md` already have it. A region answers its own incidents — that is what a region is for — and **BGM National escalates only what a region cannot reach in time.**

**Inside the United States there is no regional market factor.** One federal auction, one commodities exchange, and the two licensed refiners of `200_WORLD_BIBLE.md` Section 4 set one price: a core sells in Seattle for what it sells for in Chicago, and Section 12's tables apply unmodified anywhere on this roster. **Prague's ≈70% is a *foreign-market* factor** (Sections 12.1, 12.3) and does not generalize to Boston.

#### What escalates, and why it is this rare

The wider map — the United States outside Chicago — runs ≈**110 Gates a day** at tier III. Composed with Section 9.2.1's tier III distribution and Section 9.6's anomaly table, that is 0.55 S-Rank Gates, 2.64 red gates, and 0.44 breaks-in-progress-on-arrival **every day**. Ten regions absorb essentially all of it. What reaches BGM National is only what **the nearest adequate force cannot answer inside the Gate's Section 9.3 timer**, and each of the three has its own reason:

| Major event | Wider-map traffic/day | Escalates when | Rate | Stream/day |
|---|---:|---|---|---:|
| **S-Rank Gate** | 0.31 | nothing carded S is in range inside its two-day timer — eleven S-Ranks over ten regions, two of them in one city, several contract-locked and some on Jiu rotation | **1 in 5** | 0.06 |
| **Red gate** | 2.64 | its true Rank is **A or above**, which no regional bench enters sealed without a national asset | **2.4%**, the wider map's own A+S tail once Band IV's ceiling is applied | 0.06 |
| **Break already loose** | 0.44 | the Gate is **C-Rank or above**, so the release outruns the strike team that reached it | **25%**, Section 9.2.1's C-and-above tail, which the ceiling leaves unchanged | 0.11 |
| | | | **Total** | **≈ 0.23** |

**The S-Rank line reads 0.31 and not 0.55 because Band IV has no S-Rank Gates in it.** The wider map's S-Rank traffic is Bands I–III only — `(22 + 22.5 + 18) × 0.5%` — and Band IV's forty-eight Gates a day contribute none. The red-gate line thins for the same reason: the A+S tail is 3% across Bands I–III and 1.5% in Band IV, which weights to **2.4%** across the whole wider map rather than a flat 3%.

**Two of Section 9.6's four results never appear here at all.** A population one Rank above the assessment and a doubled archetype twist are exactly what a regional strike team exists for, and a region that cannot answer those has a bench problem rather than an incident.

**And the light tick applies the tier's anomaly band to a metro's whole flow rather than tracking confidence per Gate.** Section 9.5's band is a property of an *unconfirmed* reading, and the wider map has no per-Gate records to carry confidence on. This is a stated approximation, not an oversight: off the tracked board there is no posting for a Runtime to read a confidence off, and the band is the honest aggregate of how often a reading fails to describe the thing.

#### The roll

**One d100 on each tick, in step 11.** It is the daily national stream above, at 23 in 100:

```text
     01-77   THE WIDER MAP HOLDS       Ten regions answered their own. Written
                                       as `none`, and `none` is a result.
     78-83   AN S-RANK GATE WITH       Nothing carded S inside its Section 9.3
             NOTHING IN RANGE          two-day timer.
     84-89   A RED GATE AT A OR ABOVE  A sealed instance (Section 9.6) no
                                       regional bench will enter.
     90-00   A BREAK ALREADY LOOSE     Section 9.6's 96-00 at C-Rank or above:
                                       the release outran the team sent to it.
```

**Then site it — d100, weighted against cover rather than with flow.** An escalation is *least* likely where the benches are deepest, which is the same fact the table above is built on:

```text
     01-06   BAND I     roll d4 among New York, Los Angeles,
                        Dallas-Fort Worth and Houston
     07-24   BAND II    roll evenly among the nine
     25-52   BAND III   roll evenly among the twelve
     53-88   BAND IV    a minor metro; name one and it joins the roster
     89-00   OFF THE    federal land, tribal land, a rural county, a stretch
             ROSTER     of interstate -- thin coverage IS why it escalated
```

**On a `78-83` S-Rank result, site among Bands I–III only.** Band IV and the off-roster band have **no S-Rank Gates to site** — that is the whole of the ceiling rule and it may not be worked around by siting one there anyway. Re-roll `01-52` across the three bands, keeping their relative weights. The other two results site on the full table above, because a red gate and a break happen at every band.

**The `89-00` band is authored, not a leftover.** `200_WORLD_BIBLE.md` Section 5 states that *"the map of who dies to Gates tracks the map of who was already poor"*, and that is a domestic fact before it is an international one. **It is also the thinnest ground on the table**, so its ceiling is Band IV's or lower: federal range land does not open S-Rank Gates, and the reason it kills people is that nothing licensed is within four hundred miles of the D-Rank one it does open. A Gate on federal range land with no licensed hunter inside four hundred miles is the American version of the same sentence.

#### Whether it reaches him

**A written escalation then rolls Section 9.11 unchanged** — the same d100, the same `01-25 NAMED` / `26-50 OFFERED` / `51-00 NOT THIS ONE` bands, the same three refusal tiers including the draft. Two modifiers are added for the wider map and one existing modifier is read against National:

| Modifier | To the roll |
|---|---|
| A standing **National** incident-response agreement is live | **−20** — this is Section 9.11's existing cooperation-arrangement modifier, read against BGM National rather than a regional office |
| He holds a **standing Section 18.8 anchor** in the sited metro | **−20** — he is inside every response window by construction, which is Section 9.11's in-range modifier arriving by a second road |
| The event sites `89-00` **off the roster** and he holds no anchor within Section 18.8's travelling reach | **+20** — he cannot get there inside the timer any faster than the region could |

**This is what makes an anchor tour mechanical rather than flavour.** Section 18.8's *"has personally stood"* is a logistics fact about the world; here it is the difference between being called and hearing about it afterwards, and a Bearer expanding his reach is buying his own place on the national response.

#### What is written

**Section 9.1.1 governs, and `none` is written as a result** exactly as every other step's is. The tick's ledger entry gains one line:

```text
wider_map    the d100, the event kind and its sited metro, the Section 9.11
             reach roll and its band, and the Section 9.1.5 landing -- or `none`
```

**Every escalation lands as a *world fact* at minimum** (Section 9.1.5): a national incident is news the trade carries whether or not he was called. A `NAMED` or `OFFERED` result additionally lands as a **supply opening** on the issuing regulator, and a `NAMED` one he accepts lands as an **objective**. A `NOT THIS ONE` lands as a world fact and **who took it**, on the same footing as Section 9.1 step 4's taken postings.

#### What the light tick does not do

- **It rolls no Gates.** The band table is a rate, never a census. Nothing outside Chicago and Prague gains a per-Gate record, a tracked posting, a break timer, a contest roll, a district `pressure` value, or an agenda advance.
- **It feeds the escalation ramp nothing.** Section 9.2.2's credit accrues from districts the campaign actually tracks. A break in a metro nobody is simulating may not advance the world's tier, or the ramp becomes a function of how many cities happen to be listed rather than of how the world is going.
- **It sizes nothing to the Bearer.** Section 6.6 holds in full: an A-Rank escalation is an A-Rank Gate, and Section 9.11's own fences — creates no Gate, sizes nothing, is never a solo clear — apply here unchanged.
- **It is not a duplicate of Section 9.1.4's `92–96`.** That row is the **weekly, shape-changing** form: another metro takes the crisis and *thins Chicago's response*, which is a pressure effect on the city being simulated. This is the **daily, incident** form: one event, one place, one question about whether he is called. A tick that rolls both writes both, and they do not double-count because they change different things.

#### Promotion — when a metro stops being a band

**A metro leaves the light tick when the campaign anchors in it**, and only then: the Bearer takes residence, signs a standing arrangement with its regional office, or sets a Section 18.8 standing anchor there and works it more than once. Promotion authors what the full tick needs and nothing beyond it — **at least two districts** for Section 9.1.3, the regional office by name, the guild or co-op presence that fills its board, and whether it carries a Scar — and from the next day boundary that metro runs the full Section 9.1 beside Chicago's.

**Below that bar nothing is authored**, and this is the rule rather than the exception: **Prague was seeded as a second city because a campaign could start there, not because visiting a place earns it a tick.** A Runtime that answers a question about Boston from the band table has done the right thing; one that invents Boston's guilds, districts and board because the Bearer flew through has authored a city the world will now have to keep running.

**Everything outside the United States is Section 9.1.7's**, on a slower clock and through a different door.

### 9.1.7 The World Outside — the regions, and what crosses a border

Section 9.1.6 gives the United States a light clock because that is the geography a **BGM National** agreement covers. **The other 96% of the world's Gates had nothing at all** — and this is not a hypothetical gap in a world whose central mystery is a global break rate, whose catastrophe is Romanian, whose #1 hunter is Korean, and whose S-Ranks are counted as national assets precisely because they can be **borrowed across a border** (`200_WORLD_BIBLE.md` Sections 3, 5 and 7).

**Regions run one clock slower and one door further away.** A foreign escalation reaches an American S-Rank as *news* almost every time, and as a *call* only through an authority with standing to make one. So this rolls **once a week, on the same first tick as Section 9.1.4** — not once a day — and its ordinary output is a world fact rather than an offer.

#### The regions

**Flow tracks population here, not licensed hunters, and the switch is deliberate.** Section 9.1.6's band table reads licensed-hunter counts because inside one country the licensing rate is uniform, which makes hunter density an honest proxy for Gate density. **Across borders it is not**: `200_WORLD_BIBLE.md` Section 5 states plainly that *"where states are weak, Gates go uncleared and breaks are simply endured"*, so a thin licensed population is a symptom of coverage, never of flow. **Gates open where people are.** Licensing becomes the *coverage* term below, and reading it as flow would author a world in which poor countries have fewer Gates — the exact inversion of the setting.

**The rate is anchored on the United States, which Section 9.1.6 already derived two independent ways**: ≈116 Gates a day across ~340 million people is **0.34 Gates per million people per day**, and the world's ~1.6 million awakened at 1 in 5,000 confirms the same ~8.1 billion denominator. **That rate is a deep region's rate**, because the United States is one.

#### Mana density sets the ceiling, not just the count

**A Gate is a puncture, and a puncture is made by pressure.** Section 9.3 already reads a Gate's timer off *"its mana pressure — the same reading that Ranks it"*, and `200_WORLD_BIBLE.md` Section 2 puts hunters, Gates and loot on **one measured scale** so that *"a monster, a hunter, and a sword can be spoken of in the same breath."* **The scale is not three coincidences. It is one quantity.**

So **where the ambient mana is thin, the world does not merely open fewer Gates — it cannot open high ones.** The pressure that would have to stand behind an S-Rank interior is not there, and no amount of bad luck produces one. This is the rule, and it is the reason the world's S-Rank Gates and its S-Rank hunters are found in the same places rather than being a cruel accident of distribution:

| Mana depth | What it means | Rank ceiling |
|---|---|---|
| **Deep** | The trade's high Ranks are native here; the region produces its own S-Ranks | Full Section 9.2.1 distribution, S included |
| **Ordinary** | Capacity is real but partly bought or borrowed | **S at half share**, the freed 0.25% into A |
| **Shallow** | Thin ground. Ranks are misread far more often than they are high | **No S-Rank Gate at all**; S into A, that A halved, the freed share into B |

**Hunter population is the visible measure of it, and that is why the trade reads a map the way it does.** Guild strength *"scales with home-country size and Gate density"*, small nations *"import clearance capacity"*, and East Asia is at once the densest Gate region on Earth and the one that produced the early S-Ranks. **Those are one fact reported twice.** The measurement is honest at the top of the ladder in particular, because a state can license whoever it likes but **cannot license anyone into S-Rank** — the ~200 are counted individually, by name.

**The one place this measure lies is the Gulf**, which imports S-Ranks *"like any other luxury"*. A bought roster says what a state can pay, not what its ground carries, which is why the Middle East reads **ordinary** on depth while carrying a resident count that would otherwise argue for deep.

| Region | Population | Depth | Gates/day at tier III | S-Rank Gates/day | Resident S-Ranks | Days of S-Rank cover |
|---|---:|---|---:|---:|---:|---:|
| **Asia** | ~4.3B | **deep** | ~1,460 | 7.3 | ~95 | **13** |
| **Africa** | ~1.5B | **shallow** | ~230 | **none** | ~8 | — |
| **Europe** | ~745M | **deep** | ~253 | 1.27 | ~42 | **33** |
| **North America** | ~600M | **deep** | ~204 | 1.02 | ~26 | **25** |
| **Middle East** | ~500M | **ordinary** | ~119 | 0.30 | ~12 | **40** |
| **South America** | ~440M | **ordinary** | ~105 | 0.26 | ~14 | **54** |
| **Oceania** | ~45M | **shallow** | ~7 | **none** | ~3 | — |
| **World** | ~8.1B | | **~2,380** | **~10.2** | **~200** | **20** |

**What each region covers, so a Runtime never has to guess a boundary.** **Asia** is South, Southeast and East Asia — the densest Gate region on Earth and the most mature trade, Japan and South Korea exporting doctrine while China publishes nothing. **Europe** runs to the Urals under the European Gate Council and its licence-reciprocity treaty; the Czech Republic's ~3.6 a day sits here, Prague's 1.5 inside that, and Romania's Jiu Valley with them. **North America** is Canada, Mexico, Central America and the Caribbean **plus the United States**, whose ~116 is Section 9.1.6's own. **Oceania** is **Australia**, New Zealand and the Pacific — a small population on thin ground, and the least Gate-troubled place in this world by a wide margin. **Africa**, **South America** and the **Middle East** are the continental bodies of those names, the last taken to include the Gulf and the Levant.

**Depth scales the count as well as the ceiling** — thin ground opens fewer punctures *and* lower ones: a deep region runs the full 0.34 per million a day, an **ordinary** region **0.7 of it**, a **shallow** region **0.45**. Africa's ~230 a day against Europe's ~253 on twice the population is that factor, not an error.

**Both columns close.** The regions' S-Ranks sum to the Bible's **~200 worldwide**, and their Gate counts sum to **~2,380 a day**. Nothing here is a second rate; it is the one rate, depth-scaled, applied to the rest of the map. **North America's row contains the United States' ~116**, which stays Section 9.1.6's own and is never sited by this roll.

**Days of S-Rank cover is what the table exists to produce.** It is `resident S-Ranks ÷ (Gates/day × the depth's S-Rank share)` — how long a region's roster could answer its own S-Rank Gates at one apiece. **It is a ratio, not a schedule.** A shallow region reads `—` because it has **no S-Rank traffic to cover**, which is a different statement from having no cover.

**And this is where the ceiling rule earns itself.** Read without it, Africa carried 2.55 S-Rank Gates a day against a roster of fifteen — under six days of cover, a continent losing a valley most months, and a world that could not have exactly one Exclusion in it. **Read with it, thin ground has no S-Rank Gates at all**, and the thing that actually kills people there is the thing the Bible actually describes: *"under-ranked Gates and thin agency coverage"* at C-Rank and B-Rank, breaks *"simply endured"*, and a real toll far above the reported one. **Under-ranked is an assessment failure, not a Rank ceiling** — the two are different, and Section 9.5's confidence swing is where the first one lives.

**The world-scale figure, stated rather than smoothed.** At ~2,380 Gates a day the world opens **~10.2 S-Rank Gates daily** against 200 living S-Ranks, and they open **where the S-Ranks are**. The overwhelming majority are answered the way Section 9.3 already says — *"the nearest adequate force plus military containment"*, massed licensed hunters under a cordon, not a famous name.

**It does not demote the Jiu Valley — it is most of why there is only one.** An S-Rank Gate opens on deep ground, and deep ground is where the S-Ranks live, so the ordinary case is that one is answered. Romania is Europe: a **deep** region, thirty-three days of cover, and the Exclusion happened anyway because the timer beat the response into a valley with one road. **That is a failure of a covered region on its worst day**, which is exactly why the whole profession remembers it — and why a Runtime plays a shallow region's losses as a different and quieter kind of catastrophe rather than levelling the two.

#### The weekly roll

**One d100 on the first tick of an in-fiction week**, in the same pass as Section 9.1.4:

```text
     01-45   THE WORLD HOLDS      The regions answered their own. Written as
                                  `none`, and `none` is a result.
     46-70   NEWS THE TRADE       A named foreign incident: the region, which
             CARRIES              of Section 9.1.6's three kinds, and whether
                                  anyone reached it in time.
     71-85   A CONTAINMENT ARC    A region's cover ran out. Something is loose
                                  and stays loose -- a standing world fact on
                                  the Jiu Valley's own pattern, not a posting.
     86-95   THE REGISTRY MOVES   The IGR revises the World Ranking, a
                                  placement is disputed, or a clearance record
                                  is contested (`200_WORLD_BIBLE.md` Section 3).
     96-00   AN INTERNATIONAL     A request reaches him THROUGH an authority.
             ASK                  See the border rule below.
```

**Then site it — d100, weighted by a region's share of world flow lifted where agency coverage is thin**, because what makes news is a region being unable to answer what it opened:

```text
     01-48   ASIA             61% of the world's Gates; thirteen days of
                              cover, the tightest on the table
     49-72   AFRICA           a tenth of the world's Gates against the
                              thinnest agency reach on Earth
     73-80   MIDDLE EAST
     81-87   SOUTH AMERICA
     88-93   EUROPE
     94-99   NORTH AMERICA    Canada, Mexico, Central America, the Caribbean.
                              The United States is Section 9.1.6's own roll
                              and is never sited here.
     00      OCEANIA
```

The bands are that composition **rounded so every region stays reachable** — unrounded it puts nearly four fifths of the world's news in two regions, which is arithmetically defensible and makes five continents scenery.

**A shallow region's event is never an S-Rank Gate**, on the ceiling rule above. **Africa and Oceania produce A-Rank and below**, and a containment arc sited there is an A-Rank release nobody reached rather than an S-Rank one — which is the more common disaster in the real world of this setting, and the one its own text describes.

#### The border rule

**No regulator's call crosses a border, and the draft never does.** Section 9.11's emergency mobilization is the BGM compelling a hunter it licensed; **no authority on Earth can compel a foreign S-Rank**, and `200_WORLD_BIBLE.md` Section 3 is explicit that an S-Rank *"cannot be compelled by any institution."* A `96-00` therefore arrives as an **ask**, never as an assignment, and it arrives through exactly one of four doors:

| Door | Who asks, and what it costs |
|---|---|
| **The IGR** | The Geneva clearinghouse requesting a verified clear or a containment rotation. Prestige, a record on the World Ranking's own ledger, no compulsion. |
| **The European Gate Council** | A member state short of capacity, routed through the Council rather than bilaterally. |
| **BGM National** | His own regulator lending him out. **This is the only door that touches his standing**: declining costs him with the office he signed with, on Section 9.11's standing-cost tier — never the draft. |
| **A state buying capacity** | The Gulf pattern: a direct commercial offer at whatever a sovereign will pay. Section 12.3's national-asset scale does not bind a foreign buyer, and the number is negotiated. |

**Reach rolls Section 9.11 with one modifier and one prohibition.** A standing Section 18.8 anchor in the region is worth the usual **−20**; everything else about a foreign ask carries **+20**, because he is nobody's asset there and the request has to survive a diplomatic route his own regulator did not build. **The emergency-mobilization tier is unavailable on this roll at every result** — a foreign Gate is never a licensing offense to decline.

**And `200_WORLD_BIBLE.md` Section 7 names the price this table charges rather than models**, filing it as regulator-tier knowledge: which S-Ranks are actually available, which are contract-locked, and *"the true diplomatic cost of borrowing one across a border."* That cost is played as fiction against the asking entity's own ledger, not rolled here — and a Bearer who has not reached that knowledge tier does not get to see it priced.

#### What is written, and what this does not do

The tick's `wider_map` line carries the weekly roll alongside the daily one, or `not-due` on the six days a week it does not run — **the same treatment Section 9.1.1 already gives Section 9.1.4's weekly roll**, and for the same reason.

- **It authors no city.** A region is a rate and a coverage ratio; it has no districts, no board, no guilds and no regulator by name until Section 9.1.6's promotion rule is met, which a foreign metro reaches the same way a domestic one does.
- **It rolls no Gates**, feeds Section 9.2.2's ramp nothing, and adds no supply — every Section 9.1.6 fence applies here unchanged.
- **It never produces a System quest**, on Section 9.1.5's boundary: what the world does abroad is a world fact, and the System measures the Bearer.
- **It is not a travel system.** Reaching a foreign Gate is fiction — a flight, a Section 18.8 anchor, or someone else's aircraft — and the +20 above is the whole of what this section says about the difficulty of getting there.

### 9.1.8 Reach — how an A-Rank or S-Rank Gate arrives from anywhere on Earth

Sections 9.1.6 and 9.1.7 gave the world outside Chicago and Prague a clock, and both filtered hard on purpose. The light tick narrowed a continent to *"the three things that reach a national asset"*; the weekly roll narrowed the other six regions to news. **Both filters asked the same question — does this escalate to BGM National? — and that question has quietly stopped being the right one.**

**What the filter costs, measured rather than asserted.** Composing the two cities' own ticks with the two filters gives the whole A-Rank and S-Rank supply a campaign can see:

| Source | A/S reaching the fiction per day |
|---|---:|
| Chicago's full tick, 7.5 Gates/day at tier VII's 11% A+S | 0.83 |
| Prague's full tick, 3 Gates/day at the same share | 0.33 |
| Section 9.1.6's light tick — one d100, only bands `78-89` | 0.12 |
| Section 9.1.7's weekly roll, ordinarily news rather than a call | 0.14 |
| **Total** | **≈ 1.4, and 82% of it is two cities** |

**Against a world that opens two orders of magnitude more.** Section 9.1.7's region table is authored *at tier III* and says so in its own column header. The world is at tier VII, where Section 9.2.1 raises the count by 1.364 and the S-Rank share from 0.5% to 2% — so every figure read off that table unadjusted is 27% low on volume and **four times low at the top of the ladder**. Restated at tier VII, from the same rows and the same depth rules:

| Region | Depth | Gates/day | A-Rank/day | S-Rank/day | Resident S-Ranks | **Days of S-Rank cover** |
|---|---|---:|---:|---:|---:|---:|
| **Asia** | deep | 1,991 | 179 | 39.8 | ~95 | **2.4** |
| **Africa** | shallow | 314 | 17.3 | **none** | ~8 | — |
| **Europe** | deep | 345 | 31.0 | 6.9 | ~42 | **6.1** |
| **North America** | deep | 278 | 25.0 | 5.6 | ~26 | **4.6** |
| **Middle East** | ordinary | 162 | 16.2 | 1.6 | ~12 | **7.3** |
| **South America** | ordinary | 143 | 14.3 | 1.4 | ~14 | **9.9** |
| **Oceania** | shallow | 9.5 | 0.5 | **none** | ~3 | — |
| **World** | | **~3,246** | **~283** | **~55** | **~200** | **3.6** |

**And that last cell is the finding, not the arithmetic.** Section 9.1.7 computed twenty days of world S-Rank cover and built its whole treatment of the Jiu Valley on it — *"an S-Rank Gate opens on deep ground, and deep ground is where the S-Ranks live, so the ordinary case is that one is answered."* **At tier VII the world has 3.6 days of cover, and Section 9.3 gives an S-Rank Gate a two-day timer.** The ordinary case is no longer that one is answered. Asia, which holds nearly half the world's S-Ranks, is at 2.4 days and inside the margin already.

**Nothing was wrong when it was written.** This is the ramp's own endgame arriving on schedule — `200_WORLD_BIBLE.md` Section 9's *"Gates get denser, Ranks climb"* reaching the rung where the top of the ladder outgrows the people who answer it — and the defect is only that no section read the consequence. **A world four rungs up its own curve cannot keep routing its worst Gates through a filter built for the flat decade.**

#### Reach replaces escalation

**A Gate surfaces where he could actually be**, not where a federal office happens to escalate. Four zones carry a **daily** A/S clock; everything else keeps Section 9.1.7's weekly one and stays news.

| Zone | What it covers | Authority |
|---|---|---|
| **A full-tick city** | Chicago, Prague | Section 9.1, unchanged |
| **An operational branch** | its regulator's whole jurisdiction | Section 13.7.5 |
| **A standing anchor** | the locality the anchor stands in — **not** its region | Section 18.8 |
| **A standing agreement** | whatever the agreement itself names | Section 9.11's call list |

**An anchor buys a place and a branch buys a region, and the difference is not arbitrary.** A branch's catchment is bought by dispatch infrastructure — an officer, a bench, a filed permit, a roster that can be moved. **An anchor is one doorway.** It puts him on the ground and nothing else, so it reaches what he can reach on foot from where he lands.

**Reach is a property of the campaign, not a list in this profile.** A campaign declares its branches, its anchors and its agreements in its own records, and this section reads them. **A campaign with no branches and no anchors runs exactly as it did before 1.123** — which is the whole of what this section does to any world that has not built what this one has.

#### The daily reach roll

**One pass per tick, in Section 9.1's step 11, beside the light tick rather than inside it.** Compose the reach zones' total flow against the tier's A-Rank and S-Rank shares, and read two counts:

```text
     A-Rank in reach today      reach flow  x  the tier's A share
     S-Rank in reach today      reach flow  x  the tier's S share, depth-adjusted
```

**Worked for the campaign that needed this**, at tier VII — eight BGM regions holding all five Band I metros plus France, Germany, the Netherlands and Czechia, ≈220 Gates a day of reach flow:
...
**These are counts, not scenes, and the next two rules are what keep them that way.**

#### A-Rank: a standing posture, and one scene a day

**A guild may declare a standing commitment for its A-Rank response pool** — which catchments it answers in, and under what conditions — and that declaration resolves as **counts**, on Section 13.7.5's own terms: revenue composes, casualty exposure applies, duty rate is charged. **Committing the pool is a posture you set once, not a decision you make nineteen times a day.**

**The pool's capacity is Section 9.4's arithmetic and nothing softer.** An A-Rank Gate's legal minimum is **16 combat-rated hunters including at least one A-Rank**, and A-capable means carded A or B. A roster of 86 A-Ranks and 223 B-Ranks fields **19 concurrent A-Rank elements**, every one of them satisfying the `≥1 A` clause.

**That number is almost exactly the footprint's own A-Rank traffic, and the coincidence is the point.** A guild this size can just barely answer the A-Rank Gates its own regions open, and nothing beyond them. **Running the posture at full tilt costs 304 hunter-slots a day on top of Section 13.7.5's 882** — 1,186 against an 1,812 roster, a **65% duty rate**, which is **Strained**. The A-Rank posture is therefore a real budget decision competing with the ordinary dispatch, not a free upgrade, and a guild that answers every A-Rank Gate in its footprint has stopped being able to say yes to anything else.

**At most one A-Rank Gate becomes a scene on an ordinary day.** Roll Section 9.11 unchanged for each A-Rank in reach; the **strongest single result** — a `NAMED` over an `OFFERED`, the lowest roll among equals — is the one that arrives as a scene. Every other A-Rank that day is a count: answered by the posture, taken by a rival, handled by the region, or left, and recorded as a line rather than played.

#### S-Rank: every one surfaces, and there is no cap

**Every S-Rank Gate inside a reach zone is named and reported.** Not aggregated, not absorbed, not left to a d100 deciding whether he ever learns of it — **the guild knows, and he knows.** There is no daily budget on this and there is not going to be one: at 3.6 days of world cover against a two-day timer, an S-Rank Gate is the one event this world can no longer treat as routine, and a campaign that heard about a fraction of them would be reading last decade's world.

**Surfacing is not being called, and Section 9.11 is unchanged.** Each surfaced S-Rank rolls `NAMED` / `OFFERED` / `NOT THIS ONE` with its existing modifiers, plus one read off the table above:

| Modifier | To the roll |
|---|---|
| The sited region's **days of S-Rank cover is under 2** — below the Section 9.3 timer, so it structurally cannot answer its own | **−30** |
| The sited region's days of cover is **2 to 4** | **−15** |
| Over 4 days of cover | **0** |

**Section 9.1.7's border rule binds here without exception.** A foreign S-Rank Gate arrives as an **ask** through one of its four doors and **never as a draft**, whatever this section's counts say. The emergency-mobilization tier remains unavailable on any foreign roll.

**Concurrent calls are the ordinary case at tier VII, and they are triage rather than a queue.** Four S-Rank Gates in a day against a two-day timer and one body is a **choice**, and the profile means it as one: he answers what he can reach, and what he declines or cannot reach resolves without him. Section 9.3 already says what that resolution is — *"the nearest adequate force plus military containment"* — and Section 9.1.3 already says what it becomes when the force is not adequate. **A new Scar entering the world because he was three hundred miles away is a legitimate outcome of this section and must not be routed around.**

**And the guild cannot help him with any of it.** Its member roster carries **zero S-Ranks** and a board seat carries no membership. The only instruments that change this are the independent S-Rank network and whatever standing arrangements he builds with S-Ranks who are not his employees — which is what makes that thread load-bearing rather than social.

#### What is written

The tick's ledger entry gains one line:

```text
reach        the day's A-Rank and S-Rank counts in reach, each S-Rank named
             with its sited zone and its Section 9.11 result, the one A-Rank
             that became a scene, and what the A-Rank posture absorbed -- or
             none where the campaign declares no reach zone
```

**`none` is a result and is written as one**, on Section 9.1.1's discipline. A campaign with no branches, no anchors and no agreements writes `none` every day and is unaffected by this section entirely.

#### What this does not do

- **It creates no Gate.** Every count here is read off flow Sections 9.1.6 and 9.1.7 already author; this section decides which of them the fiction hears about, exactly as Section 9.11 decides which reach a hunter.
- **It sizes nothing to the Bearer**, and Section 6.6 holds in full. An A-Rank assignment is an A-Rank Gate.
- **It authors no city.** A reach zone is a rate and a jurisdiction. Section 9.1.6's promotion bar is untouched, and a metro gains districts, a board and a pressure value only by meeting it.
- **It feeds Section 9.2.2's ramp nothing.** Credit accrues from tracked districts, and a reach zone is not one.
- **It does not make him the world's S-Rank.** Asia opens forty S-Rank Gates a day and he will hear about none of them, because the guild holds no office there and he holds no anchor. **That silence is the argument for the next branch**, and it is meant to be felt as one.
...
| **Elsewhere in the United States** | The metro's **BGM regional office** issues the emergency contract → **nearest guild strike team** dispatched under it, on Chicago's own pattern. Where the release outruns the region — Section 9.1.6's C-Rank-and-above case — **BGM National** escalates across regional lines and the response is assembled under Section 9.11. |
...
**Above both of them sits BGM National, and it is a different list.** A regional office calls inside its own region; **National calls when no region can answer in time**, across regional lines, from a federal list carrying every carded S-Rank in the country plus every A-Rank under a standing national agreement. This is the channel Section 9.1.6's light tick feeds — an S-Rank Gate with nothing in range, a red gate at A-Rank or above, a break already loose past its region — and it is why a national incident-response agreement is worth signing and worth pricing. **A hunter on a regional list only is never called out of his region**, however capable he is, because the office that would call him has no standing to.

**Assignments are rolled on the world tick, never narrated at need.** Each A-Rank or S-Rank Gate the Section 9.1 tick rolls in a jurisdiction generates one **directed assignment** in addition to whatever the board does with it, and Section 9.1.6's step 11 generates one for each escalation it writes. For each, roll **d100** for whether it reaches the Bearer, applying the modifiers below to the roll before reading the table:
...
| 41–72 | **Rune** — roll d10 on the Section 11.3 table; it teaches that authored skill at the Gate's own Rank (Section 7.1). |
| 73–87 | **Potion cache** — 3 potions at the Gate Rank (Section 12.5). |
| 88–97 | **Instant-dungeon key** at the Gate Rank (Section 17). |
...
## 11.3 The Skill Rune Table

A **Skill Rune** (boss drop 41–72, Daily Random Box 61–86, Daily Premium purchase Section 12.5) teaches its skill at the **rank matching its source's own Rank** — a rune from a C-Rank Gate teaches its skill at C-Rank, and a rune from a Daily Random Box teaches at the Bearer's own System Rank (Section 8.1). Every instructional drop is one item, one table — Profile 1.118 folded the former rarer "skill book" tier into this one (`migrations/1.117_to_1.118.md`); there is no longer a common rune that caps below its source's Rank. Roll the taught skill on a d10:

| d10 | Skill Rune teaches |
|---|---|
...
Entries 1–8 are the eight starting skills of Section 7.3, taught here at the source's own Rank — every Skill Rune reaches it, not only a rare roll. **Rupture** and **Bulwark** (entries 9–10) are authored here and enter the ledger with their name, rank (the Gate Rank the rune dropped from, or the Bearer's System Rank for a Daily Random Box rune), Mana cost, and effect on the schedule of Section 7.2.

**Dropped instructional-item settlement.** A generated rune teaches the rolled technique at the drop's Rank, never below the skill's native Rank, and records `instruction_binding: unbound-awakened` plus its source event. That physical identity is fixed even if the Bearer already knows the technique, and generation is **never** bounded by the Section 7.5 ceiling — a C-Rank Gate drops C-Rank instruction regardless of who opens it. Recipient eligibility, NPC rank capping, ascension, the ceiling, and the Bearer's consumption-time duplicate handling all resolve at consumption under Section 7.1. A drop is never rerolled or re-drawn for teaching something the Bearer already knows.

**Class-restricted runes** exist only as **authored named items with provenance** — recorded as `class-bound:<class>`, usable by no one but the holder of their class, and entering play solely where a file authors them, exactly as the named-uniques rule (Section 11.5) requires.
...
### 12.1.1 Unbound-Awakened Runes (1.121)

An unbound-awakened Skill Rune (Section 7.1, Section 11.3) is valued against the **combined crystal and core haul of a Gate two Ranks above the rune's own**: **1x** for a defensive or utility technique, **1.5x** for an offensive one. It is never valued against the haul of its own Rank. A rune is non-renewable and crystal is renewable, so the premium prices genuine scarcity. Worked out in play (`EVT-001076`), ratified as standing convention (`EVT-001079`), and transcribed here. The ratification names no rung above A, since no Gate sits two Ranks above an A- or S-Rank rune; a Runtime meeting one stops and takes an owner ruling instead of extending the ladder.
...
| **Guild-salaried B-Rank** | ~~≈ $240,000 / year~~ **superseded — see Section 13.7.3** |

**This B-Rank salary figure is superseded by Section 13.7.3, not merely narrowed.** 13.7.3 re-derives it as $300,000/year against a real per-clear cadence check (the $240,000 figure survives only as the *freelance-average* comparison point 13.7.3 computes independently, a coincidence of arithmetic rather than a retained citation) and extends the same treatment to every Rank E through A. A Runtime pricing any guild salary reads 13.7.3, never this row.
...
**Off-world, an effect that reads his sheet degrades to its flat form.** The far side is where System evaluation stops — Section 18.13.1: *the System has never seen these things and does not price them*, which is why nothing there carries a Rank and no drop table fills. **A Greater potion's *restore to full* is an instruction to read a maximum**, and on the far side there is no reading. So **a Greater healing or mana potion drunk on the far side functions as its Standard counterpart** — a flat 90 Health or a flat 40 Mana — **and is consumed in full at its own price.** Lesser and Standard potions deliver a fixed quantity regardless of Stats, need no evaluation, and are unaffected.

**The line is what the effect reads, not what it costs.** Anything that resolves against his maximum pools degrades off-world; anything that delivers a stated quantity does not. **This reaches purchased consumables only.** Section 3.2's level-up restoration and a claimed Status Recovery are the System settling his own progression rather than pricing the place he is standing in, and both restore in full on the far side exactly as they do on Earth.

**Say plainly what this does to the Stacks.** At a level-100 Bearer's pool a flat 40 Mana is under two minutes against Section 5.2.1's drain. **Potions do not extend an expedition**, and a Bearer who plans one around drinking his way through it has misread the shop rather than found an exploit.
...
**Premium Rune — roll d10** on Section 11.3's skill table. The rune teaches the rolled technique at the current offer Rank, never below the skill's native E-Rank/D-Rank, and records `instruction_binding: unbound-awakened`. Its identity is fixed at rotation regardless of the Bearer's known skills. An eligible NPC may consume that original rune under Section 7.1. If the Bearer later goes to consume it and already knows the technique at or above the offer Rank, Section 7.1's intact-or-mastery choice applies at that moment — the rotation itself never re-draws the model.
...
- **What it holds.** **Inanimate physical objects** — gear, weapons, armor, consumables, crystals, cores, runes, books, keys, ordinary possessions. It does **not** hold living things (no creature, no person, no plant kept alive) — a creature he wants out of an instance is carried, never stored (Section 17, *Carrying a Creature Out*) — and it is not a bank: **USD/CZK is mundane wallet-and-account money, never a System good** (Section 15, the `/system gear` panel, where cash renders beside gold and never totals with it). System **gold** is likewise not "in" the inventory — it is the shop's internal currency (Section 12.5), shown as a balance, spent only in-window.
...
## 12.11 Re-coring — the ceiling was never meant to be permanent (1.110)

**The fifth trade.** Section 12.8 states that at commissioning *"the commissioner supplies the core — the core's Rank sets the ceiling of the piece's Rank."* **A core sets a ceiling**, and this section applies that rule to a piece that already exists rather than only to one being forged (`F-058`; reasoning in `migrations/1.109_to_1.110.md`).

**Re-coring replaces the core a piece was built around, and the new core sets a new ceiling.** It is the origination rule applied to an existing object. It is **artificer work** — Section 12.8's *"the signature originates; the hands may still work"* is unmoved, and this is the signature's half.

**It raises the ceiling. It does not raise the piece.** Both are paid: the artificer re-cores and the ceiling moves; ordinary Seamwork then carries the piece up to it — `Seat` at B spending material per step, `Raise` at S in one working (Section 7.3) — or a further commission does. **Section 12.8's *"ceilings are why he still shops"* stands**: the way up is still a core.

**Eligibility.** All of the following, checked before any cost is paid:

1. The piece is **functional** — Excellent, Good, Worn, or Damaged. Broken and Destroyed cannot be re-cored.
2. The piece **stands at its current ceiling.** A piece with room left is refused; seat it first. Re-coring is not a way to skip the ladder.
3. The offered core's Rank is **above the piece's current ceiling.** A core at or below it is inert and the working is refused before material is spent.
4. It is **not** a named artifact (Section 11.6), a class-bound item, or instruction of any binding (Section 7.1).
5. It is **not equipped**; re-coring takes the piece out of play for its duration.

**A fused piece is eligible and a re-cored piece may be re-cored again** — Section 12.10's once-ever rule governs fusion, not this. **Each step costs its own core**, which is the whole of what bounds the ladder.

**Cost.**

| | |
|---|---|
| **Material** | **one beast core at the target Rank, or four at the Rank below**, consumed |
| **Fee** | the Section 12.8 commissioning anchor — about the market price of a same-Rank core |
| **Bench time** | one day at C-Rank and below, two at B, three at A, **five at S** |

**The four-at-the-Rank-below clause is deliberate**: banked low-Rank cores are Section 12.5's refused goods with no outlet but an artificer, and this is the outlet.

**Resolution.** One d100 by the artificer against the standard result bands, with Section 12.10's step vocabulary — steps come from established circumstances only, never from balancing:

- **+1** — the offered core came from a creature of the same archetype family as the piece's original.
- **+1** — the commissioner assists holding **Seamwork** at C-Rank or above (Section 7.3).
- **−1** — the artificer has never completed a re-core.
- **−1** — the piece is unprovenanced or was never fully read.
- **−1 per Rank** — the artificer's established working Rank stands below the target.

| Effective | Band | Outcome |
|---|---|---|
| **01–05** | catastrophic | **The piece is destroyed.** Core spent, fee owed. |
| **06–20** | major failure | Core spent; the piece **drops one condition grade** — Damaged becomes **Broken**. |
| **21–40** | failure | Core spent. Piece returned **intact, ceiling unchanged**. |
| **41–60** | partial | The ceiling rises **one Rank only**, however high the core stood. |
| **61–80** | success | The ceiling rises to **the core's Rank**. |
| **81–95** | strong | As success, and the piece's condition rises **one grade**, to a maximum of Excellent. |
| **96–99** | exceptional | As strong, and **the core is not consumed.** |
| **100** | legendary | As exceptional, and the piece is **carried to its new ceiling in the same working** — the Seamwork step above is paid for free. |

**The 01–05 stays live at every modifier**, exactly as it does in a fusion, and a careful artificer says so before taking the commission.

### 12.11.1 The Ascension — what happens at S, and it is not an S+1

**S is the top of the Rank ladder and nothing here moves it.** Section 11.5's tables end at S, Section 6.1's Rank Health ends at S, and **no equipment Rank above S exists** or is authored anywhere in this profile. A piece at an **S ceiling, at S-Rank**, has no higher Rank to be given — so it is given a **name**. Section 11.6 already authors objects outside the ladder, *"each carrying a property ordinary ranked gear does not"* and entering play *"only where a file authors it doing so."* **An Ascension is that authoring, performed at a bench.**

**What it is:**

- The piece **stays S-Rank.** Its stat budget, weapon power, physical reduction and chassis line are **unchanged**, and no number on it rises. Section 11.5 governs it exactly as before.
- It gains **one named effect**, in the vocabulary of Section 6.2, and a **recorded provenance** naming the artificer, the materials, and the working — which is what a Section 11.6 entry is.
- **The artificer names it**, and it carries their mark. A named artificer's work already *"trades above every anchor"* (Section 12.8), and a piece with a mark and no licensed provenance is the widest Section 19 exposure surface an object can have.

**What it costs** — a campaign rather than a purchase:

1. The piece at **S-Rank, at an S ceiling**, functional, not a named artifact already.
2. **An artificer who has completed a re-core before** — the trade's very short list, and a relationship rather than a waitlist.
3. Material at a scale nothing ordinary reaches, at the Section 12.8 anchor, authored per working.

**It is not rolled.** Section 11.6 is explicit that a named unique *"is never generated at random."* An Ascension resolves on an **owner ruling**: the conditions above make the attempt possible, and what the piece becomes and what it is called are authored at that moment, on Section 7.1's ratification discipline. A Runtime reaching one with no authored result **stops and asks**; it never invents a named effect.

**Flux Sight reads the room above the ceiling.** Section 4.4's ceiling read returns whether a piece stands at its ceiling; from 1.110 it also returns whether that ceiling is **re-corable** and at what Rank a core would have to stand.
...
### 13.1.1 What this section never covered — another System's bearer (1.109)

**This section binds NPC *hunters*, and a hunter is a fixed-Rank engine character because the world grades bodies and stops there.** It has never had anything to say about a **System-bearer**, because until `270_THE_FAR_SIDE.md` was authored there was exactly one and he is the protagonist. **Section 6.1 already wrote the rule and wrote it in the singular by accident:** *"The Bearer is the declared exception. His maximum Health is the direct expression of effective Vitality."* That is a statement about what a System does to the body carrying it, not a courtesy extended to one man.

**A System-bearer therefore resolves as a Bearer resolves, whoever they are.** Five Stats, `maximum_health = 4 × effective Vitality`, `maximum_mana = 2 × effective Intelligence`, skills with mastery levels, Stat Passives at their thresholds, and a class where one was taken. **No Rank, no Rank Health, no Rank Mana, and no Section 13.1 ledger entry** — those describe a graded body and a bearer is not one.

**Nothing in the world qualifies.** Awakening is not a System (Section 2), no instrument on Earth has ever read one, and no licensed hunter, guild master, S-Rank, or regulator is or can become a bearer by any route this profile authors. **The population of this subsection is the failed champions of `270_THE_FAR_SIDE.md` Section 4 and nobody else**, which is the same closed population Section 7.1's taught route draws its teachers from — deliberately, because they are the same fact stated twice.

**How a bearer fights a bearer, and it is not the arithmetic he is used to.** Every opponent in this profile's history has been a Rank Health pool that stands still and absorbs, so damage magnitude has been the whole question and the Bearer has long since won it. Against another bearer it is **the least important term**:

- **Section 4.3 governs both sides.** A contested action resolves effective Stat against effective Stat, not against a Rank-derived domain rating. Neither party gets the Rank-comparison shortcut in the paragraph above, and **an action that would auto-succeed against a graded body is simply a roll here.**
- **Skills answer skills.** A bearer's deflection, repositioning, reaction and denial techniques apply at their own mastery against his, in the same exchange, on the ordinary Section 7.3 composition rules. **He has never fought something that could use a skill**, and nothing in Section 6.2 assumed he would.
- **Pools are small on both sides relative to what either can deliver.** Two bearers built past a hundred levels each carry strikes that exceed the other's pool, so **the exchange is decided by whether a strike lands rather than by how many are traded.** A Runtime does not soften that in either direction: the Bearer can end one of these in a single connection, and so can they.
- **What actually threatens him is rarely damage.** A bearer who cannot out-hit him may still be able to deny him a step, a seam, a door, or the Mana he needs to leave — and on the far side, Section 2's drain makes being kept somewhere the lethal outcome without a blow being struck.

**Every such entity is authored, never generated.** `270_THE_FAR_SIDE.md` Section 2's discipline governs in full: a Runtime that meets one not authored there stops and asks the owner, and **no Section 9.7 population formula, d8 archetype, drop table, or anomaly roll reaches any of them.**
...
Signatures are innate and singular: one per awakening, fixed for life, its expression growing only as its owner's measured rank allows. Runes (Section 11) add *learned* techniques on top — rare enough that a hunter with two tricks is notable and a hunter with three is a story. The Bearer is the sole exception to all of it: he manifested nothing at awakening — the notorious blank card of the weakest E-Rank — and everything he gains, the System governs (Section 7). In a world where every hunter is born as exactly one power, the man who can *accumulate* them is a walking impossibility, and hiding that is the whole game (Section 19).

**A signature may carry earned branches (1.121).** Fixity is the default, and it is not an absolute. A branch is a further expression of the same signature, earned in play by diagnosis, real Mana, and real risk of failure, and once earned it stands as permanent law. **A signature may carry any number of them**: no rule fixes a ceiling, because the cost and risk of earning each one are what limit the count. **Branches do not compound**, since each stands on its own mechanism and cost. A branch never adds a magnitude beyond the character's Rank and the three Section 13.5 effects; it changes method or reach, which is all a signature ever does. Ratified at `EVT-001079` for the three branches Standfast carries in the Pendragon campaign. **From 1.124 the Bearer's own way of earning one is Wrought Hand (Section 7.2, *Ratified Earned Technique — Wrought Hand*)**, which needs no consent and resolves as an ordinary uncertain action; every branch it earns stands under this section's limits unchanged.
...
## 13.7 Guild Institutions — Securing Gates and Guild Revenue

Sections 9.10 and 13.2 price the trade's mechanics in detail — postings, splits, harvest shares — but never price the institution that sits on top of a freelance crew and calls itself a guild. Ironline, Horizon, Karlov, the Coalition, and (from `gatefall_pendragon_001` onward) Pendragon are all named across this world's existing text without a single number attached to how any of them actually runs. This section closes that gap. It governs **any** guild-shaped institution generically; nothing here is authored to Pendragon specifically, and a Runtime applies it to Ironline, Horizon, or Karlov exactly as written the moment their own economics matter on-screen.

### 13.7.1 The Three Boards

Section 9.10's own list of posting channels — *"the BGM ranking board, a co-op or guild dispatch posting, an auction lot"* — already names three separate institutions without distinguishing them. This section makes the distinction explicit:

- **The BGM ranking board** (the CGA's equivalent in Prague, and each jurisdiction's own regulator elsewhere) is run by the regulator itself. BGM sets the posting floor, the legal minimum (Section 13.3), and the deadline (Section 9.3); it witnesses and files every contract (Section 13.4) through its own regional desk. **BGM does not employ hunters, run crews, or choose a favored bidder.** A posting sits `posted` until some signed contract meets its floor, at which point it becomes `staffed` (Section 9.10) — first qualifier wins, never best pitch.
- **The Coalition's co-op desk** (Windy City Hunters Coalition, Kesha Morrison's desk in Chicago) is a labor-side broker for unaffiliated freelancers — not a regulator and not a guild — *"where an unaffiliated freelancer registers when Horizon and Ironline will not have him"* (130_NPCS_AND_FACTIONS.md). It runs its own board of Gates it has itself secured access to, posts them for its registered freelance members to pick up individually, and files the resulting contracts with BGM for legal compliance like anyone else. It pays no salary and fields no roster of its own.
- **Guild dispatch** is a guild's internal distribution of work it already holds — a standing BGM-issued extraction permit (the pattern already on record for Ada Reyes's contractor at Cicero Scar) or a direct corporate contract — routed straight to the guild's own roster without ever touching either board. This is a guild's steadiest and highest-margin channel precisely because it isn't contested.

**Winning a BGM-board posting is never a purchase.** A guild competing for an open posting is bidding to be *awarded* the contract — selected as the crew, then paid the contract value or granted the yield — never paying BGM to be chosen. Section 9.1 step 4's per-day take rates (E 15%, D 25%, C 55%, B 75%, A 90%) describe the anonymous aggregate; when the fiction needs to know whether a **specific** guild is the one that wins a contested posting rather than a rival, roll the same d100 with:

| Modifier | To the roll |
|---|---|
| The guild carries a licensed bench at the Gate's Rank | −15 |
| The guild has a standing office or chapter covering the posting's district | −10 |
| A rival guild already holds a head start (an existing arrangement, an earlier bid) on that lot | +20 |
| The guild bids at or above the floor specifically to build reputation, eating the spread itself | −10 |

This never changes *whether* a posting gets taken (Section 9.10's deadline logic is untouched) — only which guild crews it, which is the only question a guild's own business plan actually needs answered.

### 13.7.2 Guild Contract Terms

Where a guild fields the crew on a contested posting won under 13.7.1, it stands in Section 13.2's **leader** position: the same **10% off the top** any independent leader takes, no new number there. On top of that, a guild takes a **house cut of 15% of the remaining 90%** before the equal split — for dispatch, liability coverage, and gear backing an independent leader doesn't provide. A rank-and-file crew member nets **76.5%** of the pool split among them; the guild nets **23.5%**. Section 13.2's harvest terms (20% of the pool off the top, output-weighted) are unchanged for a **mixed** crew — independently-contracted harvesters working alongside a guild's salaried strikers still draw their harvest share as written. The house cut applies only to the combat pool a guild crew actually fields.

### 13.7.3 Salaried Rosters

Where a guild employs a hunter outright rather than fielding them on a split contract, no pool is divided at all — the guild pays a flat annual salary and keeps everything the hunter's clears produce, support work included (see the worked example below for what that means for a guild's own harvest crew).

**Freelance annual income, derived, not assumed**, at Section 13.2's standard split and Section 13.3's legal-minimum crew size:

| Rank | Per-clear pool (mid, Section 12.3) | Crew size | Non-leader share/clear | Realized cadence/yr | Freelance avg/yr |
|---|---|---|---|---|---|
| E | $3,500 | 8 (Section 9.1 step 4's own "insulting split eight ways") | $394 | ~30 (uncontested; harvest-income basis, Section 9.4) | ~$55,000 |
| D | $11,500 | 8 (the same E/D posting floor) | $1,294 | ~30 (same harvest basis as E) | ~$85,000 |
| C | $45,000 | 4 (legal minimum) | $10,125 | ~15 (contested at 55%/day) | ~$152,000 |
| B | $185,000 | 8 (legal minimum) | $20,813 | ~11.5 (contested at 75%/day) | ~$240,000 |
| A | — | — | — | rare, national-asset scale | negotiated, no band |

**E and D don't run their own contracts in practice** — Section 9.4 already states most E/D-Rank hunters make their living as harvest-hire miners on higher-Rank clears, exactly as the Cicero Scar worked example shows an E-carded hunter earning off output share (Section 13.2). Their freelance-average figures above are a harvest-income estimate, not a derivation from an E/D contract band the way C and B are — the weakest legs in this table, stated as such rather than dressed up as citations. **The D-Rank per-clear column was blank through 1.121 and is filled at 1.122** at the midpoint of Section 12.3's own authored `$8,000-15,000` band, which is the identical arithmetic the E, C and B rows were always derived by; it was a gap in this table rather than a gap in the economy, and filling it invents nothing. **The freelance-average column is unchanged and stays a harvest estimate** - an E/D hunter still does not make a living off E/D contracts. **B-Rank's realized cadence is backed out from its own existing salary figure** (superseded 12.3 row): $240,000 ÷ $20,813 ≈ 11.5 clears/year is the cadence a contested Rank actually supports, which is the number this table now shows its work for.

**Guild salary, priced as a real premium over the freelance average, not a floor beneath it:**

| Rank | Freelance avg/yr | Guild salary/yr | Premium |
|---|---|---|---|
| E | ~$55,000 | $60,000 | +9% |
| D | ~$85,000 | $95,000 | +12% |
| C | ~$152,000 | $175,000 | +15% |
| B | ~$240,000 | $300,000 | +25% |
| A | negotiated | negotiated, guild-backed — institutional standing (regulator relationships, national-asset contract history) gives a guild's own negotiation more leverage than a solo hunter's, no fixed number on either side | — |

**A guild's own support crew on a fully guild-staffed clear are salaried too** — this is what closes the Section 13.2 harvest gap. They draw the E/D salary row above instead of the output-weighted 20% harvest share; the guild keeps that share along with the 80% combat pool, because nobody on the clear is owed a split. A mixed crew (guild strikers plus independently-contracted harvesters, or the reverse) still runs the unmodified Section 13.2 split for whichever side isn't salaried.

**Worked example — what a guild actually pays and profits per Gate.** Take a dedicated B-Rank team: 8 salaried strikers at $300,000/yr plus a 5-person salaried harvest crew at the E-Rank rate ($60,000/yr, matching the Cicero Scar crew's own size). Annual crew cost: **$2,700,000**. A guild's institutional standing (13.7.1's bid edge) lets a dedicated team clear at roughly **1.25× an individual freelancer's realized cadence** — about **14 clears/year** at B-Rank, well short of the uncontested ceiling — and a guild-backed contract commands a **15% pricing premium** over the freelance band for the reliability a client is buying, putting the pool at **$212,750/clear**. Revenue: **$2,978,500/yr**. **Profit: ≈$278,500/yr, a ~9.3% margin** — real, modest, not a windfall, and dependent on the bid-edge modifier actually landing; a crew run at ordinary uncontested-freelancer cadence loses money on salary alone. The same arithmetic at C-Rank (4 strikers @ $175,000 + 3 harvesters @ $60,000 = $880,000/yr cost, ~19 clears/yr, $51,750/clear pool) profits **≈$103,000/yr, ~10.5% margin**.

**This worked example is superseded in part at 1.122, and the part is its cadence (Section 13.7.5).** Its 1.25x bid-edge multiplier, its ~14 clears a year at B-Rank and the ~9.3% and ~10.5% margins that follow are all still correct **for a guild whose crews can only reach their own metro** - which was every guild in this world when this example was written, and is still most of them. They do **not** describe a guild holding a zero-latency transport network between its own offices: that guild's catchment is its regulator's whole region rather than one metro, its B-Rank crews clear nearer 90 a year than 14, and its margin is several times the figure above because the cost side - salary - did not move with the reach. **Everything else in 13.7.3 stands untouched**: the splits, the house cut, the salary table, the salaried-support rule that closes the harvest gap, and the refusal to price A/S.

**A/S-Rank clears carry no derivable number** — negotiated at national-asset scale (Section 9.4, Section 12.3) — and are almost certainly where a guild's real profit concentrates, but nothing here prices that; a Runtime with a specific A/S contract on-screen prices it in fiction against Section 9.11's directed-assignment channel, not against a table.

### 13.7.4 Guild-Owned Appraisal and Resale Margin

Section 12.1/12.8 price crystal and core at a flat licensed market rate with no distinction for who's selling. A guild running its own appraisal desk buys its own crews' declared haul at licensed market price immediately, on exit, then resells the accumulated lot at auction on its own schedule — capturing whatever spread the auction clears above licensed rate, rather than the single flat sale an independent crew settles for. **No fixed percentage is authored here** — the spread is whatever a given auction lot actually produces, on the same footing as any other Section 12 crystal/core sale; a Runtime with a specific lot on-screen prices the spread from that lot's own numbers, not from an invented margin.

### 13.7.5 Standing Branch Operations — what a guild's offices clear without its founder

Sections 13.7.1 to 13.7.4 price a guild's **margins** and never once its **throughput**. Nothing above says how many Gates an institution actually answers on an ordinary Tuesday, and while a guild is one bench in one metro that costs nothing — Section 9.1's tick rolls its city, Section 9.10's board carries its postings, and every Gate it takes is a line in a day already being narrated.

**It stops costing nothing the moment a guild holds a dozen offices, a four-figure roster, and a transport network.** At that scale every Gate the institution answers still has to arrive one at a time through the tick or through Section 9.11's channel, and the fiction can no longer afford its own world: a guild whose Prague office spent a whole Event clearing one D-Rank Gate is a guild that can never be shown working. The gap is the same class as Section 9.1's own — *an un-run world is a readiness finding, not something nobody notices* — one level up, at the institution rather than the city.

**This governs any guild-shaped institution**, on Section 13.7's own terms. Ironline, Horizon, Karlov and the Coalition's co-op desk run everything below at their own bench depth exactly as a new entrant does, and nothing here is authored to whoever's growth first needed it.

#### When a branch stops being a scene and becomes a rate

A branch is **`standing up`**, and its Gates are narrated individually, until all four of these hold:

1. **A named officer who dispatches.** A lease and a sign are premises, not a branch.
2. **Round-the-clock cover** — a night officer and the branch's own custodial staff — so a 03:00 detection reaches a person rather than a voicemail.
3. **A bench it can legally crew from** at the Ranks it intends to dispatch. Section 9.4's minimums are the whole test: a guild that cannot field four C-capable hunters does not dispatch to C-Rank Gates, however many members it has.
4. **A filed standing relationship with the jurisdiction's regulator** — the BGM regional office, the CGA, the national equivalent. Guild dispatch (13.7.1) is the distribution of work the guild *already holds*, and holding it requires a permit somebody issued.

**On the day the fourth lands the branch becomes `operational`, and from the next day boundary it runs as a rate.** This is the same promotion bar Section 9.1.6 sets for a metro and it is deliberately one-way: below it nothing is automatic, and above it nothing is narrated Gate by Gate except what this section says surfaces.

#### The catchment is a region, and response time is the only reason

An operational branch's catchment is its **regulator's whole jurisdiction** — the BGM numbered region in the United States, the national Gate authority's territory abroad — and not the metro its lease sits in.

**What fences a bench is Section 9.3's clock, nothing else.** A Gate four hundred miles away is not a Gate anyone can answer inside its timer, which is why Section 9.1.6's `89-00` band exists at all and why its own note reads *"thin coverage IS why it escalated."* A guild holding a **zero-latency transport network between its own offices** has had that fence removed, and its catchment expands to exactly what the network reaches. **A guild without one reads its own metro's band and stops** — the region is bought by the infrastructure, not granted by the charter.

**So the ground this opens is not taken from a rival.** It is Section 9.1.6's Band IV and off-roster flow — ≈65 Gates a day across the United States at tier VII — which breaks today for the single reason that nothing licensed is inside its timer. `200_WORLD_BIBLE.md` Section 5's *"the map of who dies to Gates tracks the map of who was already poor"* is a statement about precisely that ground. A guild that reaches it is answering the world's own standing indictment, not out-bidding Ironline for the Loop.

#### The daily dispatch

**One roll per operational branch per day, inside Section 9.1's step 4.** It produces **counts, not Gates**: nothing here mints a `GB-` posting, a per-Gate record, a break timer, a confidence band or an anomaly roll, and a branch's day is a number in the tick's ledger rather than a set of objects.

```text
     E-Rank     1d4 + 1      every operational branch, every day, no exception
     D-Rank     1d3          every operational branch, every day, no exception
     C-Rank     1d3 - 1      0-2, scattered across the branches
     B-Rank     1d2 - 1      0-1, scattered across the branches
     A-Rank     ---          never resolved here; it surfaces (below)
     S-Rank     ---          never resolved here; it surfaces (below)
```

**E and D carry a floor and C and B do not, and that is the distribution rather than a preference.** Section 9.1 step 4's take rates are **15% and 25% a day** at the bottom of the ladder against **55% and 75%** in the middle: E and D *sit*, and Section 9.4 says why — *"cheap jobs sit, and thin crews take them."* A freelancer splitting $3,500 eight ways cannot afford an E-Rank Gate, which is exactly why `GB-01` and `GB-04` both sat unbid to their deadlines and broke. **A salaried roster can afford it** — 13.7.3's own rule is that no pool is divided at all — and it is the only structure in this world that can. So the bottom of the ladder is work a guild takes almost all of, every day, everywhere it has an office, and the contested middle is work it bids for and mostly does not get.

Worked against a fourteen-branch institution at tier VII, whose catchments carry ≈210 Gates a day between them:

| Rank | Per branch/day | All branches/day | Catchment flow/day | Share taken |
|---|---:|---:|---:|---:|
| **E** | 3.5 | ≈49 | 69 | **71%** |
| **D** | 2 | ≈28 | 46 | **61%** |
| **C** | 1 | ≈14 | 42 | **33%** |
...
**Rivals keep two thirds of the contested middle and all of the top, and that is why they tolerate this.** A guild taking 71% of a region's E-Rank work has taken 71% of the work nobody was bidding on.

#### The bench is the limit, and the table scales to it

The table above is authored **for a guild carrying ≥125 hunters per operational branch.** Between 60 and 125 a branch halves every line; below 60 it dispatches **E and D only**, whatever its regulator has licensed it for.

**The test is guild-wide rather than per-branch, and the transport network is why.** A branch's own local signings stop being the thing that staffs it the moment a crew can be in any of the guild's cities in the time it takes to walk through a door — which is the same fact that bought the catchment above, read from the inside.

**Duty rate is tracked, not rolled.** Section 9.4's posting floors give each clear its headcount — **8** at E/D, **11** at C, **16** at B — and the day's dispatch divided by the roster is the rate:

| Duty rate | Band | What it means |
|---|---|---|
| under 50% | **Normal** | The bench absorbs it; nothing visible changes. |
| 50–70% | **Strained** | Branches decline marginal work. An officer says no to something, and somebody notices. |
| over 70% | **Overcommitted** | A branch must refuse Gates **or file a roster it cannot staff** — and the second is Ironline's own sin exactly (`EVT-000345`, `EVT-000384`): a prosecutable falsified filing, and a matter for the guild's own board. |

*(Worked: 98 clears at those floors is ≈882 hunter-slots against an 1,812 roster — **48.7%**, Normal, and one bad week from Strained. **A guild's recruiting is therefore never finished**; it is what buys the next rung of throughput rather than a phase that closes.)*

#### A and S never resolve in the background, and this is a fence

**No background pass resolves an A-Rank or S-Rank Gate — not at any bench depth, not with any roster, not ever.** An A or an S in an operational branch's catchment **surfaces**: through Section 9.11's directed-assignment channel, through the branch officer's own call to whoever the guild has put above them, or as news the guild hears late and answers for. **Which of them becomes a scene is Section 9.1.8's**, and that section is what supplies them: 13.7.5 promised surfacing and authored no source, because a branch's catchment had no A/S generator until 1.123. **An S-Rank always surfaces and carries no daily budget. At most one A-Rank becomes a scene on an ordinary day**, the rest resolving as counts under the standing posture below.

Three reasons, and none of them is pacing:

1. **Section 9.11 already says the top of the ladder does not reach a hunter through a board.** A 90%-per-day take rate means an A-Rank lot is gone before a freelancer sees it and S-Rank never posts at all; Section 9.11 is the channel those two facts imply. A background pass that cleared A and S would route the top of the ladder through the exact mechanism Section 9.11 exists to say it does not use.
2. **Section 13.7.3 already declined to price them.** *"A/S-Rank clears carry no derivable number — negotiated at national-asset scale"*, and a Runtime with one on-screen prices it in fiction against Section 9.11 rather than against a table. A count rolled here would be the invention 13.7.3 refused, arriving by a side door.
3. **It is the only work left that can still cost the institution something.** A guild whose background quietly cleared its own A- and S-Rank Gates would have automated away the only Gates on the board that can kill its people, lose it a district, or make it answer to a regulator — and an institution that cannot lose is scenery.

**A guild's A-Rank pool answers on a standing posture, declared once (1.123).** Where a guild fields A-Rank hunters outside any branch's roster they are a **response element**, and a guild may declare which catchments that element answers in and under what conditions. **The declaration is the on-screen decision; the work it then absorbs resolves as counts**, on this section's own terms — revenue composes, casualty exposure applies, and the duty rate is charged. Section 9.1.8 holds the capacity arithmetic and the one-scene-a-day budget. **Through 1.122 this paragraph required a fresh on-screen commitment every time**, which is correct for a guild answering an A-Rank Gate a week and unrunnable for one whose regions open twenty a day.

#### What the volume costs, and the call that can be answered

**A day of this work is dangerous in aggregate even when every crew is properly staffed.** Section 9.4 is explicit that *"a support crewman caught when a line breaks dies exactly as a striker would"*, and ninety-eight clears a day is a great many lines that can bend.

**But a guild running a transport network is the first institution this world has had that can put help on the ground inside a fight**, and a rule that priced attrition as an unanswerable dice roll would waste the one thing that infrastructure is for. **So attrition is a call before it is a casualty.**

1. **One d100 per operational branch per day.** On **`00`**, that branch's day produced a **crew in distress** — a line bent, a population read one Rank wrong, a break arriving early. **It is not a death and is never narrated as one.**
2. **It surfaces at once, and it is answerable.** The branch officer calls. Anyone the guild can put through a tunnel — the A-Rank pool, another branch's crew, the guildmaster himself — arrives in the time it takes to walk through a door. **This is a scene with a real clock, and the clock is the point.**
3. **Answered in time, nobody dies.** Injuries settle on Section 6.3's tiers and the crew comes home.
4. **Unanswered** — nobody was committed, or the answer arrived after the clock — **roll d100. On `95-00` the guild buries someone.** Otherwise the crew takes a permanent Section 6.3 injury and lives.

**And a hard ceiling the dice may not beat: a guild buries no more than one member per 150 days.** A qualifying fatal result inside that window settles as a permanent Section 6.3 injury instead. This is a **stated cap, not a probability** — a background rule that produced a funeral a week would be measuring the volume rather than the work, and would make the institution unplayable for the person who built it.

**The ceiling does not protect a branch that dispatched under Section 9.4's floor.** A guild that filed a roster it could not staff has committed the trade's one unforgivable act, and the cap is **lifted for that branch** until its own board has answered for it. A safety net that also caught the guild's negligence would teach precisely the wrong lesson, and this world already has Ironline to show what that looks like.

#### The revenue, composed rather than newly priced

**No new price is authored here.** A background day's gross is Section 12.3's per-clear band **at its midpoint**, times the counts above, times 13.7.3's **15% guild-backed premium** — and 13.7.3's salaried-roster rule keeps all of it, since no pool is split and the harvest share stays with the guild:

| Rank | Section 12.3 band | Midpoint | Clears/day | Gross/day |
|---|---|---:|---:|---:|
| **E** | $2,000–5,000 | $3,500 | 49 | $171,500 |
| **D** | $8,000–15,000 | **$11,500** | 28 | $322,000 |
| **C** | $30,000–60,000 | $45,000 | 14 | $630,000 |
| **B** | $130,000–240,000 | $185,000 | 7 | $1,295,000 |
| | | | | **$2,418,500 × 1.15 ≈ $2,781,000** |

Against Section 13.7.3's salary table on an 1,812 roster — E 635, D 508, C 360, B 223 — **≈$216,260,000 a year**, plus an A-Rank pool at no fixed number, and before every institutional cost this profile does not price: leases, the headquarters, liability and hunter insurance, the appraisal desk, legal, and the regulator filings each branch owes. **That is the same basis 13.7.3's own worked example used** — crew salary and nothing else — and it is stated rather than quietly widened.

**A foreign branch reads its own jurisdiction's factor.** Prague's ≈70% (Sections 12.1, 12.3) is **Prague's**; this profile authors no factor for any other foreign market, and a Runtime needing one takes an owner ruling rather than generalizing Prague's number across a continent.

#### What this supersedes in Section 13.7.3

**13.7.3's worked example is superseded in part, and the part is its cadence, not its method.** That example prices a dedicated B-Rank team at **≈14 clears a year** and **≈9.3% margin**, on a bid-edge multiplier of **1.25×** an individual freelancer's realized cadence. Every one of those figures remains correct **for a guild whose crews can only reach their own metro** — which was every guild in this world when it was written, and is still most of them.

**It no longer describes a guild holding a zero-latency transport network.** A crew whose catchment is a region rather than a metro is not flow-limited at 14 clears a year; the ≈98 clears a day above put a B-Rank crew nearer **90 a year**, and the margin that follows is several times 13.7.3's because the cost side — salary — did not move with it. **What changed is reach, and only reach.** 13.7.3's splits, house cut, salary table, salaried-support rule and A/S refusal all stand untouched, and its cadence figures stand untouched for every guild without the network.

#### What it does to the world, which is the point of running it

**Every Gate a branch clears is a break that does not happen**, and this is the first mechanism this profile has ever had that makes a city's situation improve at scale.

- **Section 9.1.3's district pressure falls.** A district the guild is working stops feeding the `+1`, `+2` and `+3` lines and starts drawing the `-1` a clear week pays. Strained districts settle, cordons come down, and a district on the edge of containment failure stops producing a Scar.
- **Section 9.2.2's `escalation_credit` stops accruing where the guild is.** Chicago's authored ~1.1 breaks a day is ~1.2 credit a day; a branch clearing the bottom of that city's ladder takes most of it off the table.

**Two fences, and they are not negotiable.**

1. **The tier still only ever rises.** Section 9.2.2 says there is no downward branch and none is added here. A guild can slow the ramp's pressure route to nothing; it cannot reverse a rung, and the schedule's clock runs regardless of how well anyone's offices are doing.
2. **Only districts the campaign actually tracks are affected.** This is Section 9.1.6's own rule read the other way round: a break in a metro nobody is simulating may not advance the tier, so a *clear* in one may not lower a pressure value either. Outside the simulated cities the effect is a **world fact** - the trade notices that a region's break count has fallen - and nothing more.

**And the world outside the footprint keeps getting worse on schedule**, which is the shape this produces: an institution whose own regions are visibly holding while everywhere it has not reached is not. That gap is the argument for the next office, and it is also the argument every regulator, rival and government will now be making at the guild rather than to it.

#### What is written, and what mints nothing

**Section 9.1.1 governs, and `none` is written as a result.** The tick's ledger entry carries one line for this section:

```text
guild_ops    per guild holding operational branches: the day's dispatched count
             by Rank, the duty-rate band, any distress call and how it resolved,
             and any A/S surfaced rather than cleared -- or `none` where no
             guild holds an operational branch
```

**A background clear mints nothing.** No `EVT-`, no `ENT-`, no `REC-`, no board posting, no identifier of any kind, and no entry in the campaign's chronicle — it is an aggregate, and Section 20.1's montage compression is the principle it runs on, applied to an institution rather than to a span of days. **Three results break that and become ordinary canon:** an **A or S surfaced**, a **distress call**, and a **death**. Each is a scene, is settled on-screen, and mints exactly what any other scene of its kind mints.

**And this pays the Bearer nothing.** Section 9.1.5's own boundary holds: the System measures the Bearer, and a Gate cleared by people he employs in a city he is not in is not something he did. **No XP, no skill use, no mastery counter, no title progress, no loot** — only money, standing, and the world being a slightly better place to live in than it was yesterday.
...
**A panel may serve its complete listing from the campaign's derived ledger (1.120).** Where a campaign declares a derived ledger for a surface in its own startup file, that panel renders its **summary and totals inline** and serves the **per-item listing** as the ledger's link rather than restating every row inside the frame. The reason is measured rather than stylistic: `/system gear` is this campaign's largest declared operation at roughly 26,800 tokens against a 26,451 ratchet, and a panel whose source cannot be read in one pass renders *partially* — a half-filled frame the player cannot tell from a complete one, which is the Section 20.2 defect this replaces. A listing served as a link is either there or plainly absent.

**Nothing interactive moves, and nothing is lost.** Equipping, unequipping, withdrawing, buying, selling and title changes all still resolve through the panel exactly as authored below; the ledger is a view of the same canon the panel reads, regenerated from it. A campaign that declares no ledger renders every panel in full, unchanged — this clause is inert without a declaration.

### 15.3.1 `/system skills`

**Superseded 2026-09-06, owner ruling, Profile 1.113.** Through 1.112 skills rendered in three mechanical groups — `ACTIVE` (carries a Mana cost), `PASSIVE` (no cost, not a Section 4.4 threshold passive), `STAT PASSIVE` (every Section 4.4 threshold passive). The owner ruled this replaced rather than supplemented: the panel now groups by **combat category** first and **acquisition source** second, with every skill's effect on its own indented line beneath its header row rather than sharing a row with its stats.

**Served from the ledger where the campaign declares one (1.120).** The panel renders the header, the per-category counts, and the ledger's link; the **per-skill rows are the ledger's**, not the frame's. The categories, their fixed order, and the classification rule below still govern — they are what the ledger groups by, and the counts the panel states are counts of those same groups. A campaign with no declared ledger renders every row inline exactly as authored here.

**Category, in this fixed order — Offense, Defense, Utility.** Classify by the skill's primary authored effect, never by name or flavor:

- **Offense** — deals damage, disables or restrains an opponent, or directly increases the Bearer's or an ally's offensive output against a target.
- **Defense** — reduces or prevents harm to the Bearer or an ally, restores Health, or grants immunity/resistance to a harmful effect (including compulsion, fear, or forced action).
- **Utility** — everything else: movement, detection, information, crafting, non-combat support, and any effect that fits neither category above.

A skill with a genuinely dual effect is classified by whichever half is load-bearing to why it was taken; a Runtime that cannot decide states the ambiguity in the render rather than silently picking one.

**Source, within each category, in this fixed order** — Native/Class first, then each taught route in the order it was first taught, then each Succession victim in the order killed. A source subheading renders even for a single skill under it; it never collapses into the category header.
...
╟──  OFFENSE  ────────────────────────────────────────────────────  <n> known  ╢
     ...
     <name>                    [<Rank>]   <★ string> <level or "Master">   Mana <n or "Passive" or "— (Class ability)">
         <effect, on its own indented line, wrapped as needed>
         DMG <n> standard · before reduction (only if the skill deals damage)

╟──  DEFENSE  ────────────────────────────────────────────────────  <n> known  ╢

     [same source-grouped, name/effect-on-its-own-line shape as OFFENSE]
...
**A Section 4.4 Stat Passive** renders `[<Rank>, Stat Passive]` in place of the mastery/Mana cell — no stars, no Mana, no NEXT column — with its effect on the indented line beneath exactly like every other entry. Its driving Stat, current value, use count, and next-rung threshold are reached through `/system skills <name>`, not shown here.

**A class ability** renders `[Class ability — <class>]` in the same position, Mana cost stated inline if it has one, and its full effect on the indented line(s) beneath — no more compressed than a native active skill.

**Mastery still renders as the five-cell star string of Section 7.4** followed by its level name, or `Master` bare where a skill entered at Master via Succession and the star string would be redundant. `NEXT` (qualifying-scene progress toward the next mastery level) and cumulative successful-use counts are dropped from this panel's row shape entirely — both remain reachable through `/system skills <name>` — because the row's second line is now spent on the actual effect instead.

**A skill's `succession`/`taught` provenance is what fixes its subheading**, read from `acquisition_route` and `taken_from`/the teaching NPC on its ledger entry — never guessed from its name or Rank. Each group renders even if a category or source subheading would hold only one skill; an empty category still renders its header and `none` beneath it, the same discipline every other panel in this profile already follows.

Ren at level 13, under the 1.113 grouping:
...
╟──  OFFENSE  ────────────────────────────────────────────────────  1 known  ╢
     ...
     Mana Bolt             [E-Rank]   ★★★★☆ Expert   Mana 4
...
╟──  UTILITY  ─────────────────────────────────────────────────────  4 known  ╢
     ...
     Sprint                [E-Rank]   ★★☆☆☆ Practiced   Mana 3
         +1 step on movement and pursuit, covering two exchanges.

     Flash Step            [D-Rank]   ★★★☆☆ Adept   Mana 6
         Closes or breaks line of sight; +1 step for three exchanges.

     Dagger Mastery        [E-Rank]   ★★★★☆ Expert   Passive
         Adds +0.25 to a Quickknife chassis, ×0.75 → ×1.00.

     Flux Sight            [D-Rank, Stat Passive]
         Reads a perceived monster's Rank, Health, and Mana.

     Pre-empt              [D-Rank, Stat Passive]
         Acts normally in a surprise exchange on an ambush warning.

╚═════════════════════════════════════════════════════════════════════════════
```

Note Pre-empt's shift out of a mechanical grouping and into Utility here: at this early Rank its whole authored effect is *not losing your action to a surprise*, which fits no offensive or protective category cleanly — a Runtime judging the ambiguity clause above would file it under Utility rather than force it into Defense on vibes alone. The Bearer's own S-Rank Pre-empt (First Blade, resolving before any opponent at any Rank) is a live offensive edge and renders under Offense instead — same skill name, different Rank-driven effect, different category, because the rule classifies the effect and not the label.
...
**`STORED` is served from the ledger where the campaign declares one (1.120).** The panel renders `WORN`, `WORN · TOTALS`, and each stored kind's **live line count**, then the ledger's link in place of the per-item rows. `WORN` never moves: it is the interaction surface, it is nine lines rather than a hundred, and equipping, unequipping and withdrawing all still resolve here. Withdrawal names an item the player read in the ledger, which reads the same canon this panel does. The five kinds below still define where a holding sits — they are the ledger's groups too.
...
**The Bearer holds one equipped slot per assessment he has passed, to a maximum of five through level 50** (Section 16.1) — one at level 10, two at 20, three at 30, four at 40, five at 50 — **and then a sixth at 60, a seventh at 80 and an eighth at 100** under Section 16.4. A title in a slot has its passive active; every earned title outside one is dormant. Slots are derived from `titles.last_assessment`, never stored separately, and a slot may be left empty.
...
|---|---|---|---|
| **Common** | 12 | **+2** | one small percentage effect, or one small resource effect |
| **Rare** | 7 | **+5** | a step that applies broadly, a damage or pool multiplier, or a real informational edge |
| **Singular** | 3 | **+10** | a capability the profile grants nowhere else. Most Bearers die never holding one. |

**A passive stated as a modifier step composes against Section 4.3's own math, and that math already spends most of the budget before any title touches it.** A Bearer whose acting Stat clears the opposing domain rating by 20 or more takes the stat-margin contribution's capped **+2** automatically — true of essentially every roll once a Bearer's base Stats pass roughly 80 — leaving exactly **one** step of headroom inside the net ±3 cap for every skill, title, and circumstance to share. Five Section 7 skill passives (Broken Rhythm, Exploit Pattern, Silent Step, Sprint, Keen Sense) already compete for that single step in their own domains. **Every entry below whose passive was pure step is rebased (Version 1.101) to pay out in a resource change, a percentage reduction or bonus, or a stated capability instead** — a currency that shares no budget with the step cap and never collides with a skill or another title. Earning criterion, grade, and point grant are untouched throughout. Titles whose passive was never a step (Red Survivor, Centurion, Prospector, Ascetic, Deathless, Executioner, Wellspring, Unbroken) are untouched below.

### Common titles

| Title | Earned when the record holds… | Passive (while equipped) |
|---|---|---|
| **Loner** | at least one Gate cleared alone — its boss killed with no other hunter inside. Section 17's instant dungeons and Section 8.3's penalty zones count: both are real clears. | **incoming damage is reduced 10% while no allied hunter is with him, without regard to whether the ground is a Gate** (widened at Version 1.91; rebased from a modifier step at 1.101). An *allied hunter* is another awakened working with him, not any nearby civilian. Widened on an owner ruling raised mid-session against `ENT-000217`, an anomaly that is expressly not a Gate; a title proving solitude inside a Gate has no textual reason to go inert the instant the ground under it stops qualifying as one. |
...
| **Untouchable** | at least one Gate cleared in which the Bearer took no damage at all | the first hit that would land against him in any combat deals **50% reduced damage** (rebased at 1.101 from a step on first-exchange evasion, which Broken Rhythm already covered against any telegraphed attack) |
...
| **Break-Breaker** | a dungeon break (Section 9.8) stopped before it cleared the cordon, the Bearer part of stopping it | civilians and any position he is defending take **15% reduced damage** from a dungeon break's monsters while he is present (rebased at 1.101) |
| **Underdog** | a creature killed while it stood **two or more Ranks above** the Bearer's System Rank — a red designation (Section 14.6) — at the moment of the kill | the Bearer's first action against each orange- or red-designated creature in a combat deals **+15% damage** (rebased at 1.101) |
| **Diehard** | Health reduced to 0 in a resolved exchange the Bearer survived (Section 6.5) | while Health stands at or below one quarter of its maximum, he takes **15% reduced damage** from all sources (rebased at 1.101) |
| **Kingmaker** | a boss killed by another hunter's hand on a weakness or opening the Bearer resolved and relayed, the clear credited to him under Section 3.8 | the first action an allied hunter takes against a target whose weakness the Bearer has relayed this scene deals **+15% damage** (rebased at 1.101) |
...
| **Penitent** | a penalty zone (Section 8.3) cleared — its boss killed before the four hours ran out | Mana cost of all actions inside a sealed instance the Bearer did not open is reduced **20%** (rebased at 1.101) |
...
| **Giantslayer** | a boss of a Rank **above** the Bearer's System Rank killed by his own hand, alone, with no other hunter inside | all actions against a boss deal **+15% damage** (rebased at 1.101) |

### Rare titles

| Title | Earned when the record holds… | Passive (while equipped) |
|---|---|---|
| **Cartographer** | at least one Gate of **every archetype** on the Section 10 d8 cleared and credited — all eight interiors walked out of | a Gate's archetype is known to the Bearer the moment he enters, without a read, a survey, or a confirmation; and every action that exploits or evades the archetype's twist deals **+15% damage**, or resolves at **+15% effect** where it is not an attack (rebased at 1.101) |
...
| **Warden** | **ten** Gate clears in which no hunter who entered alongside him took a classified injury | the Bearer takes **15% reduced damage** on all defensive actions, and every allied hunter sharing the Gate takes **15% reduced damage** on their first defensive action each scene (rebased at 1.101) |
| **Vanguard** | **ten Gates entered alone and first**, ahead of the party he arrived with, each of them cleared | the Bearer's first action in any combat he opened deals **+30% damage**, or resolves at **+30% effect** where it is not an attack (rebased at 1.101 from a two-step grant no roll ever had two steps of headroom to pay) |
...
| **Grounded** | at least one boss kill credited inside a Gate, **and** at least one kill credited entirely outside any Gate, sealed instance, or instant dungeon of a creature standing at or above his own System Rank at the moment of the kill — the same danger meeting him wherever he stands | **+1 modifier step on all actions, without regard to whether the ground is a Gate** (added at 1.101 — the one entry in this catalog whose whole reward is deliberately kept as a step; see the Compatibility Status note on why the rest were moved off it) |

### Singular titles

| Title | Earned when the record holds… | Passive (while equipped) |
|---|---|---|
| **Godslayer** | an **S-Rank boss** dead, its kill or its clear credited to the Bearer (Section 3.3, Section 3.8) | all damage dealt to a boss is increased **10%**, and a boss kill grants **double** Kill XP (Section 3.3) (replaced at 1.101 — the prior Health-and-colour read is fully subsumed by Flux Sight's own S-Rank rung and Section 14.5.4's automatic colour render, so it stated no capability the profile grants nowhere else) |
...
| **The Unmeasured** | a carded Rank standing **three or more Ranks below** his System Rank at the moment an assessment runs, never re-assessed (Section 19) | Flux Sight reads one Rank above its Section 4.4 ceiling, and any action taken to conceal his true capability (Section 19.1) **succeeds automatically unless a specific, named System capability is brought to bear against it directly** (rebased at 1.101 from a two-step grant no roll ever had two steps of headroom to pay) |
...
**Only an equipped title's passive applies.** Every passive is stated in modifier-step, percentage, resource, or stated-capability terms (Sections 4.3, 5, 6.2, 7.3), so a Runtime applies it without a ruling. A title's modifier step, where it still grants one, composes into the net ±3 cap (Section 4.3) like any other. A percentage reduction or bonus composes multiplicatively with every other percentage effect (Section 7.3) and **shares no budget with the step cap at all** — this separation is deliberate as of Version 1.101, whose rationale opens Section 16.2. Where two equipped titles both grant a step to one action they **both count toward that cap and neither is doubled**; where two equipped titles both grant a percentage to the same action the percentages compose multiplicatively and neither is capped by the other; where they apply to different things they simply both apply. **Kingmaker** and **Warden** are the passives whose effects land on someone else's roll: they apply to the allied hunter's action and are inert while nobody shares the fight. **Wellspring** is the one passive that moves a maximum: while equipped, maximum Mana is derived at ×1.10 and the pool is clamped, never truncated, when it is unequipped — an over-full pool falls to the new maximum and nothing else changes.

**Slots are the loadout, and they never reach the whole catalog.** One slot per assessment passed, capped at five through level 50, then **a sixth at 60, a seventh at 80, and an eighth at 100** (Section 16.4) — against a catalog of thirty-three, so a Bearer at the very top still runs a quarter of what he has earned and equipping stays a decision rather than a formality. **The slow rung past 50 is deliberate**: three further slots across fifty levels, where the first five came across forty, because a loadout that grows as fast as the catalog stops being a choice. Changing a loadout is free and instant, takes no action in a fight, and may be done any time `/system titles` can be opened. A newly equipped passive applies from the next resolved action; it never retroactively changes an exchange already rolled.

## 16.4 The Second Catalog — what a career past the ladder earns

**Section 16.1 already said the problem out loud and did nothing about it:** *"Past level 50 the ladder is spent and the decade is not; assessments continue at 60, 70, and beyond."* They did — against a catalog of twenty-two whose criteria a Bearer at the top of the ladder has largely exhausted, into a slot count frozen at five since level 50. **An assessment that can only ever report `none` is a ceremony**, and a level that grants nothing but a larger number is the defect this section closes.

**Eligible from the level-60 assessment onward.** Every entry below is tested by the same Section 16.1 machinery — the whole record since onset, no ruling, no partial credit, no published criterion — and grants alongside Section 16.2's entries in the same assessment. Nothing here supersedes the first catalog; a Bearer who reaches 60 still holding unearned Section 16.2 titles can earn them at any later assessment exactly as before.

**Two grades, and the point grants are honestly vestigial.**

| Grade | Titles | Points on earning | Character of the passive |
|---|---|---|---|
| **Ascendant** | 8 | **+15** | it changes how one of the world's own systems treats him |
| **Absolute** | 3 | **+30** | it changes something the profile states as law everywhere else |

**Say plainly why the points are not the reward.** Section 16.2's +2 / +5 / +10 were calibrated against a Bearer climbing from a creation array of 8s to a level-50 parity of 1,016, where five points was a real fraction of a Stat. Against the live campaign's base Stats near a thousand, **+15 is under two percent of one Stat and rounds to nothing in play.** They are kept, and raised enough not to insult, because Sections 3.2 and 4.2 name a title grant as a stat-point source and removing it would break that contract — but **the reward of a second-catalog title is its passive**, and every passive below is written to be worth holding a slot rather than worth the arithmetic.

**And the passives reach further out than the first catalog's do.** Section 16.2's twelve Common passives are percentage effects and small resource changes as of Version 1.101, which is right for a hunter learning his trade. These read the systems a Bearer only meets once he is past the ladder — directed assignment (Section 9.11), auditions (Section 9.12), the escalation ramp (Section 9.2), district pressure (Section 9.1.3), the contest test (Section 7.4), and his own carded standing (Section 19).

### Ascendant titles

| Title | Earned when the record holds… | Passive (while equipped) |
|---|---|---|
| **The Called** | **ten** Section 9.11 directed assignments accepted and cleared | every Section 9.11 assignment roll takes a further **−20**, and an assignment that resolves `NOT THIS ONE` is still reported to him by name and Rank |
| **Sealbreaker** | **five** Section 9.6 red gates entered, cleared, and walked out of | **15% reduced damage** taken on all actions inside any sealed instance (rebased at 1.101), and the exchange in which a seal closes around him costs no Mana |
| **Proven** | **one hundred** Section 7.4 qualifying contested scenes across all skills | a **combat** skill advances a mastery level on **two** qualifying scenes instead of three; the non-combat route is unchanged |
| **Bellwether** | a Section 9.1.4 major world event whose outcome the record credits to his action | the week's Section 9.1.4 roll is surfaced to him on the morning it fires — that it happened and which row, never its content |
| **Cordonholder** | a district carried from **Cordon** (Section 9.1.3, pressure 7) back to **Normal**, the Bearer part of it | Mana cost of all actions is reduced **15%** in any district standing at Strained or worse (rebased at 1.101), and pressure decays **one further** per clear week in any district he cleared a Gate in that week |
| **Deep Water** | a Gate cleared whose **true Rank stood two or more above its assessment** (Sections 9.5, 9.12) | an audition's true Rank is known to him **at the threshold** rather than on entry — he learns what he is walking into before he crosses, and may decline |
| **The Standing Name** | carded at a Rank his Section 19.4 contract cap was lifted to, with **ten** clears filed publicly under it | any action of standing, negotiation or testimony with a regulator, a guild, or the press resolves at **+30% effect**, and a failed attempt of this kind may be retried once in the same in-fiction week without the standing penalty a repeat attempt would ordinarily carry (rebased at 1.101 from a two-step grant no roll ever had two steps of headroom to pay) |
| **Ironstreak** | **two hundred** daily quests completed, consecutive or not (Section 8.1) | the daily's three rewards may be claimed **twice** on any day he also cleared a Gate |

### Absolute titles

| Title | Earned when the record holds… | Passive (while equipped) |
|---|---|---|
| **Worldmark** | an **S-Rank Gate** cleared with the Bearer **named** on a state-directed operation (Section 9.11) | his Section 9.5.1 anomaly band widens by a further **10 points**, and once per in-fiction week he may **refuse** an audition at the threshold, resolving that Gate as its assessment |
| **The Ledger's End** | **every entry in Section 16.2 held** — all twenty-two | every **earned** title's resource, recovery and yield effects apply **while dormant**; only modifier steps, percentage effects and stated capabilities still require a slot |
| **Before the Confluence** | a Gate cleared while the world stood at escalation **tier VII** (Section 9.2.1) | the escalation tier's anomaly band applies to him at **one tier above** the world's own, and he is **immune to the first anomaly-imposed condition or penalty** inflicted in that exchange (rebased at 1.101) |

**`The Ledger's End` is the one entry that reads the catalog rather than the world**, and it is deliberately the hardest thing in this profile to hold: twenty-two criteria including three Singulars, one of which (`Unbroken`) is a hundred consecutive dailies and another of which (`The Unmeasured`) is **foreclosed the moment he cards up**. A Bearer who took the Section 19.4 trade can never complete the first catalog, and that is the cost of the trade rather than an oversight. **A second, independent foreclosure was left unstated until 1.101: `Executioner` requires killing something standing above the Bearer's own System Rank, and System Rank S is this profile's ceiling — a Bearer who reaches it without already banking fifty such kills can never bank another one, `Executioner` (and therefore `The Ledger's End`) included, regardless of the Section 19.4 trade.** Whichever gate closes first, the same sentence applies: **it is stated here so nobody discovers it at level 90.**
...
## Carrying a Creature Out — what may cross with him (1.125)

*Anything left inside is gone with it* was written about loot, and it was the only sentence here that reached a living thing. `EVT-001221` carried a bound S-Rank Glasswalker out of an instant dungeon on the Bearer's voluntary exit, granted provisionally under Rules Section 4.0, and no section said whether a creature could cross at all, how long a hold on one lasts outside, or what its pools do on Earth. The owner ruled all three.

- **Only a carrying technique takes a creature across.** A creature crosses a seam with the Bearer only when he carries it with a technique whose authored effect is *carrying an unconscious or restrained creature through a seam* — in the live campaign, **Sample Handling**, a Succession technique (Section 7.6). Restraint alone is not passage: without the technique, a held creature cannot be brought through, and in an instant dungeon it ends with the instance as anything else left inside does. Nothing here lets a party member, a mender, or an unaided Bearer do it.
- **Any seam he can pass.** The technique's reach is its own wording, *a seam*, taken at face value: an instant dungeon's boundary on his exit or at the two-hour close, a public Gate's mouth, a Section 9.6 red gate once it releases him, and a far door (Section 18.13.1). The creature goes where he goes and by the route he takes; it never crosses where he does not. A creature carried out through a Gate mouth is **not a break** — Section 9.3's timer measures a Gate failing to hold its population, and this is one creature removed by hand.
- **Its pools behave normally.** A carried creature keeps its Rank, its Section 6.1 pools and its designation color, and spends and recovers on Section 5.2's fixed-Rank rules exactly as it would in a Gate. Earth neither drains nor suspends it; Section 5.2.1's draining environment is the far side alone and reaches nothing here. It is tracked as a **recurring** entity under Section 5.2's scene scoping — its pools persist between scenes and recover across the elapsed span — because it does not leave play at scene close.
- **A hold on it outside an instance lasts until midnight on the campaign clock.** A restraint technique holding a carried creature outside an instance — Binding Grip in the live campaign — holds **until the next 00:00 Chicago**, wherever the creature is kept. The Denver cage room keeps Chicago's midnight, not its own (Section 5.2 fixes `campaign_time` to Chicago).
- **Renewing it is maintenance, not a contest.** Recasting the same technique on a creature it already holds needs **no roll**: he pays the tier's ordinary Mana cost and the hold runs to the next 00:00 Chicago. He must reach the creature as the technique ordinarily requires; no range is added here. A lapsed hold is not a held creature, and restraining it again is an ordinary uncertain action.

**What this does not author.** What the world makes of a live Gate creature on Earth — a BGM finding, a jurisdictional question, a containment breach — is fiction, and no legal consequence is priced here. A creature he brought out is a danger he created, so Section 8.4.2's self-created exclusion already denies an Urgent offer for its escape; no new exclusion is written. The duration and renewal of a hold **inside** an instance are unchanged.
...
The Bearer chooses one. The choice is permanent; it attaches the class's `/system` panel (Section 15.5) and sets the Console's `Class` line. Each class has a later **evolution quest** — an authored class-quest at **level 50** — that deepens it along its own line, and a **terminal line at level 100**, authored in Section 18.13. Both rungs, what unlocks each, and how they relate are governed by Section 18.3.4. *(The pointer read Section 18.12 through Profile 1.101 and was wrong from the 1.100 split that created both sections: 18.12 is what the **world** does about him and expressly grants him nothing, while 18.13 is the class line. It also called the level-100 rung a "subclass," which 18.13 does not.)*
...
| **Evolution** | one authored unlock at level 50, and a **terminal line** at level 100 (Section 18.13); both governed by Section 18.3.4 | the far horizon, already carried by all five |
...
### 18.3.4 The Evolution Ladder — what unlocks a trial, and what a trial unlocks

Every class carries an **evolution quest hook at level 50** (Section 18.3.1's third track) and a **terminal line at level 100** (Section 18.13). Each hook was authored in its own class section and the rules governing all eight were never stated in one place, which is how two of them came to name no unlock condition at all and how the relationship between the two rungs came to be unwritten while a live Bearer stood at both.

**Every hook names a condition, and there are exactly two shapes.** A condition is a statement about the record, decidable from resolved Events without a ruling, exactly as Section 16.2's title criteria are.

| Shape | Condition | Classes |
|---|---|---|
| **A taking** | something is removed from a **boss** and kept — an extraction, an essence, a core, a relic | Shadowbinder, Soulforged Armory, Devourer, Reliquary |
| **A deed** | the class's own **premise demonstrated at its limit**, with no object taken | Echo Knight, Herald, Riftwalker, Seam |

**Which shape a class takes is not a style choice.** A taking is available only where the class's premise is *acquisition from a defeated thing* — and four of the eight are exactly that, so a boss is the obvious ceiling of the collection they already build. The other four earn their evolution by doing the thing the class exists to do, under conditions where it could most easily have failed, **because there is no boss-part for a portal, an echo, a loan of strength, or a closed puncture.** Riftwalker and Echo Knight were the two left conditionless through Profile 1.101; both now carry deeds, and the shape of those deeds is read off their own premises rather than invented for symmetry.

**A satisfied condition unlocks a trial. It never enters one.** The condition opens the evolution quest; walking into it is the Bearer's own act, on the Section 18.1 terms the class quest itself uses — **offered once, never expiring, and never re-offered.** A trial standing unlocked and unentered is an ordinary state of the campaign and carries no penalty, no decay and no deadline. **A Runtime never prompts him into one**, never treats a standing trial as a pending obligation in readiness, and never narrates the System pressing him toward it; the offer sits where it was made until he takes it. A trial open for fifty levels is not a defect in the record.

**An evolution trial is Section 18.2's structure, at Combat Tier, snapshotted at entry.** It reuses the class quest's machinery rather than authoring a second kind of trial: a sealed instance, fully lethal, no retreat, five phases and a final foe, **one budget with no refill** (Section 18.2.2), Section 6.5 throughout. Sizing at **entry** rather than at unlock is the same rule and for the same reason — a Bearer may prepare as long as he likes and the trial rises with him, so waiting neither cheapens it nor punishes him. Its phases are built to test **the class's own axis** rather than the five signals, which have already done their work; the class is chosen and the examination is now about what he did with it.

**The level-50 evolution is not a prerequisite for the Section 18.13 terminal trial, and the terminal trial carries it.** A Bearer may reach level 100 without ever having entered his evolution quest — the live campaign did exactly that, arriving at level 102 with both trials standing open — and the terminal trial is enterable regardless. What changes is its shape:

- **An unevolved Bearer's terminal trial runs six phases instead of five.** The additional phase is **his own evolution's examination**, built to the Section 18.2.3 standard and keyed to his class's own axis, and it runs inside the same single budget as the rest. It is the first phase, before the class suppression Section 18.13's trigger imposes on the middle one.

  **A folded-in phase is an examination, not the unlock deed replayed (1.103).** The **condition** is what opens the standalone evolution trial; the **phase** is what stands inside it, and the two are different objects. A Bearer taking the six-phase route **has not satisfied his condition and is never asked to** — the trial carries the evolution precisely because the condition was not met — so the additional phase tests the class's **axis** under Section 18.2.3 and never requires the deed itself to be performed. Where testing that axis needs apparatus the seal would ordinarily suppress, **the trial builds its own**: Section 18.8's trial-local anchors are the authored case, and the same principle governs any later class whose axis is bound by a rule the trial imposes. This was left implicit at 1.102 and misread at the mouth of the trial, which is where it was found.
- **Passing grants both lines** — the level-50 evolution and the terminal line together, in that order, in the same settlement.
- **Failing re-opens both** at the next decade assessment, on Section 18.13's existing terms.

**Why it subsumes rather than gates, and why it is not free.** Every terminal line is authored as **strictly the further rung of its class's own axis** — Gate Walker's permanent public tunnel is the personal gate one rung out, and Section 18.13 says so in its own words: *"the same shape as the Section 18.3 evolution, one rung further out."* A Bearer holding the terminal line while missing the evolution beneath it would hold a superset without its subset, which is incoherent on the profile's own account of what the two rungs are. **But a rung skipped is not a rung waived**: the sixth phase is the evolution's examination actually run, under one budget with no refill, against a trial already carrying three difficulties the level-50 version never had. He does not get it back cheaply; he gets it back **inside the harder thing**, which is the only place it still fits.

**A hard prerequisite was considered and rejected.** Requiring the Threshold Trial before Gate Walker is the cleaner ladder on paper and it strands the case the profile actually has: a Bearer past the level-100 trigger sent back to enter a level-50 trial that Section 18.2.2 would size at his *current* Combat Tier anyway, making it a second terminal-scale fight wearing an earlier name. The six-phase form collects the same cost in one trial instead of two.
...
- **Evolution quest hook — the Chorus Trial (level 50):** **stand a scene in which a full arsenal of echoes is deployed and every one of them disperses on interception** — the whole recorded set spent absorbing attacks meant for him, none expiring unspent, and the Bearer walks out of it. Unlocks **Simultaneity** — every recorded instant deploys together in one exchange for a single combined cost, and an echo dispersing on interception **records the attack that destroyed it**, filling its own slot from its own death. *The condition is the class's own premise at its limit* (Section 18.3.4): the Endurance grant is not a larger pool but attacks that do not arrive, and this is the scene where every one of them was stopped by a copy of him.
...
- **An anchored step is unrestricted, flat, and this is the class's real power (1.97).** Stepping to one of his **own set anchors** costs a **flat 5 Mana** from any distance — no band, no doubling, no ceiling — and **it is not bound by the walls above.** It crosses out of a Gate, out of a red gate, out of an instant dungeon, and back in. A Riftwalker standing in a sealed interior with an anchor outside it **is not sealed in**, and no other awakened on Earth can say that. That is what the class is, and it is deliberately not a small thing: Section 9.6's red gate is authored as the anomaly with *no retreat*, and this is the one power in the profile that answers it.
  ...
  **Two exceptions, and only these two: the Section 8.3 penalty zone and the Section 18.2 class trial.** Neither is a puncture in the world — both are instances the **System itself** builds and drops on him, and Section 6.7 already names them as the two content systems the System picks *for* him precisely because he cannot walk away from them. **The forge does not have a back door.** A penalty zone he could anchor out of would stop being a penalty, and it is the campaign's only remaining imposed danger source (`F-046`); a trial he could leave would measure nothing. An anchor set inside either is **unreachable from outside**, and **no anchored step crosses the wall in either direction.**

  **The bar is on crossing, not on the step (1.103).** Within a **Section 18.2 trial**, an anchored step **wholly inside the seal** — to a trial-local anchor, taken from inside the same instance — **is legal**, because everything the exception argues for is that he cannot *leave*: a step that begins and ends inside is not a back door, and a trial examining the anchor apparatus cannot forbid the apparatus and still be an examination. A **trial-local anchor** exists only inside the instance, **costs no slot** against `effective Agility ÷ 20`, and is released with it. **The penalty zone admits no such step at all** — it examines nothing, Section 8.3's whole content is that he is in it, and there is nothing there for an internal step to test. This states what the paragraph above always meant; through 1.102 it read *"unusable from within,"* which overshot the reasoning and made Section 18.3.4's folded-in phase unbuildable.
...
**The sense reports; it never authors.** A disturbance may be surfaced at an anchor **only where it would have occurred had no anchor been set there.** **Inside a Section 18.2 trial this constraint does not apply (1.103)** — the System builds the instance and everything standing in it, so a disturbance manufactured at a trial-local anchor **is** the examination rather than a breach of the reporting rule. The constraint governs **the world**, where authoring an event *because* a place is anchored would let the sense create what it claims to detect. The Runtime may not author an event at a place *because* it is anchored, may not use the sense to force a return, and may not treat it as a hook. Section 9.1's tick advances the world on its own clock whether or not he is watching; the anchor is a window onto that, not a reason for it.
...
- **Evolution quest hook — the Threshold Trial (level 50):** **answer a disturbance at a set anchor while standing another city or further from it, arriving in the exchange it reached him** — the standing anchor sense, the anchored step and the flat any-distance cost proved as one apparatus rather than three grants, with the distance that should have made him too far away doing nothing at all. *(1.102 phrased the distance as "another ledgered container," which is retired 1.96 pricing vocabulary — 1.97 replaced the container lookup with the soft bands above, and the phrase now names a scale keyed to those bands instead. The deed is unchanged; only its unit of distance is stated in live terms.)* Unlocks the **personal gate** — a stable two-way portal between **two of his own anchors**, holding for a scene, Mana scaling by distance: travel and infiltration the world's own Gates never offered. The anchors he spent twenty-five levels setting are the network it runs on. *The condition is the class's own premise at its limit* (Section 18.3.4): a Riftwalker's claim is that where he stands stops constraining where he can be, and this is the scene that proves it against a clock.
...
## 18.12 The Second Ascension — what the world does about him, at level 100

**Level 100 grants the Bearer nothing, and that is the entire design.** He arrives at it already able to clear content the world has no answer for, and a hundredth level that handed him more reach would be the fourth consecutive system to answer *"nothing new"* with *"more of what you already have too much of."* Sections 16.4, 9.11, 9.12 and 17's proving key all did that, correctly, for a Bearer who needed them. **This one changes the world instead.**

**Nothing here is a power, and nothing here is optional.** These are standing changes to the world's own tables, they fire when the level-100 assessment settles, and they do not switch off. The Bearer does not choose them, cannot decline them, and holds no new ability at the end of it. **What he gets is a world that has reorganised itself around the fact of him** — which is the one thing in this profile gold cannot buy, levels cannot reach, and no Section 16 title grants.

### 18.12.1 The instrument has no reading for him

**This is already true and Section 19.2 never noticed.** Its re-assessment band table classifies a hunter by measured maximum Mana, and its top row is **S-Rank at 1,225 and above** — an open bracket written when nobody imagined its floor being cleared several times over. The live Bearer measures **8,036**, more than six times that floor, and the table returns *S-Rank* for him exactly as it does for an ordinary S at 2,000.

**From the level-100 assessment, a re-assessment on him returns no Rank at all.** The instrument reports **beyond scale** — not a classification, a refusal. Its needle is at the stop and the appraiser has never seen one there.

**Every rule keyed to his card Rank therefore loses the thing it reads.** Section 9.4's legal minimums count cards; Section 19.4's contract cap is a card; Section 13.2's terms are written against one. **The world does not have a category for him, and a regulator cannot write a contract against `beyond scale`.** This is not permission to ignore the law — it is the law having nothing to say, which is a **political** problem for BGM and the European Gate Council long before it is a convenience for him. What they invent to replace it is campaign canon and belongs to play.

### 18.12.2 The board stops competing

Section 9.1 step 4's take rates are the trade's economics: an A-Rank posting is taken at **90% a day** because *"every major guild wants it."* **They stop wanting it.** A guild that spends a bench, a week, and a casualty list on an A-Rank Gate one man will clear for nothing is a guild explaining that to a board.

**From the ascension, A-Rank take rates fall to 25% a day and S-Rank work is not tendered at all.** Two consequences, and the second is the one that matters:

- **More high-Rank work reaches him**, which looks like a reward and is mostly a workload.
- **The trade's top-end revenue collapses.** Section 12.3 prices A-Rank work at national-asset scale, and guilds are built on it. Section 9.1.4's *a guild falls* row (64–71) is **rolled at double weight** from here, and when it fires the Runtime attributes it honestly: the guild did not fail, it was made unnecessary.

### 18.12.3 The draft points the other way

Section 9.3 names an **S-Rank draft** that binds S-Ranks to emergency mobilization, and Section 9.11 built him a call list. **He is no longer on a call list. The call list is him.**

Every A-Rank and S-Rank Gate the Section 9.1 tick rolls in his jurisdiction is **offered by default** — the Section 9.11 roll is not made. What replaces it is not a roll but a decision, taken every time, and **what he declines is a world event.** The Gate does not vanish because he said no: it goes to Section 9.10's board at the new take rates, it runs its Section 9.3 timer, and it settles as the board settles anything. **The Runtime reports what happened to every Gate he refused**, at the tick that settles it, whether or not he asks.

### 18.12.4 The world changes shape around one person

Section 9.1.4's weekly table describes a world changing shape on its own. From the ascension it gains three rows the Bearer is the cause of, replacing the `01–55` **the world holds** band's lower half — because a world with him in it holds less often:

| d100 | Event |
|---|---|
| 01–33 | **The world holds.** Ordinary week; the daily table still runs. |
| 34–41 | **Something reorganises around his availability.** A guild restructures, a regional office redraws its response plan, an insurer reprices, a contract standard is rewritten. Someone builds a plan whose load-bearing assumption is one man. |
| 42–48 | **A state asks for him.** Not Chicago's — another jurisdiction, through channels, for something it cannot answer. Refusing is free and is itself news. |
| 49–55 | **An awakening that should not read the way it reads.** Someone measures wrong, grows, or survives what should have killed them. **Authored as a fact and never explained**; the cause is owner-facing (`200_WORLD_BIBLE.md` Section 9). |

The remaining rows (56–00) are unchanged.

### 18.12.5 The cost, which is the point of the section

**A world that acquires one person who can answer anything stops building the capacity to answer it itself.** That is not a punishment aimed at the Bearer and it is not reversible by good behaviour. It is what a system does when it acquires a single point of failure.

- **Section 9.1.3 district pressure decays one slower in every district he did not work that week.** Benches thin, the mid-tier hollows out, and the response that used to exist has been reorganised around someone who was somewhere else.
- **Where he goes, the world is safer than it has ever been. Where he does not, it is worse than before he existed** — because what used to be there is gone.
- **He cannot be in two places.** Section 18.12.3 hands him every A-Rank and S-Rank Gate in the jurisdiction, and Section 9.2.1's ramp is raising how many that is. The arithmetic of that is the campaign's, not this section's, and it is meant to bite.

**This is the reward, stated as plainly as it can be.** Not a number, not a reach, not a door. **The world is measurably different because he is in it, and it is not uniformly better** — and no other thing the System has ever given him has been able to say that.

## 18.13 The Terminal Line — what the class becomes, at level 100

Section 18.12 is what the **world** does at level 100. This is what the **Bearer** does, and the two are meant to be read together: the tunnels below are exactly the kind of thing a world reorganises around.

**One line per class, on that class's own axis, permanent, and no menu.** The same shape as the Section 18.3 evolution, one rung further out. What he chose at the class quest is what he finishes as.

**Trigger.** The level-100 assessment opens a trial — Section 18.2's structure at **Combat Tier**, five phases, one budget, no refill and no retreat, Section 6.5 throughout — with three differences: **his class is suppressed for the middle phase**, **one phase cannot be resolved by killing** (restraint, protection or refusal; Section 6.2.1's pulled strike is resolvable and lethal force is not a route), and **the fifth phase is instantiated from his own record** — Section 9.7's formula filled from resolved Events, an interior that answers to a career rather than an archetype. **A Runtime never explains how the instance knows him.** Passed, the line attaches; failed, it re-opens at the next decade.

**The level-50 evolution is not a prerequisite, and an unevolved Bearer's trial runs six phases** (Section 18.3.4). The additional phase is his own evolution's examination, run first, before the class suppression above; passing grants **both** lines in one settlement and failing re-opens both. Everything else in this section is unchanged by it — the trial is the same trial, one phase longer, on the same single budget.

### 18.13.1 Riftwalker → **Gate Walker**

He has spent a hundred levels stepping through holes other things made. **He stops being a thing that uses Gates and becomes a thing that makes them.**

- **The tunnel network.** Any two places he has stood may be joined by a **permanent two-way gate**, holding indefinitely, that **anyone may cross** — awakened or not, with or without him, whether or not he is on Earth. Bounded by his anchor cap (`effective Agility ÷ 20`), one slot per standing tunnel, released freely at will. Chicago to Prague is one step for a courier, a crew, or a strike team. **This is a change to the world's logistics rather than to his own reach**, and Section 18.12's reorganisation is the world discovering it.

- **The far door.** Section 10 says what a Gate interior is: *a slice of the beasts' own world held in the void between it and ours.* **He opens onto the world itself.** Not a slice, not a pocket, not an instance — **the place the slices are cut from.**

**A far door is not a Gate, and almost nothing in Section 9 applies to it.** This is the whole of what makes it worth a hundred levels, so it is stated as a list of what is absent:

- **No Rank.** It is not assessed, not classified, and Section 9.2's distribution has nothing to say about it. Neither does Section 9.5's true-Rank roll or Section 9.6's anomaly table.
- **No population formula.** Section 9.7 does not fill it. **What is there is what is there**, and it was not put there for him.
- **No boss, and therefore no clear.** A dungeon is a bounded pocket whose apex thing dies and takes the pocket with it. **A world has no bottom.** Nothing he kills closes it and nothing he kills finishes it.
- **No collapse window and no timer.** Section 9.7's two hours and Section 9.3's countdown are properties of a puncture that is trying to close. **A far door is not trying to close.**
- **And it is two-way.** Section 9.8's threshold — the barrier that holds beasts in until a break dissolves it — is a property of a Gate. **A far door has no barrier.** What is on the other side may walk through it, at any time, in either direction.

**The System pays nothing for any of it, and the reason is the information boundary rather than an anti-farm rule.** Nothing on the far side carries a Rank, so Section 3.3 has no row for it, Section 6.1 has no Rank Health for it, Section 3.4 has no clear to award, and Section 11 has no drop table to roll. **The System has never seen these things and does not price them.** A far door is not a progression space and cannot be made into one — which is precisely why it is the capstone rather than another key.

**What he can carry back is real and has no provenance at all.** Matter is matter: he can bring things out. Nothing he brings is Ranked, appraisable, or licensable, no market has a price for it, and no chain of custody in either jurisdiction can account for it. Section 19.1's exposure surface has no wider case anywhere in this profile.

**Closing it, and the one condition that matters.** He may close a far door he opened, at the cost he opened it for, **from the side he opened it on — and only while nothing is standing in it.** A door with something coming through **cannot be shut.** He gets it back when the way is clear, and not before.

**Say plainly what this is.** `200_WORLD_BIBLE.md` Section 9 authors a terminal event for this world that is **a single break that does not close**. A Gate Walker's far door is that, in miniature, by his own hand, and the profile does not pretend otherwise. **The power is unbounded and the consequence is the setting's own ending arriving early if he is careless with it.** Opening one costs **2,500 Mana**, the only fixed price in this section.

**The price is derived from the Bearer the grant is written for (1.105).** Through 1.104 it read *Mana equal to S-Rank Rank Health — 4,000*, which sized a doorway against the fixed-Rank table rather than against the man paying for it; when Section 3.2's rungs were re-authored and the Bearer's pool stopped inflating, that flat figure silently became **73% of his pool per crossing** and a round trip stopped fitting in one pool at all. It is now anchored to the class's own entry condition: **a level-100 Bearer with nothing allocated** carries `automatic(100) = 2,508` plus the weakest legal creation Stat of 8, for Intelligence 2,516 and a **maximum Mana of 5,032** (Section 5.1) — so **a round trip is one full pool**, and the door is half of it, rounded to a stated price. **Opening and closing a far door is the whole of what a fresh Gate Walker can afford in a day, and everything he does past that he pays for out of what he has built since.**

**What is on the far side is authored, and it is not in this file.** `worlds/gatefall/270_THE_FAR_SIDE.md` holds it — **owner-facing**, on the same footing as `200_WORLD_BIBLE.md` Section 9, and a Runtime surfaces it only through what the Bearer physically perceives while standing in it. It authors five strata, the law that governs them, and who is there. Two things it establishes bind this section directly: **nothing there carries a Rank**, so every authored entity states its magnitude as a Combat Tier equivalent instead; and **an awakened human accumulates roughly a day of ordinary Gate-work's mana saturation every hour he stands in it** (`200_WORLD_BIBLE.md` Section 5's gray sleep), which is the clock on every expedition and the reason the place cannot be lived in.

**That clock is Section 5.2.1, and it runs on his Mana.** The saturation figure is what his body is *offsetting*, not a quantity anyone tracks: the pool is the organ that sheds it, so the cost of standing there is read off the pool. **Section 5.2 recovery does not accrue on the far side and the pool falls by 10% of maximum Mana per hour** — a full pool is ten hours, and the 2,500 this door costs is spent before he arrives. At zero the drain crosses into Health (Section 5.3), and Section 12.5's Greater potions degrade to their Standard form there, so **nothing in the shop buys him more time.** No new counter exists anywhere for this and none is to be invented: the expedition's whole budget is the two pools already on his sheet.

**A Runtime that reaches ground `270_THE_FAR_SIDE.md` does not author stops and asks the owner**, exactly as it does for any unauthored mechanic — the Stacks are indefinitely large, five strata are written, and **it never improvises a world.**

### 18.13.2 The other seven classes — owed, and named as owed

**Only Riftwalker's line is authored.** Shadowbinder, Soulforged Armory, Devourer, Echo Knight, Reliquary, Herald and Seam each owe a named trial, terminal grants on their own axis one rung past their level-50 evolution, and a stated reason that pair is that class's ending.

**This is a real gap and not a placeholder.** Gatefall has one campaign and one Bearer, who took Riftwalker at `EVT-000519`; authoring seven terminal lines for classes nobody holds would be inventing content against no evidence, which is how Section 18.8's pocket cache and Section 7.4's Broken Rhythm both came to grant nothing. **A class reaching level 100 without its line authored does not improvise one** — the Runtime stops and the owner authors it.

**No terminal line has a growth ladder past it.** Every ladder in this profile reads against System Rank, spent at 50; one running from 100 upward would author rungs against a horizon the world does not have, now that Section 9.2.2's ramp has a dated end.
