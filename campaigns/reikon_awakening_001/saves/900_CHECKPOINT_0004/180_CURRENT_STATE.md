...
- **Latest restorable checkpoint:** `saves/900_CHECKPOINT_0004/` — migrates the recovered post-pack-leader state to Reikon World Rule Profile 0.3 with canonical Health-recovery state.
- **Prior restorable checkpoint:** `saves/900_CHECKPOINT_0003/` — captures the same post-pack-leader fiction before the Health-recovery migration and remains valid for an explicit fork/load.
...
3. **Injury management.** Daedalus is at 60/100 Health with a painful moderate shoulder/collarbone wound that can hinder physical action until treated or healed. Health recovery is `paused` during the active Rift threat; care is `untreated` (bleeding is self-stabilized) with zero carried recovery units. Safe rest or light activity starts deterministic recovery under Profile 0.3.
