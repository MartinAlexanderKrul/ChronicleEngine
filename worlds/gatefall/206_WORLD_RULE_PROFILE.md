# Gatefall — World Rule Profile 1.62
...
```text
[SYSTEM] FLUX SIGHT — 10 × Ashroot Crawler · [E-Rank] yellow · HP 40/40 · MP 20/20 each
[SYSTEM] FLUX SIGHT — Ashroot Matriarch · [E-Rank] yellow · HP 160/160 · MP 80/80
...
During prose, the Bearer's own numeric changes render as bare compact lines carrying the delta, the new standing, and the source:
...
Each shows `before → current/maximum` (for XP, `current/next threshold`). Pool labels are `HP` and `MP`, always. Render each changed quantity once per resolved action. Messages carrying a subject or more than a number — the Flux Sight read, level-up, daily quest — take the bracketed `[SYSTEM]` form instead.

**This is a mirror. Section 14.5 is the authority**, and it authors the full contract: the grammar, both weights, the Flux Sight read template, grouping, and re-render cadence. The two must never diverge.
...
Everything the System says inline during prose shares **one grammar and two weights**. This is the A10 notification contract, identical to Section 8.2 — the two must never diverge.
...
### 14.5.2 Weight one — the Bearer's own numbers, bare

His own pools and XP move several times an exchange, so they render **bare**, with no prefix:
...
### 14.5.3 Weight two — reads and events, bracketed

A message carrying a **subject** or more than a number takes the `[SYSTEM]` prefix: the Flux Sight read, level-up, mastery, quests, rotation, title assessment, warnings (Sections 3.2, 7.4, 8.1, 8.4, 12.5, 16.1).

**Every bracketed message shares one head**, and this is the same family the quest and title blocks have always used:

```text
[SYSTEM] <EVENT NAME IN CAPS> — <subject>
```

The subject is omitted where the message has none (`[SYSTEM] HIDDEN QUEST DETECTED`). **What differs below the head is the body, and it differs by a rule rather than by taste:**

- **A fixed tuple renders inline on the head row**, `·`-separated, in the Section 14.5.1 order. The fields are always the same fields in the same order, so naming them would be noise — the same reason a bare pool line does not say `Pool:` and `Delta:`.
- **A variable field set renders as labelled rows** beneath the head, one field per row. The reader cannot know which fields are present, so each states its own name.

```text
[SYSTEM] LEVEL UP — Level 15 · all Stats +1 · Ability Points +5 · HP/MP restored.
[SYSTEM] SKILL MASTERY — Mana Bolt is now Adept.
[SYSTEM] FLUX SIGHT — Crypt Warden · [C-Rank] yellow · HP 1000/1000 · MP 500/500
...
A Flux Sight read is a fixed tuple — subject, Rank, color, HP, MP — so it is one row. A quest carries objectives, rewards, deadlines and warnings in combinations that vary by quest, so it is labelled rows. **Neither shape is available to the other**: a read never grows labelled rows, and a quest never compresses onto its head.

### 14.5.4 The Flux Sight read (exact)

A Flux Sight read (Section 4.4) is Tier 1, fires on sight with no roll and no Mana, and **carries the designation color on the same line** — the color is derived from the Rank the read states (Section 14.6), so rendering them separately says one thing twice.

```text
[SYSTEM] FLUX SIGHT — <subject> · [<Rank>-Rank] <color> · HP <cur>/<max> · MP <cur>/<max>
```

Worked, at System Rank C:

```text
[SYSTEM] FLUX SIGHT — Crypt Warden · [C-Rank] yellow · HP 1000/1000 · MP 240/240
[SYSTEM] FLUX SIGHT — 10 × Crypt Ghoul · [E-Rank] gray · HP 40/40 · MP 20/20 each
[SYSTEM] FLUX SIGHT — Marisela Kwan · [B-Rank] orange · HP 600/600 · MP 300/300
[SYSTEM] FLUX SIGHT — Gate, Lakefront Corridor · [D-Rank] · confirmed by capability
```
...
**Grouping.** Identical commons render as one group line with `<n> × <name>` and `each` after the pools. **Anything individually consequential is named on its own line** — every elite, every boss, every named hunter, and any common whose pools have diverged from its group. A group line is a compression of identical subjects, never a summary of different ones.

**A populated room renders one room line, then individuals on engagement.** Where the Bearer perceives many awakened at once — a guild hall, a staging floor, a bar — the read does not emit one line per person. It emits a single line giving the room's shape, counts by Rank with each Rank's color, and then a **full individual read on anyone he engages, is approached by, singles out, or fights**:

```text
[SYSTEM] FLUX SIGHT — 20 awakened present · [B-Rank] ×1 orange · [C-Rank] ×6 yellow · [D-Rank] ×13 white
```
...
**Full read on first perception of a subject in a scene** (for an awakened he has met before, Section 14.5.6 governs instead). Thereafter that subject's pools re-render **only in an exchange the Bearer's own resolved action changed them**, as a bare line in the Section 14.5.1 grammar carrying the subject — the same rule his own numbers already follow, pointed outward:

```text
Crypt Warden · HP −258 · 1000 → 742/1000
```

An exchange that changes nothing about a subject renders nothing about it. The full bracketed read repeats only if the subject's Rank is re-resolved or the Bearer's System Rank changes mid-scene and recolors it (Section 14.6).
...
[SYSTEM] FLUX SIGHT — Owen Callahan · [C-Rank] yellow · HP 198/275 · MP 137/137
```
