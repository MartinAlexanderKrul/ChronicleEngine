...
A diegetic command's `dispatch` may name **more than one section**, and must name every section its render actually obeys. A panel's own template is not always the whole of its procedure: a world may hold the frame grammar in one section and compose several templates in another, and a command dispatching only its own heading produces a plan carrying every value it renders and none of the layout it renders them into — which is how a panel comes back narrower than the template it claims to obey. A command that composes other panels dispatches those panels' sections too; the composing section is an instruction to render them, not a substitute for them. A command's declared `aliases` resolve to the same plan, reported as `alias_of`, because the profile requires a plan before a diegetic command and an unresolvable alias leaves the likeliest player spelling with no plan at all.

A `required_live_reads` entry is a path — a whole-file read — or a mapping of `file` and `heading` where the ledger holds the rendered block in one place. Narrow only where the named heading carries the *current* value: a ledger whose current figure is derived across dated history is read whole, and a heading selector there renders a stale number with a plan that looks correct. Declared `protagonist_fields` bind to the protagonist's own record whether or not the character sheet is separately listed as a live read.
...
The validator scans live Markdown under `worlds/` and `campaigns/`, excluding save and checkpoint snapshots from live duplicate-definition checks. It verifies registry high-water bounds and allocation-log coverage, unique live object definitions, resolved identifier references, universal Persistent Object fields, Canonical Record references, placeholder rejection, and the world-specific mechanically decidable state relations described in the Save Algorithm. It does not infer omitted real-world facts from prose; the promotion audit matrix remains responsible for completeness until those facts have typed storage.
...
1. **Promotion Barrier first (unchanged).** Run Canon Reconciliation at Promotion. If an unreconcilable contradiction exists, reject the mutation, record a Rejected Simulation, and write nothing. The barrier runs before any write, so contradictory canon never reaches the repository.
   ...
   Build a bounded **promotion audit matrix** for the entire unpromoted span before declaring the barrier passed. One row per resolved Event records: exact start/end anchor when time advanced; Health and Mana entering and leaving after costs, recovery, restoration, and cap; every known skill activated or materially applied (including automatic passives); whether each use succeeded; whether the scene qualified; and the exact `counter_deltas` owed. Reconcile the row totals against the live pools, temporal carries, `tracked_counters`, and rendered skill lines. A prose summary such as "already full", a sibling skill delta, or `promotion_barrier_passed: true` is not evidence that this matrix was performed.
...
1a. **Write the narration audit.** Record `narration_telemetry` on Current State for the span just closed: `as_of` at `campaign_time`, counts of runtime voice breaches, player disputes and deferred world answers, and one row per **speaking** NPC -- record loaded before its first line, lines, questions aimed at the protagonist, beats of its own, and the one voice tell from its record that showed up. Step 5 rejects a stale `as_of`, three-plus lines with no beat of its own, two NPCs sharing a tell, a record never loaded, and a breach count with no `breach_note`. **A block of zeros written to clear the gate is worse than none, because the next reader trusts it** (`F-072`).
...
3. **Derive the complete live target set, then attempt the canonical update.** A checkpoint is not a single-ledger write. The target set is **derived from the session's events, never judged from memory of what the session felt like it touched.** For every event promoted this session, take every identifier that event references — its subjects, the objects it moved, harmed, consumed, created, or relocated — and add **the owning ledger of each** to the target set. Then add the ledgers that always carry a played session: Current State, chronicle, changelog, and `system/ID_REGISTRY.md` for new identifiers. Objectives, relationships, inventory, knowledge, and world promotion enter the set by the same derivation, not by recollection.
   ...
   **Every Character an Event references also targets** (Decision 095): its Relationship with the Bearer — re-read `qualities` and `type` against the `state` you write, and advance `qualities_as_of` — and its own `location` and `condition`. The gate checks the first by Event number; nothing can check the second.
