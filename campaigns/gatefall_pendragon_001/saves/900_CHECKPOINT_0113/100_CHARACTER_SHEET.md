...
provenance:
  source: EVT-000645
  game_date: "2026-08-22T20:40:00-05:00"
...
canonical_state:
  ...
  system_state:
    ...
    temporal_state:
      campaign_time: "2026-08-22T20:40:00-05:00"
      mana_recovery_mode: resting
      mana_recovery_remainder_units: 0
      health_recovery_mode: resting
      health_recovery_remainder_units: 0
      note: "**Settled at session close (`EVT-000642`), anchor 19:10 → 20:35, 85 minutes stationary at Nakamura's bench** — mode switched active/light → resting on arrival (Section 5.2/6.1.1's mode-change rule, zero elapsed time at the switch instant itself). Both pools were already within 1 point of cap; the resting-rate accrual over 85 minutes overflows the gap by roughly two orders of magnitude, so Health closes at full cap **2504/2504** and both remainders zero per the at-cap rule. No Mana spent this span — the Dormant Core read and the Seamwork read are both native-rung/passive, no cost; the fusion's opening phase spent no Mana either, being Nakamura's own working rather than a Bearer action.\n\n  **Prior:** **2026-08-22 16:00 (`EVT-000619`): anchor 14:00 -> 16:00.** Mana entered 972/990 active and recovered to cap on the 14:00-14:26 walk; both pools were then restored to their NEW maxima by the level-41 award (Section 3.2 step 4), so both remainders are zero at full. No damage was taken across five D-Rank instances and no Mana was spent - weapon kills only, Resonance Extraction being passive. Prior: Five scalars, by owner ruling 2026-08-10. How the anchor advanced is narrated by the promotion Event that moved it and by `170_CHANGELOG.md`, neither of which can drift from the clock the way a note sitting beside it can. Health closed this span at full cap. Mana closed 18 short of cap this span (one E-Rank anchored rift-step at 5, one Keen Sense at 1, spent while out on errands) and its remainder is zeroed pending the next settled rest span; no elapsed-time recovery narrated between activations."
    unspent_points: 5                      # rendering of `ability_points_earned - sum(allocated)` = 337 - 332 (Profile 1.82)
    pending_rewards:
      ability_points: 0
    ...
    stats:
      ...
      intelligence: 623
    effective_stats:
      strength: "581 (base 567 + equipment 14)"
      agility: "587 (base 543 + equipment 44)"
      vitality: "626 (base 625 + equipment 1)"
      perception: "549 (base 544 + equipment 5)"
      intelligence: "630 (base 623 + equipment 7)"
      note: "**2026-08-22 16:00 — LEVEL 41 (`EVT-000616`).** The Section 3.2 System Rank A rung pays **+63 to every base Stat**; equipment lines are unchanged, so every effective figure moves by the same +63. **Maximum Health 2,000 -> 2,252** (4 x effective Vitality 563); **maximum Mana 990 -> 1,116** (2 x effective Intelligence 558). Both pools restored to the new maxima in the same exchange. **The Section 19.2 S-Rank licence gap falls 243 -> 109 maximum Mana**, inside one further level; Combat Tier S's Health gap falls 700 -> 448. No Title Assessment — 41 is not a decade level, and the fifth title slot still waits at 50.\n\n  **Prior level-up and Stat history is not restated here.** It ran to roughly 7,600 bytes of superseded narration inside a single field and was the largest line in this record; `160_CAMPAIGN_CHRONICLE.md` and `170_CHANGELOG.md` hold every crossing in full, with the Events that caused them. Compressed at `EVT-000619` when the `gatefall_protagonist_object` ratchet failed at 162,328 bytes against 159,345 — the growth being exactly the duplicated event history the budget names. **Load-bearing values live in `stats`, `effective_stats`, `tracked_counters` and `pending_rewards`, not in this note.**"
    system_rank: A
    equipment:
      main_hand: "Finished dagger [S-Rank] — Quickknife chassis, all-Agility weapon stat line · Agility +22, weapon power 22 (Section 11.5 S-Rank weapon budget) · **effective chassis ×2.00** with Dagger Mastery [A-Rank] Expert Adept's +1.20 (`EVT-000588`) · **DMG 1206 standard before reduction** at effective Strength 581 — `(581 + 22) × 2.00 = 1206`, rounded to nearest under Section 6.2 (re-derived at `EVT-000616`'s level-41 crossing from 930 at effective Strength 455; at `EVT-000568` from 408 at effective Strength 218, the level 30–34 crossings and the Acc.2 vambrace swap moving it; at the Profile 1.80 backfill from 206 at effective Strength 99; before that at `EVT-000548`'s leg swap from 194 at effective Strength 92) · armed strike ×0.75 base — recovered from the loading dock sweep (`EVT-000326`), auto-identified S-Rank only (`EVT-000328`, no chassis/provenance beyond Rank at the time); its full combat line is authored here from the standard S-Rank weapon table on first equip in combat (`EVT-000339`). Equipped to main hand mid-fight, displacing the Ghost Quickknife. **Not owned in any ordinary sense** — the missing artificer's own commissioned work, held rather than owned (`120_INVENTORY_AND_OWNERSHIP.md`); equipping it is a live Section 19 exposure risk Alexander has accepted knowingly."
      off_hand: "Ghost Quickknife [A-Rank] — Premium model, Excellent · Agility +18 (A-Rank weapon budget 16 + 2 premium) · weapon power 16 · **effective chassis ×2.00** with Dagger Mastery [A-Rank] Expert Adept's +1.20 · **DMG 1194 standard before reduction** at effective Strength 581 — `(581 + 16) × 2.00 = 1194`, rounded to nearest under Section 6.2 (re-derived at `EVT-000616`'s level-41 crossing from 918 at effective Strength 455; at `EVT-000568` from 398) · armed strike ×0.75 · +1 modifier step on the first attack made unseen in a combat · bought from the Daily Premium Umbral cycle, 50,000 g (`EVT-000557`) · EQUIPPED, displacing the Ghost Quickknife [B-Rank] (Agility +13, weapon power 11, same perk), now banked unequipped, unsold."
    ...
    tracked_counters:
      ...
      - { path: progression.ability_points_earned, baseline_value: 323, baseline_as_of: EVT-000597, current_value: 337 }
      ...
      - { path: skills.flash_step.successful_uses, baseline_value: 6, baseline_as_of: EVT-000130, current_value: 75 }
      ...
      - { path: skills.keen_sense.successful_uses, baseline_value: 1, baseline_as_of: EVT-000130, current_value: 92 }
      - { path: skills.keen_sense.qualifying_scenes_total, baseline_value: 1, baseline_as_of: EVT-000130, current_value: 63 }
      ...
      - { path: skills.exploit_pattern.successful_uses, baseline_value: 0, baseline_as_of: EVT-000132, current_value: 85 }
      - { path: skills.exploit_pattern.qualifying_scenes_total, baseline_value: 0, baseline_as_of: EVT-000132, current_value: 46 }
      - { path: skills.exploit_pattern.mastery_progress, baseline_value: 0, baseline_as_of: EVT-000132, current_value: 0 }
      - { path: skills.mana_bolt.successful_uses, baseline_value: 0, baseline_as_of: EVT-000303, current_value: 81 }
      - { path: skills.mana_bolt.qualifying_scenes_total, baseline_value: 0, baseline_as_of: EVT-000303, current_value: 20 }
      ...
      - { path: skills.resonance_extraction.successful_uses, baseline_value: 0, baseline_as_of: EVT-000132, current_value: 44 }
      ...
      - { path: skills.flux_sight.successful_uses, baseline_value: 0, baseline_as_of: EVT-000130, current_value: 90 }
      ...
      - { path: skills.silent_step.successful_uses, baseline_value: 0, baseline_as_of: EVT-000235, current_value: 40 }
      - { path: skills.silent_step.qualifying_scenes_total, baseline_value: 0, baseline_as_of: EVT-000235, current_value: 20 }
      ...
      - { path: skills.dimensional_projection.successful_uses, baseline_value: 0, baseline_as_of: EVT-000436, current_value: 25 }
      - { path: skills.dimensional_projection.qualifying_scenes_total, baseline_value: 0, baseline_as_of: EVT-000436, current_value: 14 }
      ...
      - { path: skills.rift_step.successful_uses, baseline_value: 0, baseline_as_of: EVT-000519, current_value: 9 }
    ...
    daily_premium:
      ...
      offers:
        ...
        rune: "Premium Rune — teaches **Mana Bolt** · [A-Rank] · UNBOUND · NPC-ELIGIBLE · 500,000 g · unpurchased — **not inert**: he holds Mana Bolt at [B-Rank] Expert, and this offers ascension to [A-Rank], genuinely above current Rank"
