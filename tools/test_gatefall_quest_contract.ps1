...
Assert-True ($profile -match '(?m)^# Gatefall .+Profile 1\.19\r?$') "Gatefall Profile 1.19 is not active."
...
Assert-True ($character -match 'profile_version: "1\.19"') "Live Gatefall character was not migrated to Profile 1.19."
...
Assert-True ($startup -match 'world_rule_profile: "Gatefall World Rule Profile 1\.19"') "Campaign startup does not bind Profile 1.19."
...
Assert-True ($startup -match 'Sections 7\.1, 8\.4, and 14\.3 before readiness completes') "Gatefall startup does not preload the proactive trigger contracts."
...
Assert-True ($index -match 'World Rule Profile 1\.19, frozen') "World index does not advertise frozen Profile 1.19."
